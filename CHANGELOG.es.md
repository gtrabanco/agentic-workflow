...
|---|---|---|---|
| 4.1.1 | 2026-09-09 | parche | **Bun runtime de primera clase (#197)**: los scripts del paquete invocan bun como runtime (`bun test`, `bun scripts/…`, cadenas `bun run`) con una variante explícita de fallback `test:node` bajo node; `engines` añade `bun ">=1.4"` junto a `node ">=18"`. Sin cambios en `src/`/dist/schemas — los scripts ya eran módulos ES agnósticos del runtime (los especificadores `node:` están soportados por diseño en bun). El job node-compat del workflow de publish garantiza el fallback. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 0.7.2 | 2026-09-09 | parche | **Bun runtime de primera clase (#197)**: los scripts `bundle:skills`/`test`/`mutation`/`prepare` invocan bun (`bun test` sobre la suite estilo `node:test` — verificado en 1.4.2), con variante explícita de fallback `test:node`; `engines` añade `bun ">=1.4"`. Re-empaqueta las 4 skills de redacción de runtime — `pre-execution-review` 2.2.1 (nota de convención en la columna validator de LEDGERS), `review-spec` 1.7.1, `review-plan` 1.6.1, `workflow-status` 3.2.1 (comandos de snapshot bun-first). |
