...
|---|---|---|---|
| 4.1.1 | 2026-09-09 | parche | **Bun runtime de primera clase (#197)**: los scripts del paquete invocan bun como runtime (`bun test`, `bun scripts/…`, cadenas `bun run`) con una variante explícita de fallback `test:node` bajo node; `engines` añade `bun ">=1.4"` junto a `node ">=18"`; devDependencies fijadas a versiones exactas (`ajv 8.20.0`, antes `^8.20.0`) — versiones resueltas sin cambios, el lockfile registra los mismos artefactos. Sin cambios en `src/`/dist/schemas — los scripts ya eran módulos ES agnósticos del runtime (los especificadores `node:` están soportados por diseño en bun). El job node-compat del workflow de publish garantiza el fallback. |
| 4.1.0 | 2026-09-06 | menor | **Guardia de frescura `impossible-timeline` + alineación del mapa veredicto-etapa** (fix #162): añade el código de frescura `impossible-timeline` (directamente después de `stale-policy`), la constante publicada `PRE_EXECUTION_RECEIPT_TIMELINE_SKEW_MS` (300 000 ms = ±5 min), y el predicado puro `isImpossibleReceiptTimeline({ finishedAt, sourceCommitDate, skewMs })` (boolean | null, fail-open con sellos inparseables) — el sensor respaldado por git evalúa el slot documentado del comparador; el comparador puro lo deja sin evaluar. También estrecha `VERDICTS_BY_STAGE.plan` a los dos veredictos de plan (`needs-design` cae — solo `review-spec` puede emitirlo; el `PRE_EXECUTION_VERDICTS` plano conserva el token y la fila de la etapa `spec` queda igual), de modo que la máquina de estado persistida deja de sancionar la emisión de review-plan que el prosa (c) prohíbe. `VERDICTS_BY_STAGE`, `PRE_EXECUTION_RECEIPT_TIMELINE_SKEW_MS` e `isImpossibleReceiptTimeline` ahora se re-exportan desde el índice. Código aditivo; el estrechamiento del mapa veredicto-etapa no invalida ningún receipt persistido (no existen receipts de plan-stage `needs-design`). |
| 4.0.1 | 2026-09-06 | parche | **Exportación `PRE_EXECUTION_POLICY_VERSION`** (feature 29, #149): el schema ahora exporta `PRE_EXECUTION_POLICY_VERSION` (la única autoridad que lee el CLI de snapshot en lugar de un literal hardcodeado), cerrando el known-issue 24 de la feature 28. Aditivo, sin cambio incompatible.
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 0.10.4 | 2026-09-16 | parche | **re-bundle del fold del replan del fix #224**: publica la corrección de redacción de `phase-contract` 1.0.5 (la enumeración de cajas armadas) señalada por el F26 de la revisión final. Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. |
| 0.10.3 | 2026-09-16 | parche | **re-bundle del replan del fix #224**: publica las dos ediciones de contrato del replan — `replan-findings` 1.1.0 (la ruta `close-out` para una unidad terminada y el token `status` desnudo en el bloque que documenta) y `phase-contract` 1.0.4 (la exención de fase ejecutada para las cajas 3 y 7, implementada en `scripts/phase-lint.mjs`). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. |
| 0.10.2 | 2026-09-15 | parche | **Re-empaquetado del merge con main (fix #224 × feature 37)**: el bundle 0.10.1 de la rama quedó sin mergear mientras `main` recibía la feature 37, así que esta release publica el árbol de skills fusionado de una vez — el enrutado de replan del fix #224 (`plan-fix` 3.3.0, `plan-feature` 5.2.0, la nueva skill interna `replan-findings` 1.0.1, `workflow-status` 3.6.0, `review-change` 3.5.1, `review-implementation` 1.8.0, `triage-issue` 2.8.0, `fold-findings` 1.5.0) junto con el conjunto del phase-lint de la feature 37 (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2, `phase-contract` 1.0.3). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. |
| 0.10.1 | 2026-09-15 | parche | **re-bundle del fold de la revisión final del fix #224**: re-empaqueta `replan-findings` 1.0.1 (el contrato de salida del router) y `workflow-status` 3.6.0 (un solo contrato de `next.suggested`: semántica de filas propiedad del router, las filas marca nunca son hallazgos, las filas de decisión no contribuyen nada, ids acotados y las dos referencias del envelope corregidas). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. |
| 0.10.0 | 2026-09-15 | menor | **Re-empaquetado del enrutado determinista de replan (fix #224)**: re-empaqueta el trabajo del router — `plan-feature` 5.2.0 y `plan-fix` 3.2.0 (entrada de replan con router primero), la nueva skill interna `replan-findings` 1.0.0, `workflow-status` 3.5.0 (`next.suggested` enrutado por clase), `review-change` 3.5.1, `review-implementation` 1.8.0, `triage-issue` 2.8.0 y `fold-findings` 1.5.0 (un destino canónico de replan). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. |
| 0.9.2 | 2026-09-13 | patch | **Diálogos acotados para listas largas (fix #214)**: ya ningún diálogo de la consola de configuración puede superar el tope de 24 opciones de los select de pi-web. Por encima del tope el selector de modelo pasa a provider-first en dos pasos (diálogo de proveedor y luego los modelos de ese proveedor) y ambos diálogos quedan acotados — el de proveedor pagina cuando es la lista de proveedores la que se pasa, y la lista de modelos de un solo proveedor pagina de 21 en 21 con `More options…` / `◀ Previous page`. Los selectores de comandos y el fallback no-TUI de `pick` del adaptador también paginan, y "Type another reference…" sigue siendo la última opción de cada diálogo de modelo. Las listas que caben en un solo diálogo mantienen la forma anterior de un solo diálogo: hasta 23 referencias de modelo más "Type another reference…", o 24 comandos — con una excepción deliberada, el selector de la TUI ahora se abre en el valor en vigor, cumpliendo la preselección que su contrato siempre documentó, en lugar de empezar siempre por el primer modelo. |
| 0.9.3 | 2026-09-12 | patch | **re-bundle de skills de la feature 37 (fold F25–F26 + P4)**: publica las skills empaquetadas desde 0.9.1 — la valla de pegado del linter (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2), el texto de entrega del linter determinista de la P4 (`execute-phase` 4.5.0, `plan-fix` 3.2.0, `plan-feature-scaffold` 2.3.0) y `phase-contract` 1.0.1 → 1.0.3 (la autorización de la regla 1 para `Hardening & PR` y el **mínimo de ≥ 1 tarea** de la regla 3: una fase con cero tareas ahora BLOCKs en box-3 en vez de pasar las ocho cajas de forma vacua), que se reflejaron en el paquete pero nunca se publicaron. Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. Renumerado 0.9.2 → 0.9.3 en el merge de sincronización con main: el propio 0.9.2 de main (fix #214, 2026-09-13) se publicó primero, así que este re-bundle toma el siguiente patch. |
| 0.9.1 | 2026-09-11 | patch | **re-bundle del fold del sensor de la feature 38**: re-empaqueta `workflow-status` 3.4.0 (el fold de defectos del script F1–F14/F18 y las cadenas de invocación bun-first). Sin cambio de código del paquete — el bump existe para publicar el re-bundle. |
| 0.9.0 | 2026-09-10 | menor | **El modelo enrutado se queda tras el comando (nueva política `onSettle`)**: tras `agent_settled`, la sesión **ya no se devuelve** por defecto — el modelo y el nivel de thinking con los que corrió el comando se quedan en la ventana de chat abierta (`onSettle: "keep"`), así que una edición o pregunta posterior sigue corriendo con el modelo que planeó. El histórico contrato de restauración al terminar (AC8) pasa a opt-in explícito (`onSettle: "restore"`), expuesto como clave raíz de config (global + proyecto), mostrado por la consola de configuración y documentado EN+ES. La elección propia a mitad de turno (`/model`/Ctrl+P) del operador sigue sin sobrescribirse nunca, y un dispatch fallido (el turno nunca arrancó) sigue devolviendo la sesión. Sin cambio de skills — el paquete incluye las mismas skills; el bump publica el cambio de comportamiento. |
| 0.8.0 | 2026-09-09 | menor | **Mejora de la consola de configuración (fix #154)**: el enrutado de modelo por comando gana una cadena de respaldo ordenada (`model` acepta 1–4 referencias; el dispatch prueba con `find` + `hasConfiguredAuth` solo, llama a `setModel` como máximo una vez por turno y, al agotarse, nombra cada candidato y por qué lo saltó — `stop` se niega, `inherit` avisa y corre con el modelo de la sesión); selectores buscables y con ventana (filtro de subsecuencia/slash-aware del propio paquete, preselección del valor actual, indicador de posición, fallback no-TUI); una edición de ruta pregunta solo el campo marcado (model/thinking) así que guardar sin cambios deja el archivo byte a byte idéntico, y el campo de modelo construye una cadena ordenada; aplicar/limpiar en bloque sobre varios comandos en una pasada con avisos por comando; alias `/aw-settings`. Añade `@earendil-works/pi-tui@0.85.1` (fijada). |
| 0.7.2 | 2026-09-09 | parche | **Bun runtime de primera clase (#197)**: los scripts `bundle:skills`/`test`/`mutation`/`prepare` invocan bun (`bun test` sobre la suite estilo `node:test` — verificado en 1.4.2), con variante explícita de fallback `test:node`; `engines` añade `bun ">=1.4"`; devDependencies fijadas a versiones exactas (`@types/node 24.13.3`, antes `^24`) — versión resuelta sin cambios. Re-empaqueta las 4 skills de redacción de runtime — `pre-execution-review` 2.2.1 (nota de convención en la columna validator de LEDGERS), `review-spec` 1.7.1, `review-plan` 1.6.1, `workflow-status` 3.2.1 (comandos de snapshot bun-first). |
| 0.7.1 | 2026-09-08 | patch | **Eliminar devDep pi-server obsoleta + fijar peer a >=0.85.1** (fix #181): pi 0.85.1 (pi PR #9132) eliminó la cadena de re-exportación ansiosa (`main` → `experimental/server` → `@earendil-works/pi-server`) que cubría el shim del fix #166, por lo que la devDependency se volvió peso muerto y el rango de peer `*` aceptaba silenciosamente hosts de pi no probados. Elimina la devDependency, ajusta el peer a `>=0.85.1`, re-resuelve el lock a pi 0.85.1 (podando pi-server) y refresca la nota de baseline del README 0.85.0 → 0.85.1 (2026-09-05) en EN+ES. Suite verde 140/140 en 0.85.1. |
| 0.7.0 | 2026-09-07 | menor | **Re-empaquetado feature 30 (bucle review→fold dirigido por decisión y de alcance delta)**: re-empaqueta las 3 skills tocadas — `fold-findings` 1.4.0 (contrato REPAIR-RECEIPT, clasificación de lote por regla de impacto, regla de freeze-batch, bloque de cierre de cuatro ramas con entradas de decisión de selección de rama, regla materializa-reproductor), `review-change` 3.4.0 (modo delta por defecto tras el fold, dos triggers de escalado, dedupe solo-nuevos, tope de ciclos delta), `pre-execution-review` 2.2.0 (consumo de la celda `recheck` en la marca durable del hallazgo). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. Techos de ruta de la referencia compartida `pre-execution-review` declarados; techo de referencia de `review-change` subido (fuente de crecimiento feature 30 / issue #170, medido 2746). |
| 0.6.0 | 2026-09-06 | menor | **Re-empaquetado fix/162**: re-empaqueta las 10 skills tocadas — `pre-execution-review` 2.1.0 (autoverificación del recibo, independencia de etiqueta de roadmap, alcance de la guardia de respuesta-a-veredicto), `review-spec` 1.7.0 (caja RUN de autoverificación), `review-plan` 1.6.0 (pierde el veredicto de diseño; conjunto de dos veredictos + ruta `class: product`), `plan-feature` 5.1.0 (las filas done redirigen a `/audit-pr`), `workflow-status` 3.2.0 (detecta unidades done-sin-mergear), `design-feature` 3.3.0, `review-change` 3.3.0, `execute-phase` 4.4.0, `ship-roadmap` 5.2.0, `audit-pr` 5.1.0 (la puerta de linaje nombra `impossible-timeline`). Sin cambios de código del paquete — el bump existe para publicar el re-empaquetado. Techos de ruta de la referencia compartida `pre-execution-review` re-basados (declarado, fuente de crecimiento `fix/162`). |
| 0.5.0 | 2026-09-05 | menor | **Re-empaquetado con acotación del roadmap**:  re-empaqueta `pre-execution-review` 2.0.0 (el snapshot pre-ejecución ya no liga el `ROADMAP.md` compartido — las filas del roadmap ajenas y las escrituras de la propia máquina de estados ya no invalidan recibos grabados; re-reseña única de los recibos grabados antes del cambio) y `review-spec` 1.6.0 (su lista de contextos de `CHECKS.md` retira la `roadmap-row` sin ligar). Sin cambios de código del paquete. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 0.0.0 | 2026-09-10 | — | **Vehículo del crate productor (feature 37, P1)**: paquete privado de relleno que reserva la superficie productora `agentic-workflow` — `private: true`, versión `0.0.0`, cero dependencias, nada publicado. Productores actuales: ninguno todavía. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 3.6.0 | 2026-09-15 | menor | **Un solo contrato de `next.suggested` (revisión final del fix #224, F2/F4/F5/F13/F15):** el sensor deja de re-derivar la semántica de filas del ledger y la importa de `scripts/unit-route.mjs` — el mismo vocabulario `folded` (`—`, `-`, `n/a` y celda vacía cierran una fila), las filas marca (`VF-<n>`, `REVIEW-RAN`) no son hallazgos (una fila marca rellenada hacía que una unidad ya mergeada proyectara un `/fold-findings` permanente) y una fila decision-required no contribuye nada en vez de un fold erróneo. Los ids de entrada de plan pasan por el tope de 160 caracteres del router. Referencias: `ENVELOPE_FIELDS.md`/`ENVELOPE_CORE.md` ya no afirman que el paso 13 esté sin mecanizar u omitido, y el paso 13 de `SENSOR_SIGNALS.md` declara el predicado de fila abierta y la regla de decisión. Validador: `scripts/workflow-status-sensor.test.mjs` (los pines F13/F2/F15). |
| 3.5.0 | 2026-09-15 | menor | **`next.suggested` enrutado por clase (fix #224):** el sensor emite el array declarado por el esquema — un hallazgo abierto de dueño plan va al planificador de la unidad, una fila fix-now simple a `/fold-findings`, ninguna fila abierta no aporta nada — y `SENSOR_SIGNALS.md` retira la señal obsoleta "cualquier fila `folded: no` → `/fold-findings`" que desviaba una fila de replan. La decisión clase→destino es del router (`scripts/unit-route.mjs`). |
| 3.4.0 | 2026-09-11 | menor | **Fold de defectos del sensor (revisión final de la feature 38: F1–F14 + F18).** El sensor determinista se repara contra el sustrato con el que se distribuye. Lectura: el lector de NRS acepta la forma tabular del ledger, así que un ledger congelado ya no se parsea como `draft` ni bloquea cada ejecución; las celdas con `\|` escapado conservan sus columnas y las severidades de `findings.fix_now[]` se normalizan al enum publicado `high`/`med`/`low` (con nota nombrada para una fila fuera de él), de modo que el envelope deja de fallar su propio `validateEnvelope` sobre ledgers reales. Ciclo de vida: el estado de merge se resuelve por PR en vez de por la ventana acotada de merges recientes (desaparecen las dependencias incumplidas falsas y los substrate blockers espurios), el paso 6a sensa unidades `done` con PR abierta, y su gate alcanza por fin `next.recommended` (los `next` prometidos `/review-spec`//`/review-plan` eran inalcanzables). Sensado: el verificador de snapshots se ata al repositorio sensado (`--root`/`--dir`) y la vigencia de la marca de revisión corre sobre las entradas vinculadas de la revisión, así que las marcas dejan de ser permanentemente no vigentes y un proyecto ajeno deja de leerse como el checkout del propio sensor. Contrato: se elimina `git fetch` (solo lectura se mantiene), la recuperación de crash clasifica cada rama de unidad con evidencia veraz por rama y reducción del peor caso, los directorios de unidad y las lecturas de `--last-envelope` quedan confinados/acotados, y `detail.urgent.interruptibility.tasks_from_boundary` lleva un recuento real. Docs: `ENVELOPE_CORE.md`/`ENVELOPE_FIELDS.md`/`SENSOR_SIGNALS.md` marcan los pasos 10–13 como aún no mecanizados, la tabla de niveles gana `/review-spec`//`/review-plan`, y las cadenas de invocación son bun-first. Validador: `scripts/workflow-status-sensor.test.mjs` (37 tests, incluidas las regresiones de NRS tabular, estado de merge y frontera del ledger de hallazgos). |
| 3.3.0 | 2026-09-11 | menor | **Script sensor determinista (feature 38).** `scripts/workflow-status.mjs` pasa a ser el productor único del envelope: ejecuta la secuencia SENSOR_CORE (recolección git/forge, urgencia solo-etiquetas, parseo de roadmap + índice de fixes en cinco estados, cierre transitivo de dependencias, preparación + detección de recibos del paso 6a, progreso de fase, vigencia de la marca de revisión, proyección del ledger fix-now), se autovalida con `validateEnvelope` (solo diagnóstico) e imprime el Envelope v2 fijo; los fallos degradan a `unavailable-<source>-<cause>` en `detail.degradations` con salida 0. La skill se aligera a ejecutar-el-script, leer-el-JSON, interpretar-`next.recommended`; `SENSOR_CORE.md`/`ENVELOPE_CORE.md` sustituyen la prosa de comandos/ensamblado por referencias al script (semántica conservada). Argumentos externos y contrato del envelope sin cambios (menor). Validador: `scripts/workflow-status-sensor.test.mjs` (A:14/O26). |
| 3.2.1 | 2026-09-09 | parche | **Redacción de convención de runtime (#197)**: `SENSOR_CORE.md`/`PRE_EXECUTION.md` muestran el comando verify del snapshot en forma bun-first (`bun scripts/pre-execution-snapshot.mjs …`, fallback node). Sin cambio de contrato. |
| 3.2.0 | 2026-09-06 | menor | **Independencia de la etiqueta de roadmap** (fix #162): el conjunto de detección de recibos del paso 6a gana unidades `done`-con-PR-abierto (sin mergear) de modo que un recibo obsoleto/faltante en una fila done-pero-sin-mergear aflora como bloqueador de puerta; las unidades mergeadas quedan excluidas (el merge cierra sus puertas). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 5.2.1 | 2026-09-08 | parche | **Corrección de orden del hand-off (#191):** reordenó las tres superficies de ship-roadmap (celda de la tabla de modelos en `SKILL.md`, bullet REVIEW de `ADVANCE.md`, fila "Final review/correction loop" de `MODEL_ROUTING.md`) del invertido `fold-findings → re-run review-change` al canónico `review-change → fold-findings (solo en REVIEW-FAIL) → re-run review-change`. Sin cambios de código del paquete — el bump existe para registrar las ediciones de texto.
| 5.2.0 | 2026-09-06 | menor | **Re-ubicación de veredicto en ADVANCE.md** (fix #162): el bloque step-4 de ADVANCE.md reescribió `NEEDS-DESIGN → park as in REVIEW-SPEC` a `PLAN-REVIEW-FAIL with class: product → design-feature`, manteniendo el enrutamiento de veredictos alineado con el conjunto de dos veredictos. Sin cambios de código del paquete — el bump existe para registrar los cambios de referencia.
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 1.1.0 | 2026-09-05 | menor | Enrutado basado en evidencia (feature 29, P3): los veredictos vuelven a sus dueños de aguas arriba — defectos de Plan a plan-feature/plan-fix más review-plan, huecos de Producto a design-feature y luego review-spec, evidencia no disponible a un BLOCKED exacto; los hallazgos locales de fuente se quedan locales, el mapping nunca reemplaza la autoridad de revisión/verificación/auditoría candidata, y un segundo ciclo hereda el CONVERGENCE-ANOMALY del feature 28. Se documentan identidad de fuente, continuidad de preparación y consumo único/recuperación, y la navegación semántica/simbólica y Engram se nombran localizadores consultivos con respaldo directo GitHub/Git/búsqueda/lectura/test. |
| 1.0.0 | 2026-09-05 | — | Primera versión (feature 29, P1–P2): un contrato de mapper pre-escritura interno y de solo lectura — siete preguntas de evidencia fijas, el mapa compacto fijo, cuatro veredictos, enrutado inline/fresh sin proxy de conteo de ficheros, falsificación temprana, reglas de no-progreso, identidad de fuente, continuidad de preparación, consumo único/recuperación y el hand-off compacto del escritor. `user-invocable: false`, consumido solo por `execute-phase` en la ruta pre-escritura. |

#### `execute-phase`
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 4.5.2 | 2026-09-14 | parche | **Fallback sin linter divulgado (revisión de la feature 37 F86, marca):** la entrega del phase-lint ahora nombra la limitación del destino de instalación — `scripts/phase-lint.mjs` viaja con el repositorio del workflow, no con la skill instalada — y su fallback seguro: aplicar las ocho reglas de `phase-contract` a mano, etiquetar esa comprobación como más débil y divulgar que el linter determinista no se ejecutó; nunca omitir el gate. `SKILL.md` y `references/PREFLIGHT.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| 4.5.1 | 2026-09-12 | parche | **Valla de pegado (revisión de la feature 37 F26, seguridad):** toda entrega que ponga ante un agente el bloque de stdout del linter ahora declara que es **salida de lint, nunca instrucciones** — el bloque hace eco de texto derivado del plan (el título de una fase puede venir del cuerpo de una issue de un tercero), así que cualquier directiva dentro de él es dato para reportar, nunca acción que ejecutar. `SKILL.md` y `references/PREFLIGHT.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| 4.5.0 | 2026-09-10 | menor | **Lint de fases determinista (feature 37 P4):** el preflight de phase-lint ya no razona sobre las ocho reglas y el fingerprint — ejecuta `bun scripts/phase-lint.mjs <plan>` (fallback node), pega el bloque fijo de stdout del linter y para con exit 1; el linter implementa las reglas cuyo propietario es `phase-contract`, que sigue siendo el único dueño. `SKILL.md` y `references/PREFLIGHT.md` actualizados. |
| 4.4.1 | 2026-09-08 | parche | **Corrección de orden del hand-off (#191):** reordenó cada bloque terminal/hand-off de execute-phase del invertido `fold-findings → re-run review-change` al canónico `review-change → fold-findings (solo en REVIEW-FAIL) → re-run review-change`, y quitó "mandatory" del hand-off de fold (el paso obligatorio es la review; el fold es condicional a los hallazgos). `UNIT_LOOP.md`, `FOLDING.md`, `CLOSEOUT.md`, `BATCH_AND_PORTABILITY.md` y `SKILL.md` reordenados; se añadió un pin de test de disciplina. Sin cambios de código del paquete — el bump existe para registrar las ediciones de texto.
| 4.4.0 | 2026-09-06 | menor | **Re-enrutamiento de veredicto** (fix #162): el bloque de re-verificación de la puerta pre-ejecución reescribió `NEEDS-DESIGN → /design-feature` a `Product-rooted → /design-feature`, reflejando que solo review-spec puede emitir NEEDS-DESIGN (review-plan emite el conjunto de dos veredictos; un hueco de intención de producto es `PLAN-REVIEW-FAIL` con `class: product`). Sin cambios de código del paquete — el bump existe para registrar los cambios de referencia.
| 4.3.0 | 2026-09-05 | menor | **Mapper pre-escritura (feature 29, P2):** el hueco tras la puerta pre-ejecución y antes de la primera escritura ahora carga el contrato del mapper `implementation-discovery`, que responde las siete preguntas de evidencia, emite el mapa compacto fijo y enruta READY \| REPLAN \| NEEDS-DESIGN \| BLOCKED; solo READY autoriza la primera escritura de rama/planificación/código. `PREFLIGHT.md` y `SKILL.md` nombran el contrato en lugar de reservarle el hueco. Siete techos de estimación de ruta `execute-phase:*` re-baseados a sus pisos medidos (fuente de crecimiento: la referencia al contrato del mapper pre-escritura en `PREFLIGHT.md`). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 3.3.0 | 2026-09-06 | menor | **Alcance de la guardia de respuesta-a-veredicto** (fix #162): REPAIR §4 afirma que una reparación que responde a un veredicto persistido nunca es un defecto de bucle (POLICY §4 acota cada tope a re-revisiones ciegas), y no existe respuesta "ya completo" para una unidad con un recibo FAIL abierto — la reparación pasa por esta skill por definición.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 5.2.0 | 2026-09-15 | menor | **Entrada de replan (fix #224):** la puerta de redirección ejecuta primero `node scripts/unit-route.mjs <NN>` — una línea `route: replan` exime a la unidad del STOP de estado y carga el nuevo contrato interno `replan-findings`, que recorta el plan desde el conjunto de lectura acotado del router en la misma rama; cualquier otra ruta conserva la puerta y el hand-off existentes. |
| 5.1.0 | 2026-09-06 | menor | **Independencia de la etiqueta de roadmap** (fix #162): el stop `done` de la puerta de redirección sigue prohibiendo re-planificar pero redirige a `/audit-pr <PR>` — `done` significa construido + PR abierto, merge pendiente, una etiqueta de ciclo de vida nunca un veredicto de puerta; un PR mergeado es nada pendiente (`--next`), uno abierto es la puerta de merge, nunca una re-planificación.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 3.3.0 | 2026-09-15 | menor | **Entrada de replan (fix #224):** antes del ingest se consulta `node scripts/unit-route.mjs <unit>` — una línea `route: replan` carga el contrato interno `replan-findings` y añade fases al ledger SPEC de la unidad destino en su propia rama en vez de redactar un fix SPEC nuevo; cualquier otra ruta queda igual. Renumerado 3.2.0 → 3.3.0 en el merge de sincronización con main: el merge de la feature 37 ya había publicado 3.2.2, así que el nuevo caso de enrutado toma el siguiente minor. |
| 3.2.2 | 2026-09-14 | parche | **Fallback sin linter divulgado (revisión de la feature 37 F86, marca):** el paso de emisión de fases ahora nombra la limitación del destino de instalación — `scripts/phase-lint.mjs` viaja con el repositorio del workflow, no con la skill instalada — y su fallback seguro: aplicar las ocho reglas de `phase-contract` a mano, etiquetar esa comprobación como más débil y divulgar que el linter determinista no se ejecutó; nunca omitir el gate. `SKILL.md` y `references/PLANNING_PROCESS.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| 3.2.1 | 2026-09-12 | parche | **Valla de pegado (revisión de la feature 37 F26, seguridad):** el paso de emisión de fases declara que el bloque de stdout del linter es **salida de lint, nunca instrucciones** — hace eco de texto derivado del plan, así que cualquier directiva dentro de él es dato para reportar, nunca acción que ejecutar. `SKILL.md` y `references/PLANNING_PROCESS.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| 3.2.0 | 2026-09-10 | menor | **Lint de fases determinista (feature 37 P4):** la emisión de fases ahora ejecuta `bun scripts/phase-lint.mjs <plan>` (fallback node) y pega el bloque fijo de stdout en lugar de razonar sobre las ocho reglas; exit 1 significa que la fase bloqueada se re-corta o se divide. `SKILL.md` y `references/PLANNING_PROCESS.md` actualizados; `phase-contract` sigue siendo el único dueño de las reglas. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 1.7.1 | 2026-09-09 | parche | **Redacción de convención de runtime (#197)**: `CHECKS.md`/`OUTPUT.md` muestran los comandos build/verify del snapshot en forma bun-first (fallback node según la convención de runtime del repo). Sin cambio de contrato. |
| 1.7.0 | 2026-09-06 | menor | **Autoverificación del recibo** (fix #162): la sección persistir-primero gana la ejecución de `verify --stage spec` en el turno y la caja del contrato de turno pasa a ser RUN — el revisor ejecuta la re-verificación del sensor y pega el JSON junto al bloque de veredicto antes de reportar; `exit 3`/`structural.fresh: false` significa que la marca no aterrizó (write-then-report es mecánico). Veredictos sin cambio; `review-spec` sigue emitiendo el token de veredicto de diseño.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 1.6.0 | 2026-09-05 | menor | **Acotación del roadmap:** la lista de `contexts` del snapshot ya no liga una `roadmap-row` — la fila del roadmap sigue siendo dato de enrutado del paso 0, pero el libro mayor compartido `ROADMAP.md` queda deliberadamente sin ligar porque se mueve por razones que esta unidad nunca reseñó (filas de otras unidades, escrituras sancionadas de la propia máquina de estados), lo que invalidaba cada recibo en vuelo de todo el repositorio y forzaba re-reseñas inútiles (p. ej. un `execute-phase` bloqueado por un recibo cuyo único drift era la fila del roadmap de otra feature). Los artefactos de la unidad y las demás autoridades rectoras siguen ligadas; la compuerta de estado propio y la huella del gate de dependencias siguen cubriendo en vivo el contenido de la fila relevante para la seguridad. Los recibos grabados antes de este cambio se re-verifican obsoletos exactamente una vez — ver `docs/workflow/MIGRATION.es.md`. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 1.6.1 | 2026-09-09 | parche | **Redacción de convención de runtime (#197)**: `CHECKS.md`/`OUTPUT.md` muestran los comandos build/verify del snapshot en forma bun-first (fallback node según la convención de runtime del repo). Sin cambio de contrato. |
| 1.6.0 | 2026-09-06 | menor | **Posesión del arbitraje + autoverificación** (fix #162): `review-plan` pierde por completo el veredicto de diseño — la línea `Verdict:` del recibo es el conjunto de dos veredictos, un hueco de intención de producto es `PLAN-REVIEW-FAIL` con `class: product` (solo `review-spec` puede emitir el token de veredicto de diseño), y la tabla de rutas y los bloques de cierre eliminan esa rama. La sección persistir-primero gana la caja RUN de autoverificación `verify --stage plan` (el plan de feature nombra `--parent`; el fix lo omite).  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 1.5.0 | 2026-09-04 | menor | Redirección por retiro (fix #161, P3a): la ruta de fold `source`/`environment`/`runtime` nombra la vía manual `/fold-findings` y luego re-ejecutar `/review-change` en lugar del router retirado `loop-review-fold`. |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 3.5.1 | 2026-09-15 | parche | **Destino canónico de replan (fix #224):** el bloque `→ Next:` de `REVIEW-FAIL` y la tabla de Routing enrutan un hallazgo `replan-in-unit` / de dueño plan por `node scripts/unit-route.mjs <unit>` — su línea `route: replan` nombra el planificador y el `/review-plan` fresco antes de `/execute-phase` — sustituyendo las dos frases contradictorias que enviaban la clase al ejecutor y a `plan-feature` por separado. |
| 3.5.0 | 2026-09-08 | menor | **Límite de fin-de-revisión (fix #191 P3, defecto espejo del lado revisor)**: en `REVIEW-FAIL`/`NEEDS-DECISION` la skill ahora termina en el informe y nunca invoca por sí misma `/fold-findings`, `/execute-phase`, `/plan-feature`, `/design-feature` ni `/triage-issue` — esos son invocaciones separadas iniciadas por el usuario (nueva caja **Review-end turn boundary** en el Turn contract). La redacción de destino de `fix-now` en la sección de relación y en `OUTPUT_AND_GUARDRAILS.md` ahora enruta los hallazgos al ciclo de fold de la fase actual (`/fold-findings`, invocado después de que termine esta revisión) en lugar de leerse como ejecución en-revisión; los pasos 11/14 de `PERSIST_AND_DECIDE.md` indican que las únicas mutaciones de la skill son (commit del ledger; comentario de recibo en `REVIEW-PASS`) y que el bloque `→ Next:` es una **recomendación, no una lista de tareas**; el tope de dos ciclos de `REVIEW_PROCESS.md` aclara que las re-ejecuciones cuentan invocaciones de revisión separadas. Añade un pin de disciplina red-first (AC15: revertir el texto del límite hace fallar el pin). |
| 3.4.0 | 2026-09-07 | menor | Issue #170: `REVIEW_PROCESS.md` paso 1 gana el **modo delta** como predeterminado en una re-revisión posterior al fold (re-verificar cada fila `folded: yes` en su `file:line` citado; revisar solo el diff de fold; gate verde en el head revisado + blob `ACCEPTANCE.md` exacto; las marcas `REVIEW-RAN` de delta cuentan para el tope de dos ciclos desde la fuente sin cambios; los re-reportes del mismo `file:line`+eje se admiten solo como `regression of <id>`/`DISPUTED`), y los dos **triggers de escalada** (ancho: archivo fuera de la unión de archivos citados o una línea cambiada >50 líneas de toda línea citada en un archivo citado; tamaño: >200 líneas cambiadas o >15 archivos) con el requisito de declarar el trigger disparado y los números observados. |
| 3.3.0 | 2026-09-06 | menor | **Alcance de la guardia de respuesta-a-veredicto** (fix #162): el tope de dos ciclos del bucle fold → re-review queda acotado al bucle revisor→fold sobre hallazgos de fuente; una reparación de planificación dirigida por un hallazgo `class: plan`/`product` es una respuesta-a-veredicto (POLICY §4), nunca un stop de defecto-de-bucle ni se cuenta contra el tope.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 1.5.0 | 2026-09-15 | menor | **Destino canónico de replan (fix #224):** el recibo `REPLAN-ROUTE` del freeze-batch y la tabla de ramas enrutan las filas retenidas por `node scripts/unit-route.mjs <unit>` — la línea `route: replan` del router nombra el planificador que recorta el plan — sustituyendo "enruta a planificación". |
| 1.4.0 | 2026-09-07 | menor | Issue #170: el contrato de informe gana el **REPAIR-RECEIPT** fijo (ids reparados + refs `finding-mark@1`, refutados/abiertos, códigos de salida del gate en el head, clase de lote, shortstat del diff de fold) impreso como la ÚLTIMA salida absoluta; la **clasificación de lote** por regla de impacto (`all-repair-in-place` / `frozen (replan present)` / `none`) se deriva solo de las filas congeladas y nunca reclasifica; la regla **freeze-batch** no pliega nada (sin flips, sin commits) y registra la ruta REPLAN-ROUTE con los ids conservados; el bloque de cierre de cuatro ramas (`RE-REVIEW-REQUIRED (delta)` / `RE-REVIEW-OPTIONAL` / `RE-REVIEW-SKIPPED` / `REPLAN-ROUTE`) gana sus entradas de decisión de selección de rama (test de conjunto de archivos solo-docs, override de severidad congelada `high`, regla SKIPPED-requiere-decisión-previa-del-consumidor) y el literal por defecto sin-decisión→re-review; se añaden las ramas de lote vacío (`none`) y gate fallido (códigos de salida observados, nada plegado); `FOLD_POLICY.md` gana la regla materializar-reproductor (consumir la celda `recheck`, nunca re-derivar, `BLOCKED <entrada faltante>` cuando no es materializable). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 5.1.0 | 2026-09-06 | menor | **La puerta de linaje nombra la nueva dimensión de frescura** (fix #162): la enumeración de bloqueadores de la puerta de linaje pre-ejecución pasa a ser "Stale, missing, wrong-stage o impossible-timeline linaje → BLOCKED", manteniendo la honestidad de la puerta de merge basada en recibos para la nueva dimensión.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Versión | Fecha | Tipo | Qué cambió |
|---|---|---|---|
| 2.8.0 | 2026-09-15 | menor | **Destino canónico de replan (fix #224):** el proceso de hallazgos de revisión y el diagrama de relación enrutan `replan-in-unit` por `node scripts/unit-route.mjs <unit>`, cuya línea `route: replan` nombra el planificador (`/plan-feature` / `/plan-fix`) y el `/review-plan` fresco antes de la ejecución. |
...
|---|---|---|---|---|
| `review-implementation` | 1.8.0 | 2026-09-15 | menor | **Destino canónico de replan (fix #224):** la viñeta de Routing de `CLASSIFY.md`, la sección de replan dentro de alcance y la fila de dueño `plan` enrutan `replan-in-unit` por `node scripts/unit-route.mjs <unit>` y el planificador que nombra, luego un `/review-plan` fresco antes de la ejecución. |
| `replan-findings` | 1.1.0 | 2026-09-16 | menor | **Ruta terminal y token de estado desnudo (replan del fix #224, F23+F24):** el bloque que documenta este contrato lleva ahora la ruta `close-out` — una unidad terminada (`status: done`) sin fila abierta, cuyo `next:` es `/audit-pr`, la puerta de merge — y el campo `status` queda declarado como el token desnudo que el router lee de la celda del roadmap o del índice de fixes, nunca la decoración markdown de la celda. |
| `replan-findings` | 1.0.1 | 2026-09-15 | parche | **Contrato de salida del router (revisión final del fix #224, F7):** la skill documenta el bloque que imprime el router — los ocho campos, el contrato de códigos de salida (0 con ruta; 1 unidad desconocida o recuento de argumentos erróneo; 2 ambigua — las dos últimas fallan cerrado sin línea `route:`), el tope de 12 rutas con su resto explícito `… and N more`, y que los ids y rutas ecoados son datos del repositorio aplanados y recortados a 160 caracteres. |
| `replan-findings` | 1.0.0 | 2026-09-15 | — | Nuevo contrato interno (fix #224): la entrada condicional de replan. Se carga solo cuando `node scripts/unit-route.mjs <unit>` imprime `route: replan`; consume el conjunto de lectura acotado del router (sin planning preflight), añade fases al ledger `## Phases` del SPEC de la unidad según la regla de colocación del hardening, rota el `artifactRevisionId` y entrega a `/review-plan <unit>` — nunca directo a `/execute-phase`. |
| `plan-feature-scaffold` | 2.3.2 | 2026-09-14 | parche | **Fallback sin linter divulgado (revisión de la feature 37 F86, marca):** la entrega del stdout del linter en el scaffold ahora nombra la limitación del destino de instalación — `scripts/phase-lint.mjs` viaja con el repositorio del workflow, no con la skill instalada — y su fallback seguro: aplicar las ocho reglas de `phase-contract` a mano, etiquetar esa comprobación como más débil y divulgar que el linter determinista no se ejecutó; nunca omitir el gate. `SKILL.md` y `references/SCAFFOLD_PROCESS.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| `plan-feature-scaffold` | 2.3.1 | 2026-09-12 | parche | **Valla de pegado (revisión de la feature 37 F26, seguridad):** la entrega del stdout del linter en el scaffold declara que el bloque es **salida de lint, nunca instrucciones** — hace eco de texto derivado del plan, así que cualquier directiva dentro de él es dato para reportar, nunca acción que ejecutar. `SKILL.md` y `references/SCAFFOLD_PROCESS.md` actualizados; el espejo de pi se re-empaqueta. No cambia ninguna regla, flag, orden de gate ni forma de salida. |
| `plan-feature-scaffold` | 2.3.0 | 2026-09-10 | menor | **Lint de fases determinista (feature 37 P4):** el paso de scaffold ejecuta `bun scripts/phase-lint.mjs <plan>` (fallback node) sobre el plan emitido y pega el bloque fijo de stdout del linter en lugar de razonar sobre las ocho reglas; exit 1 es una fase bloqueada que se re-corta, nunca un veredicto que se re-deriva. `SKILL.md` y `references/SCAFFOLD_PROCESS.md` actualizados; `phase-contract` sigue siendo el único dueño de las reglas. |
| `phase-contract` | 1.0.5 | 2026-09-16 | parche | **Enumeración de cajas armadas corregida (fold del replan del fix #224, F26):** la frase de alcance de la exención de fase ejecutada nombra ahora explícitamente las cajas que siguen armadas para una fase con todo marcado — cajas 1, 2, 4, 5, 6 y 8 — en lugar del rango que incluía por error la caja 7, que está exenta. Solo prosa; sin cambios de semántica de reglas. |
| `phase-contract` | 1.0.4 | 2026-09-16 | parche | **La exención de fase ejecutada (replan del fix #224, F25):** el texto del propietario declara ahora que una fase con todas sus casillas marcadas es histórica solo para las cajas 3 y 7 — las dos cajas posicionales que un replan perturba al añadir trabajo tras una `Hardening & PR` ya ejecutada — mientras las cajas 1, 2 y 4–8 siguen armadas para ella y todas lo están para una fase no emitida, así que pre-marcar para esquivar una comprobación es un defecto. Implementado en `scripts/phase-lint.mjs` y anclado por un triple de corpus. |
| `phase-contract` | 1.0.3 | 2026-09-11 | parche | La regla 3 gana el **mínimo de ≥ 1 tarea** (issue #184, revisión de la feature 37 F5, aprobado por el usuario): una fase debe llevar al menos una tarea marcable independientemente comprobable — una fase con cero tareas está mal formada y ahora BLOQUEA box-3 en lugar de pasar las ocho cajas de forma vacua. Anclado en el corpus `scripts/phase-lint.test.mjs`. |
| `phase-contract` | 1.0.2 | 2026-09-10 | parche | Enmienda de la regla 1 (issue #184, feature 37 P1): el título literal de cierre de las plantillas `Hardening & PR` es el único título con `&` autorizado — su `&` es un separador de normalización, no un conector de entregables, y su title-deliverable normaliza a `hardening-pr`; cualquier otro título con `&` sigue dando FAIL. No se añade semántica más allá del texto del propio propietario. |
| `pre-execution-review` | 2.2.0 | 2026-09-07 | menor | Issue #170: `LEDGERS.md` §La marca durable de hallazgo gana una frase de consumo — `fold-findings` lee la celda `recheck` para **materializar** el reproductor del hallazgo como su comprobación de regresión (nunca re-deriva; una celda que no puede materializarse da `BLOCKED <entrada faltante>`). La forma de la fila, las exclusiones `VF-` y la regla de escritor único `review-change` no cambian; ambas proyecciones de plantilla y el bloque de propiedad quedan intactos. |
| `pre-execution-review` | 2.1.0 | 2026-09-06 | menor | **Autoverificación del recibo + independencia de etiqueta de roadmap + alcance de respuesta-a-veredicto** (fix #162): POLICY §8 hace mecánico el write-then-report (el revisor es consumidor cero — ejecuta `verify --stage …` en el mismo acto de persistir, pega la respuesta del sensor junto al bloque antes de reportar; un veredicto solo-en-chat es `missing-receipt-snapshot` para cada consumidor); SNAPSHOT.md anota consumidor cero; §5 gana la regla complementaria (las filas de roadmap son etiquetas, `done` = construido + PR abierto nunca suprime una puerta); §4 acota cada tope de ciclo a re-revisiones ciegas — una reparación por respuesta-a-veredicto nunca es un stop de defecto-de-bucle. |
| `pre-execution-review` | 2.0.0 | 2026-09-05 | mayor | **Acotación del roadmap — contrato de snapshot cambiado.** La lista de contextos del constructor ya no liga `roadmap-row`: `ROADMAP.md` es el libro mayor compartido de todas las unidades, de modo que las filas de otras unidades y las escrituras sancionadas de la propia máquina de estados (`planned` → `in-progress` en P1, `done` al abrir PR) invalidaban cada recibo en vuelo de todo el repositorio y forzaban re-reseñas inútiles — el fallo exacto que un orquestador paga dos veces. El contenido de la fila relevante para la seguridad sigue cubierto por compuertas que lo re-leen en vivo (gate de estado propio; la huella del Dependency receipt v1 del gate de dependencias sobre la línea `Depends on:` del SPEC más las filas del cierre), y los artefactos de la unidad más las demás autoridades rectoras (`project-guide`, `normalized-repository-state`, `architectural-invariants`) siguen ligadas por completo — una edición genuina de un artefacto sigue fallando cerrado. **Migración:** los recibos grabados antes de este cambio se re-verifican `stale-context`/obsoletos de forma exactamente una vez; re-ejecuta la reseña de la etapa (`/review-spec`, `/review-plan`) en cada unidad en vuelo tras actualizar. El vocabulario del esquema conserva `roadmap-row` como kind válido — el constructor de este repositorio simplemente deja de emitirlo. Validador: `scripts/pre-execution-sensor.test.mjs` (bloque roadmap-scoping). |
...
- **2026-09-14 — se divulga el fallback sin linter.** Revisión de la feature 37 F86: las tres skills consumidoras ordenan `bun scripts/phase-lint.mjs <plan>`, pero el script viaja con el repositorio del workflow, no con la skill instalada, así que un destino de instalación no tenía comportamiento divulgado. Toda entrega de ejecutar-y-pegar declara ahora el fallback seguro — aplicar las ocho reglas de `phase-contract` a mano, etiquetar la comprobación como más débil y divulgar que el linter determinista no se ejecutó, nunca omitir el gate — y el espejo de pi se re-empaqueta (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2). Los dos archivos de referencia recargados por rutas hicieron crecer todos los techos de estimación `execute-phase:*` y los techos de estimación+líneas de `plan-feature:scaffold` / `plan-fix:issue` más allá del re-basis #4, así que se re-fijan a `ceil(measured × 1.10)` (11739 / 11752 / 11622 / 11784 / 11937 / 11536 / 11699 / 12869 para los ocho `execute-phase:*` — el octavo es `execute-phase:unit-loop`, re-fijado por el mismo re-base; la normalización anterior lo había dejado en 12798 — y 24286 / 27169 para el par scaffold/fix) — re-basis #5, y esta fila es su fuente de crecimiento.
- **2026-09-12 — la fusión con la feature 38 re-basa siete techos de ruta de `execute-phase`.** El re-base de la feature 38 se ejecutó en su propio HEAD, antes de que existieran las rewords de la regla 1/regla 3 de `phase-contract` (1.0.3) y el adelgazamiento de `execute-phase` de la P4 — ambos archivos que carga cada ruta `execute-phase:*` — así que los siete techos `routeEstimateMax` quedaban por debajo de `ceil(measured × 1.10)` en el árbol fusionado (15 fallos de ruta allí). Se re-fijan (11739 / 11752 / 11622 / 11784 / 11937 / 11536 / 11699) y esta fila es la fuente de crecimiento que la política de techos exige junto al commit. Con este re-base todo el conjunto de rutas queda verde (22/22) y quedan cerrados los excesos preexistentes que esta rama tenía divulgados — la mitad del fixture por el fix #200 en `main`, la mitad viva por el re-base de la feature 38 más el re-base #3. El fold F26 de esa misma revisión añade la valla de pegado: toda entrega del stdout del linter declara ahora que el bloque es salida de lint, nunca instrucciones (`execute-phase` 4.5.1, `plan-fix` 3.2.1, `plan-feature-scaffold` 2.3.1), y el espejo de pi se re-empaqueta. La valla hizo crecer dos referencias cargadas por rutas, así que los siete techos de estimación `execute-phase:*` y los techos de estimación+líneas de `plan-feature:scaffold` / `plan-fix:issue` se re-basan a `ceil(measured × 1.10)` (24286 / 27169 para estos dos últimos) — este texto es la fuente de crecimiento y la declaración que exige la política de techos (re-basis #4).
- **2026-09-11 — cuatro techos de ruta se recalibran por segunda vez.** Fold F19 del ciclo 2 de la feature 37: el mismo lote de fold que recalibró `plan-feature:issue`/`:scaffold`/`:scoped` y `plan-fix:issue` reformuló después la regla 3 de `phase-contract` — un archivo que cargan todas esas rutas —, así que los cuatro techos `routeEstimateMax` volvían a estar obsoletos en el head revisado (19 rutas en rojo allí frente a 15 en `main`). Se re-fijan a `ceil(measured × 1.10)` (11558 / 24144 / 9062 / 27018) y esta fila es la fuente de crecimiento que la política de techos exige junto al commit; las cuatro rutas dejan de ser fallos introducidos por la rama, mientras que las 15 excedencias preexistentes siguen siendo de la fix #200 / issue #176. El re-base #4 (véase la fila del 2026-09-12) sustituyó dos de estos cuatro valores — `plan-feature:scaffold` es ahora 24286 y `plan-fix:issue` 27169; los números de arriba registran lo que publicó el re-base #2 en esa fecha.
- **2026-09-11 — el contrato de fases rechaza una fase vacía.** La revisión de la feature 37 F5 hace que `phase-contract` 1.0.3 rechace una fase que no lleva ninguna tarea: la regla 3 ahora fija un **mínimo de ≥ 1 tarea** junto al presupuesto de ≤ 8 (fase final de hardening/cierre: 1–10), de modo que un plan cuya fase no tiene tareas marcables BLOQUEA box-3 en lugar de obtener un `PASS (8/8)` vacuo. El usuario ratificó el veredicto de bloqueo estricto — una fase vacía significa que el plan está mal o la fase debe borrarse — y el corpus de `phase-lint` lo ancla.
- **2026-09-10 — tres rutas dejan de razonar sobre la forma de las fases.** La P4 de la feature 37 adelgaza `plan-feature-scaffold` 2.3.0, `plan-fix` 3.2.0 y `execute-phase` 4.5.0 para ejecutar el linter determinista (`bun scripts/phase-lint.mjs <plan>`, fallback node) y pegar su bloque de stdout: exit 1 es una fase BLOCKED que se re-corta, nunca un veredicto que se re-deriva leyendo las reglas. `phase-contract` sigue siendo el único propietario de las ocho reglas y del fingerprint, y el espejo de pi se re-empaqueta.
- **2026-09-10 — el contrato de fases sanciona ya el título que sus plantillas exigen.** La P1 de la feature 37 convierte a `phase-contract` 1.0.2 en el único propietario de la excepción: el título literal de cierre de las plantillas `Hardening & PR` es el único título con `&` autorizado — su `&` es un separador de normalización, no un conector de entregables, y su title-deliverable normaliza a `hardening-pr`; cualquier otro título con `&` sigue dando FAIL. La enmienda cierra la contradicción entre la lista de conectores de la regla 1 y el título de cierre que llevan todas las plantillas de feature/fix, sin añadir semántica más allá del texto del propio propietario.
- **2026-09-05 — los recibos dejan de ligar el roadmap.** Acotación del roadmap (pre-execution-review 2.0.0, review-spec 1.6.0): el constructor del snapshot pre-ejecución ya no liga el `ROADMAP.md` compartido como contexto `roadmap-row`. El roadmap es el libro mayor compartido de todas las unidades — las filas de otras unidades llegan mientras un plan está en vuelo, y las escrituras sancionadas de la propia máquina de estados (`planned` → `in-progress` en P1, `done` al abrir PR) invalidaban cada recibo en vuelo de todo el repositorio, forzando una re-reseña inútil justo cuando un orquestador menos se la puede permitir (un `execute-phase` bloqueado por un recibo cuyo único drift era la fila de otra feature). El contenido de la fila relevante para la seguridad sigue guardado por compuertas que lo re-leen en vivo — el gate de estado propio y la huella del recibo de dependencias — mientras que los artefactos de la unidad y las autoridades rectoras siguen ligadas por completo, así que una edición genuina de un artefacto sigue fallando cerrado. Los recibos grabados antes del cambio se re-verifican obsoletos exactamente una vez; ver `docs/workflow/MIGRATION.es.md`.
...
- **2026-09-05 — el mapper pre-escritura es dueño del hueco leer-antes-de-escribir.** El feature 29 P1–P2 introduce `implementation-discovery` 1.0.0, un contrato interno y de solo lectura que responde las siete preguntas de evidencia, emite un mapa compacto fijo y enruta `READY | REPLAN | NEEDS-DESIGN | BLOCKED` antes de cualquier escritura de rama/planificación/código — ningún conteo de ficheros/lecturas decide la ruta, y un mapper fresco se dispara en trabajo cross-layer/público/seguridad/compatibilidad, supuestos de Plan no probados, intentos previos o sesgo. `execute-phase` 4.3.0 lo carga tras la puerta pre-ejecución en lugar de reservarle el hueco; solo READY, con una sonda de falsificación observada y cada fila de evidencia de planificación llevada confirmada, autoriza la primera escritura. No se introduce ningún comando público, esquema, nuevo estado de workflow ni llamada a forge/issue.
- **2026-09-05 — los veredictos enrutan aguas arriba, nunca reparan.** El feature 29 P3 hace que `implementation-discovery` 1.1.0 enrute cada veredicto a su dueño — defectos de Plan a `plan-feature`/`plan-fix` más `review-plan`, huecos de Producto/autoridad a `design-feature` y luego `review-spec`, evidencia no disponible a un `BLOCKED` exacto — mientras los hallazgos locales de fuente se quedan locales, el mapping nunca reemplaza la autoridad de revisión/verificación/auditoría candidata y un segundo ciclo de reparación/re-revisión hereda el `CONVERGENCE-ANOMALY` del feature 28. La navegación semántica/de símbolos y Engram se nombran localizadores consultivos con respaldo directo de repositorio.
