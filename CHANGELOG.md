...
| Version | Date | Type | What changed |
|---|---|---|---|
| 3.7.0 | 2026-09-19 | minor | **Feature 32 P3/P4: ad-hoc finder scale removed + GATE-RAN mark recording:** P3 removes the ad-hoc severity scale mapping that let reviewers override the canonical severity table; P4 adds GATE-RAN mark documentation to the review-findings owner cell (`review-change:review-gate-ran-marks`), the `POLICY.md` §7 column-sets for gate-run recorders, and the Turn contract row. No rule, flag, or output shape changed. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 2.1.0 | 2026-09-19 | minor | **Feature 32 P3/P4: phantom MEDIUM severity fixed + severity-based scoring:** replaces the hard MEDIUM check in the 14-check report with a severity-based scoring system that no longer produces phantom MEDIUM severity findings for well-formed docs. |
...
| `pre-execution-review` | 2.5.0 | 2026-09-19 | minor | **Feature 32 P4 (collision fix + GATE-RAN):** bumps version from 2.4.0 to resolve CHANGELOG collision with feature 31's row; adds GATE-RAN mark recorders to the review-findings owner cell (both template projections) and records gate-run recorders in `POLICY.md` §7 column-sets. `LEDGERS.md` (§gate-ran@1) documents the mark. No rule, flag, or output shape changed. |
