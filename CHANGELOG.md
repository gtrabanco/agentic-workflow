...
| `review-implementation` | 1.2.1 | 2026-07-19 | patch | Clarified the fix-now override checks' scope: they gate `postpone`/`intentional-tradeoff` for a *confirmed real defect* only — `ignore` (false positive/negligible) is decided first and was never meant to route through this gate. Wording-only; no behavior change. |
