...
| 0.11.2 | 2026-09-19 | patch | **Companion-package re-bundle (fix #244):** publishes the updated `fold-findings` 1.5.1 — freeze-batch consumer now names the router's conclusion (`/plan-fix <n>` / `/plan-feature <slug>`) instead of the raw invocation, `<unit>` format documented, one-physical-line rule, regression pin. No package code change — the bump exists to publish the re-bundle. |
