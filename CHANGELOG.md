...
| Version | Date | Type | What changed |
|---|---|---|---|
| 0.6.0 | 2026-09-06 | minor | **fix/162 re-bundle**: re-bundles the 8 touched skills — `pre-execution-review` 2.1.0 (receipt self-check, roadmap-label gate independence, verdict-response guard scope), `review-spec` 1.7.0 (self-check RUN box), `review-plan` 1.6.0 (loses the design-verdict verdict; two-verdict set + `class: product` route), `plan-feature` 5.1.0 (done rows reroute to `/audit-pr`), `workflow-status` 3.2.0 (senses done-but-unmerged units), `design-feature` 3.3.0, `review-change` 3.3.0, `audit-pr` 5.1.0 (lineage gate names `impossible-timeline`). No package code change — the bump exists to publish the re-bundle. Route ceilings for the shared `pre-execution-review` reference re-based (declared, growth source `fix/162`). |
