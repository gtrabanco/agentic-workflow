...
| Version | Date | Type | What changed |
|---|---|---|---|
| 0.7.2 | 2026-09-09 | patch | **Bun first-class runtime (#197)**: `bundle:skills`/`test`/`mutation`/`prepare` scripts invoke bun (`bun test` over the `node:test`-style suite — verified 1.4.2), with an explicit `test:node` fallback variant; `engines` gains `bun ">=1.4"`. Re-bundles the 4 runtime-wording skills — `pre-execution-review` 2.2.1 (LEDGERS validator-column convention note), `review-spec` 1.7.1, `review-plan` 1.6.1, `workflow-status` 3.2.1 (bun-first snapshot commands). |
