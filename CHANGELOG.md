...
#### `execute-phase`
| Version | Date | Type | What changed |
| 4.7.1 | 2026-09-21 | patch | `references/PRE_EXECUTION_GATE.md` names `AGENTS.md` as the `project-guide` context row a plan snapshot binds, matching `scripts/pre-execution-contract.mjs`. No rule, gate, phase contract, or output shape changed. |
...
| `bump-skill` | 2.3.4 | 2026-09-21 | patch | The policy this skill enforces is cited as `AGENTS.md` section "Version every change", and the authoring-invariant lint reads the same seven rules from that file. No lint rule, semver rule, or synchronization surface changed. |
| `pre-execution-review` | 2.5.1 | 2026-09-21 | patch | **Fold F29: owner-entry wording:** `LEDGERS.md` §"Durable ledger write ownership" calls `fold-findings:folded-flag` the map's **owner entry** instead of its "annotator", matching the `ledger-ownership@1` row (the mechanical annotator is `scripts/ledger-provenance.mjs`). No rule or output-shape change. |
