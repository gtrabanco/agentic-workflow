...
- **2026-09-18 — the merge-ready box names its runtime (fix #182 replan).** The `audit-pr` turn-contract box now names `bun scripts/audit-pr-gate.mjs comment` as the step that posts the MERGE-READY comment and proves its marker landed, so the hand-assembled `gh pr comment --body-file` prose path is gone from the box; the reference (`03_AUDIT_PROCESS.md` step 7) already ran the runtime. `audit-pr` 5.2.1 — first of the #182 close-out phases P6–P9.
