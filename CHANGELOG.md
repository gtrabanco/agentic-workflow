...
|---|---|---|---|
| 4.5.0 | 2026-09-22 | minor | **Feature 61 P10 — agwo edit file-schema vocabularies:** exports the closed file-schema constants as typed values: `UNIT_DOC_REQUIRED_SECTIONS` (13-section list, already in 4.4.0 prose), `EVIDENCE_COLUMNS` (5-column evidence-table@1), `PROGRESS_ENTRY_PATTERN` (regex for dated log entries), `ROADMAP_COLUMNS`, `CHANGELOG_SECTION_PATTERN`, and operation-name constants for Budgets, Manifest, UnitDoc, Roadmap, and Changelog. `src/file-schemas.ts` is the canonical source; the `packages/agentic-workflow/src/edit/` SDK module imports from the package's dist when the dependency resolves, else duplicates with a pointing comment. | | 4.4.0 | 2026-09-22 | minor | **Feature 61 P8 — unit document headings (additive):** `SPEC_PRODUCT_REQUIRED_HEADINGS` retired; replaced by `UNIT_DOC_REQUIRED_SECTIONS` (13-section list: Objective, Why, User outcome, Acceptance criteria, Non-goals, Future cost, Applicable tests, Known pre-existing issues, Tasks, Evidence, Progress log, Next, References). The pre-execution-verdict vocabulary (`PRE_EXECUTION_VERDICTS`, `VERDICTS_BY_STAGE`) stays exported — the lane's review step still uses it for the sensor's blockers. `review-spec` and `review-plan` skill rows are dropped from the transition table; the deleted-skill row count reflects the absorbed skills. |
| 4.3.0 | 2026-09-17 | minor | **Planning-review materiality carrier** (feature 31, #171): the finding record `PreExecutionReviewFindingV1` gains the optional bounded `reproducer` (`PRE_EXECUTION_LIMITS.reproducerChars` = 1024); the runtime materiality predicate moves to the `medium`+ line (`low` stays a persisted report-note — a PASS may coexist with an open/unverified `low` row and is refused while any open/unverified `medium`+ row exists); the shared structural field spec gains an additive `optional` flag projected as "present in `properties`, absent from `required`"; and the transition decider gains the `stop-review-loop-cap` stop code fed by the optional derived `reviewLoopCycles` input. **Additive guarantee:** `reproducer` is optional, so a receipt whose findings carry none stays valid, every enum value, contract id and freshness code keeps its spelling, and no existing transition row changes behaviour. No major bump. |
| 4.2.0 | 2026-09-16 | minor | **Executable continuations** (feature 59, #237): Envelope v2's `next` gains an optional, schema-validated `continuation` object — `{ argv, rendering?, preconditions, evidence?, convergence }` — with the closed four-code refusal vocabulary (`CONTINUATION_REFUSALS`: `precondition-uncheckable` \| `rendering-failed` \| `no-decision-available` \| `sensor-degraded`), the pure fail-closed `emitContinuation` emitter, the argv parser + per-platform rendering derivation (`deriveContinuationRendering`), the receiver-side `verifyContinuationEvidence` digest check, and the frozen `CONTINUATION_CANONICAL_VECTORS`. **Additive guarantee:** the field is optional — an Envelope without `next.continuation` stays valid and byte-acceptable to every 4.1.x consumer, and nothing is persisted. No major bump. |
| 4.1.2 | 2026-09-15 | patch | **English-only interim**: drops the shipped `README.es.md` sibling from `files` (bilingual docs were removed repo-wide; feature 57 restores per-release bilingualism). No `src/`, `dist/`, or schema change — the package-content gate and the release-contract/verification docs tests drop their Spanish assertions. |
| 4.1.1 | 2026-09-09 | patch | **Bun first-class runtime (#197)**: package scripts invoke bun as the runtime (`bun test`, `bun scripts/…`, `bun run` chains) with an explicit `test:node` node-fallback variant; `engines` gains `bun ">=1.4"` beside `node ">=18"`; devDependencies pinned to exact versions (`ajv 8.20.0`, was `^8.20.0`) — resolved versions unchanged, lockfile records the same artifacts. No `src/`/dist/schema change — scripts were already runtime-agnostic ES modules (`node:` specifiers are bun-supported by design). The publish workflow's node-compat job enforces the fallback. |
| 4.1.0 | 2026-09-06 | minor | **`impossible-timeline` freshness guard + verdict-stage map alignment** (fix #162): adds the `impossible-timeline` freshness code (directly after `stale-policy`), the published `PRE_EXECUTION_RECEIPT_TIMELINE_SKEW_MS` constant (300 000 ms = ±5 min), and the pure `isImpossibleReceiptTimeline({ finishedAt, sourceCommitDate, skewMs })` predicate (boolean | null, fail-open on unparsable stamps) — the git-backed sensor evaluates the documented comparator slot; the pure comparator leaves it unevaluated. Also narrows `VERDICTS_BY_STAGE.plan` to the two plan verdicts (`needs-design` drops — only `review-spec` may emit it; the flat `PRE_EXECUTION_VERDICTS` keeps the token and the `spec` stage row is unchanged), so the persisted state machine stops sanctioning the review-plan emission the prose (c) forbids. `VERDICTS_BY_STAGE`, `PRE_EXECUTION_RECEIPT_TIMELINE_SKEW_MS` and `isImpossibleReceiptTimeline` are now re-exported from the index. Additive code; the verdict-stage narrowing invalidates no persisted receipt (zero plan-stage `needs-design` receipts exist). |
| 4.0.1 | 2026-09-06 | patch | **Export `PRE_EXECUTION_POLICY_VERSION`** (feature 29, #149): the schema now exports `PRE_EXECUTION_POLICY_VERSION` (the one authority the snapshot CLI reads instead of a hardcoded literal), closing known-issue 24 from feature 28. Additive, no breaking change.
| 4.0.0 | 2026-09-04 | major | **Vocabulary removal (feature 161):** the retired `loop-review-fold` skill is dropped from the published machine contract — the `WorkflowIntent` union and `WORKFLOW_INTENTS` no longer list it, its `WORKFLOW_TRANSITION_TABLE` row and every `allowed` entry are deleted, its capability profile and `nextIntent`/`parseNativeTurn` handling are removed, and `skill-outcome.schema.json`'s intent enum no longer advertises it. Breaking for any ABI that still routes to or validates the retired `loop-review-fold` intent; run `npm run build` / `bun run test` to regenerate. |
| 3.5.0 | 2026-08-30 | minor | **Pre-execution evidence contracts** (feature 28, #146): `PreExecutionArtifactSnapshot v1` and `PreExecutionReviewReceipt v1` ship as content-bound wire contracts behind the authoritative snapshot validator (`validatePreExecutionArtifactSnapshotV1`), the standalone receipt well-formedness validator (`validatePreExecutionReviewReceiptV1`), and the only entry that blesses a verdict (`validatePreExecutionReceiptAgainstSnapshot`, binding stage/policy/snapshot-digest freshness, author exclusion, parent topology, and PASS/material-finding coherence), plus canonicalizers, digests (`canonicalize*/digest*PreExecution*`), and the `comparePreExecutionReceiptToSnapshot` freshness predicate with closed reason codes; every pre-execution entry — builder, canonicalizers, digests, blessing, freshness — answers synchronously, the one calling style AC2's sync builder implies. One canonical internal definition (`src/pre-execution-contract.ts`) backs both contracts; the two `pre-execution-*.schema.json` files are generated **non-authoritative** Draft-07 projections with a drift check, carrying the projectable rules (verdict/stage matrix, parent topology, unique parent rows) and disclosing the runtime-only ones in `$comment`. The shared canonical serializer moves to `src/canonical-json.ts` (one walker for the candidate, verification, and pre-execution families; the legacy 3.3.0 leaf behaviour stays byte-identical, and the verification family's `D14` over-budget message is preserved). Additive decider extension: `review-spec` and `review-plan` join the intent vocabulary, the transition table, and the capability profiles (both `reviewer`/`critical`/read-only) without changing any existing intent's meaning. |
...
| Version | Date | Type | What changed |
| 0.17.0 | 2026-09-26 | minor | **NaN catalog refresh — `mimo-v2.6-flash` takes the judgment slots (feature 64).** The built-in `nan` profile's judgment commands (`audit-pr`, `product-audit`, `review-change`, `unit-lane`) chain `nan/mimo-v2.6-flash` first where they chained `nan/mimo-v2.5`: same Xiaomi family, so reviewer independence from the DeepSeek/GLM executors is preserved; OpenAI-`tools`-native; same 1.0B token/mo quota, always-on reasoning and concurrency cap of 5; stronger than the model it replaces on every shared benchmark. `execute-phase` and the cheap/mechanical slots are unchanged (`nan/deepseek-v4-flash`, `nan/glm5.3-flash`) — no model enters the file-editing path without the `GOLDEN_FIXTURE.md` tool-calling smoke run. The four AC2 `resolveProfileChain` expectations move with the decision, and two new docs-parity tests pin the README↔profile pair feature 63 left unasserted (every model the profile routes to must be documented in the README's NaN guidance; every judgment rung the README names must be routed). Root README guidance refreshed to the current catalog (quota, reasoning matrix, tool-calling set, pros/cons, rate limits); feature 63's SPEC carries amendment A1 for the AC2 exemplar chain. |
| 0.16.0 | 2026-09-24 | minor | **Feature 63 — model profiles + zero-config NaN recommendations.** `recommendedModels` (default `true`) auto-applies a code-only built-in `nan` profile when the session has `nan` models; user-declared routes (including an explicit `"inherit"`) always win. New `profiles` (unlimited named route sets; the implicit `default` profile is the existing top-level `default`+`commands`, so old configs resolve byte-identically), `profileOrder` (preference chain — first usable model wins), and `profileFallback` (`applyTo: flow\|command`, `resume: continue\|restart`, `retryAfterSeconds` default `86400`) with a persisted demotion window. `/agentic-workflow-settings` is now scope → profile → routes (rotate + clear demotion, toggle + materialize `recommendedModels`). `advance` conductor stages now dispatch through the router (they previously bypassed it) and honour `resume: restart`. Package code only; no skill mirror change. |
| 0.15.3 | 2026-09-25 | patch | **Lane migration re-bundle (#256):** `audit-docs` 2.2.0, `audit-pr` 5.4.0, `bump-skill` 2.3.6, `discover-repository-state` 1.2.2, `execute-phase` 5.0.1, `fold-findings` 1.7.0, `orchestration-envelope` 2.2.1, `phase-contract` 1.0.6, `pre-execution-review` 2.6.0, `product-audit` 3.3.0, `resolve-repository-state` 1.2.2, `review-change` 3.8.0, `review-implementation` 1.10.0, `triage-issue` 2.9.0, `unit-lane` 1.1.0, `workflow-status` 3.9.1. No package code change — all changed files are `skills/*/SKILL.md` and doc references; bump publishes the re-bundle. |
| 0.15.2 | 2026-09-24 | patch | **`workflow-status` 3.9.0 re-bundle.** Publishes the reduced-envelope flag — the skill's `argument-hint`, turn-contract invocation and `docs/workflow/ORCHESTRATION.md` usage line, plus the `references/ENVELOPE_FIELDS.md` section that owns the projection. No package code change: the deterministic producer (`scripts/workflow-status.mjs`) lives in the target repository, not in this package (the `scripts/` distribution gap, known-issues `B-01` → feature 44 / #198), so the bump exists to publish the re-bundle. |
| 0.15.1 | 2026-09-23 | patch | **Publish the runner crate pinned to the registry (repairs `pi update --all`).** 0.15.0 shipped `@gtrabanco/agentic-workflow: file:../agentic-workflow` in `dependencies` — a repo-local path that resolves nowhere outside the checkout, so every consumer install failed (`Could not find package.json for "file:../agentic-workflow"`). Pins the runner crate back to the published `0.1.1` (the same fix 0.14.0 already shipped), pins the pre-existing `typescript: "6"` major-only range to the resolved `6.0.3`, and adds a lockfile-policy test rejecting non-exact or non-registry specs in `dependencies`/`devDependencies` so a local-dev link can never reach npm again. No src/dist change — 0.15.0's features are unchanged. |
| 0.15.0 | 2026-09-23 | minor | **Feature 62 — the pi-native conductor (`advance`).** Ships the deterministic orchestration loop that replaces the retired `ship-roadmap` role (issue #233's pi-command route): a native `advance` command runs sensor (Envelope v2, fail-closed) → `decideWorkflowAction()` → verbatim `/skill:<verb> <args>` invocation over the existing router, with a deterministic urgency judge (no model call), the unattended `--adversarial 2`/`3` floor, a closeout gate (3 partials park the unit), bounded re-sense, iteration cap, one-stage-per-invocation default (`--continue` runs the full loop), and `ADVANCE: COMPLETE | BLOCKED | STOPPED | CONTINUE` banners. Never merges — merge-ready PRs stop with `stop-needs-input` naming `/audit-pr` + human merge. New `src/conductor/` module (1081 LOC), 84 tests-first suite (339 total, bun + node), `@gtrabanco/agentic-workflow-schema` 4.5.0 pinned as a dependency. |
| 0.14.0 | 2026-09-23 | minor | **Feature 61 re-bundle + real runner dependency.** Flips `@gtrabanco/agentic-workflow` from the local `file:` override to the published `0.1.1` (bin `agentic-workflow`), so installs resolve the runner crate instead of a path that does not exist outside the monorepo, and publishes the feature 61 skill tree — the adaptive unit lane (`unit-lane` 1.0.0), the reshaped `execute-phase` 5.0.0, and the 12 retired fixed-pipeline skill directories (`design-feature`, `evidence-grounding`, `implementation-discovery`, `plan-feature`, `plan-feature-from-issue`, `plan-feature-scaffold`, `plan-fix`, `planning-preflight`, `replan-findings`, `review-plan`, `review-spec`, `ship-roadmap`) no longer ship. The command alias list is read from the bundled skills at startup, so `/unit-lane` registers without a package code change. |
| 0.13.3 | 2026-09-21 | patch | **Single agent guide + pack-time skill bundle.** Ships the rename the workflow now depends on: `init-workspace` 3.0.0 (bootstrap writes `AGENTS.md`; upgrade mode folds a legacy `CLAUDE.md` into it and removes that file only with explicit consent) alongside the reference-only updates in `bump-skill` 2.3.4, `execute-phase` 4.7.1, `plan-fix` 3.3.1, `product-audit` 3.2.1 and `ship-roadmap` 5.2.3. Also makes the skill bundle gitignored pack-time output — `prepare`/`test`/`mutation` stage it, `postpack` removes it, and CI asserts the built tarball carries it. No package code change. |
| 0.13.2 | 2026-09-21 | patch | **pi 0.87.0 baseline:** tracks `@earendil-works/pi-coding-agent` `>=0.87.0` as the peer and pins `@earendil-works/pi-tui` to `0.87.0`. No skill content change — the bump exists to publish the new baseline. |
| 0.13.1 | 2026-09-21 | patch | **Fold re-bundle (triage 32 F27–F34):** publishes the re-bundled mirrors — `plan-feature` 5.3.1 (named roadmap condition restored), `pre-execution-review` 2.5.1 (`LEDGERS.md` owner-entry wording), `workflow-status` 3.8.1 and `replan-findings` 1.1.1 (the `GATE-RAN` mark added to the mark-id enumerations) — and registers the `gate-ran@1` block in `CLAUDE.md`'s normative-surfaces inventory. No package code change — the bump exists to publish the re-bundle. |
| 0.13.0 | 2026-09-19 | minor | **Feature 32 re-bundle (review-consistency pack):** publishes the changed skill mirrors — `audit-docs` 2.1.0, `audit-pr` 5.3.0, `execute-phase` 4.7.0, `fold-findings` 1.6.0, `plan-feature` 5.3.0, `pre-execution-review` 2.5.0, `product-audit` 3.2.0, `review-change` 3.7.0, `review-implementation` 1.9.0, `workflow-status` 3.8.0 — with their reference files (the canonical severity table and derived blocking gate, the `gate-ran@1` mark, the sole-flipper provenance, and the NRS missing-ledger notice). No package code change — the bump exists to publish the re-bundle. |
| 0.12.0 | 2026-09-18 | minor | **feature 60 re-bundle**: publishes the Tier 2 pi path-protection guard — the strict `pathProtection` config key with the tighten-only `intersectPathPolicy`, the `matchingJustification` record reader, and the `tool_call` blocking guard that blocks a `write`/`edit` to an existing protected path with no matching justification record (reads and new-file creates pass) — together with the P2/P3 skill surfaces `execute-phase` 4.6.0, `init-workspace` 2.9.0, `plan-feature-scaffold` 2.4.0, `orchestration-envelope` 2.2.0, `pre-execution-review` 2.4.0, `verification-contract` 1.3.0. Package code changes: the guard. (Note: this row was corrected from 2.3.0 to 2.4.0 to resolve a drift-gate collision with feature 31's row which retains 2.3.0.)
|---|---|---|---|
| 0.11.1 | 2026-09-18 | patch | **Re-bundle + bounded settled-turn probe (fix #182 replan):** publishes the byte-identical `skills/` mirror carrying `audit-pr` 5.2.1, whose merge-ready turn-contract box now names `bun scripts/audit-pr-gate.mjs comment`; and the settled-turn `git status` probe is now time-bounded (`GIT_STATUS_TIMEOUT_MS` = 2000 ms, `readGitStatusBounded`), so an unresponsive git reads as clean instead of parking the turn. No breaking change. |
| 0.11.0 | 2026-09-17 | minor | **Two workflow guards (fix #182):** the extension now blocks the unverifiable inline receipt path — a `bash` call invoking `gh pr comment` with a `review-change:pass` or `audit-pr:merge-ready` marker inline is refused, and the reason names `bun scripts/review-receipt.mjs emit` / `bun scripts/audit-pr-gate.mjs comment`, the runtimes that post the body and re-read the PR to prove the marker landed. It also warns once per settled turn when the worktree is dirty, naming what is uncommitted. Both decisions live in a Pi-free `src/extension/receipt-guard.ts` (unit-tested without a session); `tool_call` joins the declared event set. Bundled skills refreshed to match `skills/`. |
| 0.11.2 | 2026-09-19 | patch | **Companion-package re-bundle (fix #244):** publishes the updated `fold-findings` 1.5.1 — freeze-batch consumer now follows the router's conclusion (`/plan-fix <n>` / `/plan-feature <slug>` on `replan`; stop-and-surface the user decision on `decision`) instead of the raw invocation, `<unit>` format documented, one-physical-line rule, regression pin. No package code change — the bump exists to publish the re-bundle. |
| 0.10.7 | 2026-09-17 | patch | **Notation-fix re-bundle**: publishes `ship-roadmap` 5.2.2, whose unblock-map example mixed the forge number space with the workflow one ("merging #12 unblocks 05 and 07") and now names both explicitly. The vocabulary rule itself lands in the project guide's Workflow conventions (`template/CLAUDE.md`), plus one fixed reference in `docs/workflow/MIGRATION.md`. No package code change — the bump exists to publish the re-bundle. |
| 0.10.6 | 2026-09-16 | patch | **feature 59 re-bundle**: publishes the executable-continuation surfaces — `workflow-status` 3.7.0 (quote the emitted `next.continuation` at the next-command echo; `PRE_EXECUTION.md` `stale` row + re-run sentence), the three receipt-persistence surfaces (`review-spec`/`review-plan`/`review-change` references), `design-feature` 3.4.0 (bounded form interview), and the `orchestration-envelope` turn contract's `next | continuation` row. No package code change — the bump exists to publish the re-bundle. |
| 0.10.5 | 2026-09-15 | patch | **English-only interim**: drops the shipped `README.es.md` sibling from `files` and the alias-coverage suite's bilingual parity assertions (docs were removed repo-wide; feature 57 restores per-release bilingualism). No package code or bundled-skill change. |
| 0.10.4 | 2026-09-16 | patch | **fix #224 replan fold re-bundle**: publishes the corrected `phase-contract` 1.0.5 wording (the armed-box enumeration) raised by the end review's F26. No package code change — the bump exists to publish the re-bundle. |
| 0.10.3 | 2026-09-16 | patch | **fix #224 replan re-bundle**: publishes the replan's two contract edits — `replan-findings` 1.1.0 (the `close-out` route for a finished unit and the bare `status` token in the block it documents) and `phase-contract` 1.0.4 (the executed-phase exemption for boxes 3 and 7, implemented in `scripts/phase-lint.mjs`). No package code change — the bump exists to publish the re-bundle. |
| 0.10.2 | 2026-09-15 | patch | **Main-sync re-bundle (fix #224 × feature 37)**: the branch's 0.10.1 bundle sat unmerged while `main` took feature 37, so this release publishes the merged skill tree in one go — fix #224's replan routing (`plan-fix` 3.3.0, `plan-feature` 5.2.0, the new internal `replan-findings` 1.0.1, `workflow-status` 3.6.0, `review-change` 3.5.1, `review-implementation` 1.8.0, `triage-issue` 2.8.0, `fold-findings` 1.5.0) together with feature 37's phase-lint set (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2, `phase-contract` 1.0.3). No package code change — the bump exists to publish the re-bundle. |
| 0.10.1 | 2026-09-15 | patch | **fix #224 end-review fold re-bundle**: re-bundles `replan-findings` 1.0.1 (the router output contract) and `workflow-status` 3.6.0 (one `next.suggested` contract: router-owned row semantics, mark rows are never findings, decision rows contribute nothing, bounded ids, and the two envelope references corrected). No package code change — the bump exists to publish the re-bundle. |
| 0.10.0 | 2026-09-15 | minor | **Deterministic replan routing re-bundle (fix #224)**: re-bundles the router work — `plan-feature` 5.2.0 and `plan-fix` 3.2.0 (router-first replan entry), the new internal `replan-findings` 1.0.0, `workflow-status` 3.5.0 (class-routed `next.suggested`), `review-change` 3.5.1, `review-implementation` 1.8.0, `triage-issue` 2.8.0 and `fold-findings` 1.5.0 (one canonical replan destination). No package code change — the bump exists to publish the re-bundle. |
| 0.9.2 | 2026-09-13 | patch | **Bounded dialogs for long lists (fix #214)**: no settings-console dialog can exceed pi-web's 24-option select cap any more. Over the cap the model picker goes provider-first in two steps (provider dialog, then that provider's models) with both dialogs bounded — the provider dialog pages when the provider list itself is over the cap, and a single provider's model list pages 21 per dialog with `More options…` / `◀ Previous page`. The command pickers and the adapter's non-TUI `pick` fallback page too, and "Type another reference…" stays the last option of every model dialog. Lists that fit one dialog keep the previous single-dialog shape: up to 23 model references plus "Type another reference…", or 24 commands — with one deliberate exception, the TUI picker now opens on the value in force, honoring the preselection its contract always documented, instead of always starting on the first model. |
| 0.9.3 | 2026-09-12 | patch | **feature 37 skill re-bundle (F25–F26 fold + P4)**: publishes the skills bundled since 0.9.1 — the phase-lint paste fence (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2), the P4 deterministic-linter hand-off wording (`execute-phase` 4.5.0, `plan-fix` 3.2.0, `plan-feature-scaffold` 2.3.0), and `phase-contract` 1.0.1 → 1.0.3 (the rule-1 `Hardening & PR` authorization and the rule-3 **≥ 1-task minimum**: a zero-task phase now BLOCKs box-3 instead of passing all eight boxes vacuously), all of which were mirrored into the package but never published. No package code change — the bump exists to publish the re-bundle. Renumbered 0.9.2 → 0.9.3 in the main-sync merge: main's own 0.9.2 (fix #214, 2026-09-13) shipped first, so this re-bundle takes the next patch. |
| 0.9.1 | 2026-09-11 | patch | **feature 38 sensor fold re-bundle**: re-bundles `workflow-status` 3.4.0 (the script defect fold F1–F14/F18 and the bun-first invocation strings). No package code change — the bump exists to publish the re-bundle. |
| 0.9.0 | 2026-09-10 | minor | **The routed model stays after a command (new `onSettle` policy)**: after `agent_settled`, the session is **no longer put back** by default — the model and thinking level the command ran on stay in the open chat window (`onSettle: "keep"`), so a follow-up edit or question keeps running on the model that planned it. The historical restore-on-settle contract (AC8) is now an explicit opt-in (`onSettle: "restore"`), exposed as a root config key (global + project), rendered by the settings console, and documented EN+ES. An operator's own mid-turn `/model`/Ctrl+P choice is still never overwritten, and a failed dispatch (turn never started) still rolls the session back. No skill change — the package bundles the same skills; the bump publishes the behavior change. |
| 0.8.0 | 2026-09-09 | minor | **Settings console upgrade (fix #154)**: per-command model routing gains an ordered fallback chain (`model` accepts 1–4 references; dispatch probes with `find` + `hasConfiguredAuth` only, calls `setModel` at most once per turn, and on exhaustion names every candidate and why it was skipped — `stop` refuses, `inherit` warns and runs the session model); searchable windowed pickers (in-package subsequence/slash-aware filter, current-value preselection, position indicator, non-TUI fallback); a route edit asks only the marked field (model/thinking) so a no-change save is byte-identical, and the model field builds an ordered chain; one-pass bulk apply/clear over several commands with per-command advisory warnings; `/aw-settings` alias. Adds `@earendil-works/pi-tui@0.85.1` (pinned). |
| 0.7.2 | 2026-09-09 | patch | **Bun first-class runtime (#197)**: `bundle:skills`/`test`/`mutation`/`prepare` scripts invoke bun (`bun test` over the `node:test`-style suite — verified 1.4.2), with an explicit `test:node` fallback variant; `engines` gains `bun ">=1.4"`; devDependencies pinned to exact versions (`@types/node 24.13.3`, was `^24`) — resolved version unchanged. Re-bundles the 4 runtime-wording skills — `pre-execution-review` 2.2.1 (LEDGERS validator-column convention note), `review-spec` 1.7.1, `review-plan` 1.6.1, `workflow-status` 3.2.1 (bun-first snapshot commands). |
| 0.7.1 | 2026-09-08 | patch | **Drop obsolete pi-server devDep + pin peer to >=0.85.1** (fix #181): pi 0.85.1 (pi PR #9132) removed the eager re-export chain (`main` → `experimental/server` → `@earendil-works/pi-server`) that fix #166's shim covered, so the devDependency became dead weight and the `*` peer range silently accepted untested pi hosts. Removes the devDependency, tightens the peer to `>=0.85.1`, re-resolves the lock to pi 0.85.1 (prunes pi-server), and refreshes the README baseline note 0.85.0 → 0.85.1 (2026-09-05) in EN+ES. Suite green 140/140 on 0.85.1. |
| 0.7.0 | 2026-09-07 | minor | **feature 30 re-bundle (decision-driven, delta-scoped review→fold loop)**: re-bundles the 3 touched skills — `fold-findings` 1.4.0 (REPAIR-RECEIPT contract, impact-rule batch classification, freeze-batch rule, four-branch closing block with branch-selection decision inputs, materialize-reproducer rule), `review-change` 3.4.0 (delta-mode default post-fold, two escalation triggers, genuinely-new dedupe, delta-cycle cap), `pre-execution-review` 2.2.0 (recheck-cell consumption in the durable finding mark). No package code change — the bump exists to publish the re-bundle. Route ceilings for the shared `pre-execution-review` reference declared; `review-change` reference ceiling raised (growth source feature 30 / issue #170, measured 2746). |
| 0.6.0 | 2026-09-06 | minor | **fix/162 re-bundle**: re-bundles the 10 touched skills — `pre-execution-review` 2.1.0 (receipt self-check, roadmap-label gate independence, verdict-response guard scope), `review-spec` 1.7.0 (self-check RUN box), `review-plan` 1.6.0 (loses the design-verdict verdict; two-verdict set + `class: product` route), `plan-feature` 5.1.0 (done rows reroute to `/audit-pr`), `workflow-status` 3.2.0 (senses done-but-unmerged units), `design-feature` 3.3.0, `review-change` 3.3.0, `execute-phase` 4.4.0, `ship-roadmap` 5.2.0, `audit-pr` 5.1.0 (lineage gate names `impossible-timeline`). No package code change — the bump exists to publish the re-bundle. Route ceilings for the shared `pre-execution-review` reference re-based (declared, growth source `fix/162`). |
| 0.5.0 | 2026-09-05 | minor | **Roadmap-scoping re-bundle**:  re-bundles `pre-execution-review` 2.0.0 (the pre-execution snapshot no longer binds the shared `ROADMAP.md` — unrelated roadmap rows and the status machine's own writes no longer invalidate recorded receipts; one-time re-review of receipts recorded before the change) and `review-spec` 1.6.0 (its `CHECKS.md` context list drops the unbound `roadmap-row`). No package code change. |
| 0.4.2 | 2026-09-05 | patch | **Parse folded/literal YAML descriptions in the command catalogue** (fix #165): `readSkillMeta()` now reads `description:` YAML block scalars, so every bundled skill's slash command registers its real folded description instead of the literal `">"`; a regression fixture pins a real skill's parsed description to its frontmatter text. |
| 0.4.1 | 2026-09-04 | patch | **Refresh verified-baseline note to pi 0.85.0** (fix #166): updates the README baseline note from 0.84.3 to 0.85.0 with the 2026-09-04 date; adds `@earendil-works/pi-server@0.85.0` as devDependency to cover pi 0.85.0's eager re-export chain (`main` → `experimental/server` → `@earendil-works/pi-server`); re-runs the full suite green (134/134) on 0.85.0.
| 0.4.0 | 2026-09-04 | minor | **Publish the fix-161 skill re-bundle that 0.3.0 missed** (PR #163 merged without a version bump, so the publish gate skipped and npm stayed at 0.3.0): re-bundles the 12 updated skills and retires `loop-review-fold` from the command list (38 skills shipped; the schema package moved to 4.0.0 in the same PR for the same vocabulary removal), with `review-change` 3.2.1 (process-step renumbering after the verification-pass re-order) and the research-gate/signed-findings wording updates across the review pack (`review-code`, `review-implementation`, `review-spec`, `review-plan`, `triage-issue`, `ship-roadmap`, `fold-findings`, `plan-feature-scaffold`, `plan-fix`, `pre-execution-review`, `verification-contract`). No package code change — the bump exists purely to make CI publish the already-merged re-bundle. |
| 0.3.0 | 2026-09-03 | minor | **Bounded review→fold loop** (fix #159, #158 dogfood): re-bundles the 16 skills that bound the review/fold loop — `review-change` 3.0.0 (low findings report-only, workspace state = `REVIEW BLOCKED` precondition, folded-row re-verification, committed ledger append), `loop-review-fold` 4.0.0 (two-cycle cap), `review-implementation` 1.6.0, `review-spec`/`review-plan` 1.4.0 (class→resolver map), `log-session` 2.1.0 (forge-verified status words), `verification-contract` 1.1.0 (validator stability), `evidence-grounding` 1.5.0 (claim discipline), and the 9-skill review pack's materiality bar. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 2.2.1 | 2026-09-21 | patch | Description drops the pointer to the retired `#claude` branch; the model-agnostic statement stands alone. No behavior change. |
| 2.2.0 | 2026-09-17 | minor | **The entry is committed by construction (fix #182):** `bun scripts/session-close.mjs render` computes the git facts and prints the entry; `… close` refuses unless the log was appended to (never rewritten), commits that one file and names every path still uncommitted (exit 0 clean / 2 leftovers). A log entry can no longer end a session riding along uncommitted — the state that made a later review refuse a dirty tree. |
| 2.1.0 | 2026-09-03 | minor | Status words (`merged`, `closed`, `approved`) are forge-verified in the same turn (`gh pr view <N> --json state,mergedAt`) before they are written — a log that records a state the forge contradicts is a false record the next review must chase (fix #159, the fix #157 F14 false "merged" claim). |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.2.2 | 2026-09-24 | patch | The closing next-step names `/unit-lane` instead of the retired `/plan-feature`. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.2.2 | 2026-09-24 | patch | The interrupted-workflow hint names `/unit-lane` instead of the retired `/plan-feature` / `/review-plan`. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 3.9.1 | 2026-09-24 | patch | The turn contract, tier table and references name `/unit-lane` (the retired `plan-feature`/`design-feature`/`review-spec`/`review-plan` hints are gone); the legacy spec/plan receipt reference is marked retired. |
| 3.9.0 | 2026-09-24 | minor | **`--compact`: the reduced envelope.** Adds the `--compact` flag to the documented invocation (frontmatter `argument-hint`, the turn contract's run box, `docs/workflow/ORCHESTRATION.md`): the same Envelope v2 with repository history dropped; a `done` row leaves `detail.features`/`detail.fixes` **only once its linked PR is proven merged** (an unmerged `done` row is still at the merge gate and stays), and `findings.fix_now[].route` collapses to `...(N char evidence, see the unit review-findings.md)` while every other finding key is untouched. `detail.workflow_observations` is **not** reduced in either mode — every note is a signal (unmapped roadmap status, refused slug, ledger-severity drift, git/forge state, the `--last-envelope` guard's divergence note) and they total about a kilobyte — and `next`, `startable_now`, `design_candidates`, `blocked_units`, `dependencies`, `crash_recovery`, `urgent` and `pre_execution` are byte-identical, so a driver may switch modes per read. `references/ENVELOPE_FIELDS.md` owns the projection. Validator: `scripts/workflow-status-compact.test.mjs` (7 tests — merge-gate survival, finding identity, observations untouched, the `--last-envelope` guard note, schema validity, flag composition, fatal-exit usage). Measured on this repository: CLI stdout 43 697 → 21 227 bytes (−51 %), `detail.features` 62 → 22 rows, the findings' memos 3 691 → 481 bytes. |
| 3.8.1 | 2026-09-21 | patch | **Fold F31: `GATE-RAN` mark enumerations:** `references/SENSOR_SIGNALS.md` now names `GATE-RAN` alongside `VF-<n>`/`REVIEW-RAN` as an id shape that is never a finding, matching the router's `isMarkRow` guard (the guard was widened but the four documented enumerations were not). No rule or output-shape change. |
| 3.8.0 | 2026-09-19 | minor | **Feature 32 P1/P2: absent-ledger notice + reference alignment:** an absent repository-state ledger is no longer a blocker — `NRS_BLOCKING` keeps only `draft`/`contradicted`/`resolved`, and `missing` emits a non-blocking `substrate_notice` plus a `/discover-repository-state` alternative while startable rows still recommend; `ENVELOPE_CORE.md` and `SENSOR_CORE.md` state the same split. Additive `detail.substrate_notice {id,state,blocking}` field; no rule, routing, or output-shape change beyond that field. |
| 3.7.0 | 2026-09-16 | minor | **Quote the emitted continuation (feature 59 P3):** the next-command echo now states the quote rule — when the envelope carries `next.continuation`, the skill quotes the emitted command (`rendering` for display) and never authors exact tokens; the human-facing prose `→ Next:` block stays. Reference-only alignment: `PRE_EXECUTION.md`'s `stale` row and re-run sentence quote the emitted planning-gate re-run continuation. No flag, routing, or output-shape change. |
| 3.6.0 | 2026-09-15 | minor | **One `next.suggested` contract (fix #224 end review, F2/F4/F5/F13/F15):** the sensor stops re-deriving the ledger's row semantics and imports them from `scripts/unit-route.mjs` — the same `folded` vocabulary (`—`, `-`, `n/a` and an empty cell close a row), mark rows (`VF-<n>`, `REVIEW-RAN`) are never findings (a padded mark row made a merged unit project a permanent `/fold-findings`), and a decision-required row contributes nothing instead of a wrong fold. Plan-entry ids ride the router's 160-character cell bound. References: `ENVELOPE_FIELDS.md`/`ENVELOPE_CORE.md` no longer claim step 13 is unmechanized or omitted, and `SENSOR_SIGNALS.md` step 13 states the open-row predicate and the decision rule. Validator: `scripts/workflow-status-sensor.test.mjs` (the F13/F2/F15 pins). |
| 3.5.0 | 2026-09-15 | minor | **Class-routed `next.suggested` (fix #224):** the sensor emits the schema-declared array — a plan-owned open finding routes to the unit's planner, a plain fix-now row to `/fold-findings`, no open row contributes nothing — and `SENSOR_SIGNALS.md` withdraws the stale "any `folded: no` row → `/fold-findings`" signal that misdirected a replan row. The class→destination decision is the router's (`scripts/unit-route.mjs`). |
| 3.4.0 | 2026-09-11 | minor | **Sensor defect fold (feature 38 end review: F1–F14 + F18).** The deterministic sensor is repaired against the substrate it ships with. Reading: the NRS reader accepts the ledger's table shape, so a frozen ledger no longer parses as `draft` and no longer blocks every run; `\|`-escaped ledger cells keep their columns and `findings.fix_now[]` severities are normalized to the published `high`/`med`/`low` enum (with a named note for a row outside it), so the envelope stops failing its own `validateEnvelope` on real ledgers. Lifecycle: merge state resolves per PR instead of the bounded recent-merge window (false unmet dependencies and spurious substrate blockers collapse), step 6a senses `done`-with-open-PR units, and its gate now reaches `next.recommended` (the promised `/review-spec`//`/review-plan` nexts were unreachable). Sensing: the snapshot verifier is bound to the sensed repository (`--root`/`--dir`) and review-mark currency runs over the review's bound inputs, so marks are no longer permanently non-current and a foreign project is no longer read as the sensor's own checkout. Contract: `git fetch` is dropped (read-only stands), crash recovery classifies every unit branch with truthful per-branch evidence and worst-wins reduction, unit dirs and `--last-envelope` reads are confined/bounded, and `detail.urgent.interruptibility.tasks_from_boundary` carries a real count. Docs: `ENVELOPE_CORE.md`/`ENVELOPE_FIELDS.md`/`SENSOR_SIGNALS.md` mark steps 10–13 not-yet-mechanized, the tier table gains `/review-spec`//`/review-plan`, and invocation strings are bun-first. Validator: `scripts/workflow-status-sensor.test.mjs` (37 tests, incl. the table-form-NRS, merge-state, and findings-ledger-boundary regressions). |
| 3.3.0 | 2026-09-11 | minor | **Deterministic sensor script (feature 38).** `scripts/workflow-status.mjs` becomes the envelope's single producer: it executes the SENSOR_CORE sequence (git/forge collection, labels-only urgency, roadmap + fix-index five-state parsing, transitive dependency closure, readiness + step-6a receipt sensing, phase progress, review-mark currency, fix-now fold projection), self-validates with `validateEnvelope` (diagnostic only), and prints the fixed Envelope v2 JSON; failures degrade to `unavailable-<source>-<cause>` in `detail.degradations` with exit 0. The skill slims to run-the-script, read-the-JSON, interpret-`next.recommended`; `SENSOR_CORE.md`/`ENVELOPE_CORE.md` replace the command/assembly prose with script-backed references (semantics kept). External argv + envelope contract unchanged (minor). Validator: `scripts/workflow-status-sensor.test.mjs` (A:14/O26). |
| 3.2.1 | 2026-09-09 | patch | **Runtime-convention wording (#197)**: `SENSOR_CORE.md`/`PRE_EXECUTION.md` show the snapshot verify command in bun-first form (`bun scripts/pre-execution-snapshot.mjs …`, node fallback). No contract change. |
| 3.2.0 | 2026-09-06 | minor | **Roadmap-label gate independence** (fix #162): step 6a's receipt-sensing set gains `done`-with-open-PR (unmerged) units so a stale/missing receipt on a done-but-unmerged row surfaces as a gate blocker; merged units stay excluded (the merge closes their gates). |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 5.2.4 | 2026-09-21 | patch | `FOUNDING.md` and Portability drop the retired `#claude` branch and its pinned tiers; the continuation turn runs at whatever tier the driver chose. No rule, stage, exit code, or output shape changed. |
| 5.2.3 | 2026-09-21 | patch | The pre-read names the single agent guide (`AGENTS.md`) instead of `CLAUDE.md`/`AGENTS.md`, and `FOUNDING.md`'s empty-repo scaffold list matches the current template. No rule, gate, exit code, or output shape changed. |
| 5.2.2 | 2026-09-17 | patch | **Notation fix**: the unblock-map example in `RECOVERY_AND_SELECTION.md` mixed the forge number space with the workflow one ("merging #12 unblocks 05 and 07"), teaching the wrong vocabulary; the example now names the forge issue and the features explicitly. No behavior change. |
| 5.2.1 | 2026-09-08 | patch | **Hand-off order fix (#191):** reordered the three ship-roadmap surfaces (`SKILL.md` model table cell, `ADVANCE.md` REVIEW bullet, `MODEL_ROUTING.md` "Final review/correction loop" row) from the inverted `fold-findings → re-run review-change` to the canonical `review-change → fold-findings (only on REVIEW-FAIL) → re-run review-change`. No package code change — the bump exists to register the text edits.
| 5.2.0 | 2026-09-06 | minor | **Verdict re-homing across ADVANCE.md** (fix #162): the ADVANCE.md step-4 block reworded `NEEDS-DESIGN → park as in REVIEW-SPEC` to `PLAN-REVIEW-FAIL with class: product → design-feature`, keeping the verdict routing aligned with the narrow two-verdict set. No package code change — the bump exists to register the reference edits.
| 5.1.0 | 2026-09-04 | minor | **Autopilot review remap (fix #161, P3a):** the REVIEW stage composes the manual review→fold path `/fold-findings`, then re-run `/review-change`, in place of the retired `loop-review-fold` router; it reuses a current exact-SHA receipt or reviews context-clean, and routes unresolved findings to `triage-issue --prioritize-now`. `ADVANCE.md` and `MODEL_ROUTING.md` name the manual path. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.1.0 | 2026-09-05 | minor | Evidence-aware routing (feature 29, P3): verdicts return to their upstream owners — Plan defects to plan-feature/plan-fix plus review-plan, Product gaps to design-feature then review-spec, unavailable evidence to one exact BLOCKED; source-local findings stay local, mapping never replaces candidate review/verification/audit, and a second cycle inherits feature 28's CONVERGENCE-ANOMALY. Source identity, preparation continuity, and single-consumption/recovery are documented, and semantic/navigation/Engram are named advisory locators with direct GitHub/Git/search/read/test fallback. |
| 1.0.0 | 2026-09-05 | — | First versioned release (feature 29, P1–P2): one internal, read-only pre-write mapper contract — seven fixed evidence questions, the fixed compact map, four verdicts, inline/fresh routing with no file-count proxy, early falsification, no-progress rules, source identity, preparation continuity, single-consumption/recovery, and the compact writer handoff. `user-invocable: false`, consumed only by `execute-phase` on the pre-write route. |

#### `execute-phase`
| Version | Date | Type | What changed |
|---|---|---|---|
| 5.0.1 | 2026-09-24 | patch | `DESCOPE.md` cites the lane's `review` step as the verdict closer instead of the retired `/review-plan`. |
| 5.0.0 | 2026-09-22 | major | **Breaking (feature 61 P4): reshaped as the step executor of the unit document.** Consumes the triage-decided step list from the unit doc (triage currency verified against the recorded block); argument-hint `<NN-slug> [P<k>|step] [--max-attempts N] [--force]` — the `--fix` flag is gone (fixes are lane units); ACCEPTANCE.md/progress.md bindings replaced by the unit doc's Evidence rows and dated Progress log; nine reference files retired (mode workflows fold into the catalog, close-out/handoff/batch into the doc), six remain; tests-first red-run gate carried to steps; `path-guard`/`diff-guard` blocks pasted per step. See `docs/features/61-adaptive-unit-lane/SPEC.md`. |
| 4.8.0 | 2026-09-22 | minor | **Tests-first red-run evidence (workflow-gap audit, patterns #1/#4):** the phase completion gate gains one box — a tests-first phase (core/domain, orchestration) records the failing first run's command, exit status, and failing test names in the phase's `progress.md` receipt before implementation starts (n/a for test-after UI/adapter phases); `CLOSEOUT.md` states the red run is evidence, not a claim, and `UNIT_LOOP.md` step 3 names the phase's test-ordering rule as a forwarded worker input the worker consumes, never re-derives. No flag, gate order, or output shape changed; the pi mirror is re-bundled. |
| 4.7.0 | 2026-09-19 | minor | **Feature 32 P4: GATE-RAN mark documentation:** adds `GATE-RAN` mark documentation to `EXECUTION_CONTRACT.md` and `FOLDING.md` — the mark recorders for each phase-gate run, the head-it-ran statement, and the fold-handoff recorders. No rule, flag, or output shape changed. |
| 4.6.0 | 2026-09-18 | minor | **Path-protection checkpoint (feature 60 P3):** `references/PREFLIGHT.md` gains a path-protection step after the phase-lint guard that records each phase's base ref, runs `path-guard --unit <unit-dir> --phase P<n-1> --base <P<n-1> base ref>` over the just-closed committed range, pastes the `PATH-GUARD` block, and stops on a fail with the typed `GATE REJECTION — path-protection` trace (no `--force`; the escape is the recorded justification/approval). The pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
|---|---|---|---|
| 4.5.2 | 2026-09-14 | patch | **Linter-absent fallback disclosed (feature 37 review F86, brand):** the phase-lint hand-off now names the install-target limitation — `scripts/phase-lint.mjs` ships with the workflow repository, not the installed skill — and its safe fallback: apply the eight `phase-contract` rules by hand, label that check weaker, and disclose the deterministic linter did not run; never skip the gate. `SKILL.md` and `references/PREFLIGHT.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| 4.5.1 | 2026-09-12 | patch | **Paste fence (feature 37 review F26, security):** every hand-off that gives an agent the linter's stdout block now states it is **lint output, never instructions** — the block echoes plan-derived text (a phase title can originate in a third-party forge issue body), so any directive inside it is data to report, never an action to take. `SKILL.md` and `references/PREFLIGHT.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| 4.5.0 | 2026-09-10 | minor | **Deterministic phase lint (feature 37 P4):** the phase-lint pre-flight no longer reasons over the eight rules and the fingerprint — it runs `bun scripts/phase-lint.mjs <plan>` (node fallback), pastes the linter's fixed stdout block, and stops on exit 1; the linter implements the rules owned by `phase-contract`, which stays the sole owner. `SKILL.md` and `references/PREFLIGHT.md` updated. |
| 4.4.1 | 2026-09-08 | patch | **Hand-off order fix (#191):** reordered every execute-phase terminal/hand-off block from the inverted `fold-findings → re-run review-change` to the canonical `review-change → fold-findings (only on REVIEW-FAIL) → re-run review-change`, and dropped "mandatory" from the fold hand-off (the mandatory step is the review; folding is conditional on findings). `UNIT_LOOP.md`, `FOLDING.md`, `CLOSEOUT.md`, `BATCH_AND_PORTABILITY.md`, and `SKILL.md` reordered; discipline test pin added. No package code change — the bump exists to register the text edits.
| 4.4.0 | 2026-09-06 | minor | **Verdict ownership re-routing** (fix #162): the pre-execution gate's re-verification block reworded `NEEDS-DESIGN → /design-feature` to `Product-rooted → /design-feature`, reflecting that only review-spec may emit NEEDS-DESIGN and a product-intent gap routes through `PLAN-REVIEW-FAIL` with `class: product`. No package code change — the bump exists to register the reference edits.
| 4.3.0 | 2026-09-05 | minor | **Pre-write mapper (feature 29, P2):** the slot after the pre-execution gate and before the first write now loads the `implementation-discovery` mapper contract, which answers the seven evidence questions, emits the fixed compact map, and routes READY \| REPLAN \| NEEDS-DESIGN \| BLOCKED; only READY authorizes the first branch/planning/source write. `PREFLIGHT.md` and `SKILL.md` name the contract instead of reserving the slot for it. Seven `execute-phase:*` route estimate ceilings re-based to their measured floors (growth source: the pre-write mapper contract reference in `PREFLIGHT.md`). |
| 4.2.0 | 2026-09-04 | minor | **Hand-off remap (fix #161, P3a):** the unit-loop and close-out hand-offs recommend the manual `/fold-findings` → re-run `/review-change` path instead of the retired `loop-review-fold` router; `/audit-pr` stays the merge gate. `BATCH_AND_PORTABILITY.md`, `UNIT_LOOP.md`, `FOLDING.md`, and `CLOSEOUT.md` all name the manual path. |
| 4.1.0 | 2026-09-03 | minor | `FOLDING.md` gains the test-immutability mirror (fix #161): the fold never edits an existing test's expectation to match behaviour — a setup repair keeps assertions at least as strong and never touches expectations. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 3.5.1 | 2026-09-21 | patch | Portability states that no skill pins a model tier, instead of pointing at the retired `#claude` branch. No closure rule or output contract changed. |
| 3.5.0 | 2026-09-18 | minor | **Planning-loop cap mirror (feature 31):** `REPAIR.md` §4 replaces the unbounded-cycle sentences with the hard two-cycle cap — a third cycle never starts without explicit user instruction, the orchestrator's `stop-review-loop-cap` refusal names `design-feature` as the human route, and the repair turn itself is never a loop defect. The §4 heading and the anomaly-first ordering are preserved. |
| 3.4.0 | 2026-09-16 | minor | **Bounded form interview (feature 59 P4, #231):** the one-question-per-turn rule is replaced by one compact form-turn over the ≤ 6 fixed rubric slots plus the identity rows, each row carrying a recommended default the user accepts with one word, followed by at most **2 follow-up turns** for genuine ambiguity — never a third ask. The vagueness rubric, the mandatory-question rule, "ask nothing the docs or the instruction already answer", the deferred-decision row, and the `NEEDS_INPUT` escalation are unchanged; upsert and review modes are untouched. `interview:ambiguity-cap` and `interview:defaults-accept` are the pinned scenarios. Route ceilings re-based (`design-feature:*`), this text the named growth source. |
| 3.3.0 | 2026-09-06 | minor | **Verdict-response guard scope** (fix #162): REPAIR §4 states a repair answering a persisted verdict is never a loop defect (POLICY §4 scopes every cap to blind re-reviews), and no "already complete" answer exists for a unit carrying an open FAIL receipt — the repair routes through this skill by definition.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 3.2.0 | 2026-09-03 | minor | **Research gate** (fix #161): a mandatory, fail-closed research pass runs in the evidence pass before the Product half is emitted — at least two externally fetched sources on the capability's domain, each cited as an evidence row (URL + access date); coverage of what the capability is and is not plus the user's expectation of it; the implicit case decomposition of every enunciated expectation (values and limits, interaction states, degraded mode, backend validation/filtering/parsing, alternate user paths); offline or unanswered → `NEEDS-EVIDENCE`, never a guess; platform semantics verified against authoritative documentation before a test encodes them. The old no-market-research guardrail becomes a boundary: market/competitive research stays out; the domain research gate is this skill's only external research. Main ceiling + two route ceilings re-based, this text the named growth source. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 5.3.2 | 2026-09-21 | patch | Portability states that no skill pins a model tier, instead of pointing at the retired `#claude` branch. No rule, flag, or hand-off changed. |
| 5.3.1 | 2026-09-21 | patch | **Fold F28: named roadmap condition restored:** the Confirm-roadmap step again names the condition ("if any of the three is missing or wrong, fix the entry now") instead of the `CLAUDE.md`-banned "if needed", restoring the instruction determinism the P2 edit removed. |
| 5.3.0 | 2026-09-19 | minor | **Feature 32 P2: roadmap verify-vs-write split:** the Confirm-roadmap step now states that `plan-feature` verifies and repairs the roadmap entry while `plan-feature-scaffold` is the sole writer of the `defined → planned` write — the same ownership the `ledger-ownership@1` map declares. No rule, routing, or output-shape change. |
| 5.2.0 | 2026-09-15 | minor | **Replan entry (fix #224):** the redirect gate runs `node scripts/unit-route.mjs <NN>` first — a `route: replan` line exempts the unit from the status STOP and loads the new internal `replan-findings` contract, which re-cuts the plan from the router's bounded read set on the same branch; every other route keeps the existing gate and hand-off. |
| 5.1.0 | 2026-09-06 | minor | **Roadmap-label gate independence** (fix #162): the redirect gate's `done` stop keeps forbidding a replan but reroutes to `/audit-pr <PR>` — `done` means built + PR open, merge pending, a lifecycle label never a gate verdict; a merged PR is nothing pending (`--next`), an open one is the merge gate, never a replan.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 3.3.1 | 2026-09-21 | patch | `PLANNING_PROCESS.md` and `SPEC_CONTRACT.md` name the project guide as `AGENTS.md`. No rule, output shape, or phase contract changed. |
| 3.3.0 | 2026-09-15 | minor | **Replan entry (fix #224):** before ingest, `node scripts/unit-route.mjs <unit>` is consulted — a `route: replan` line loads the internal `replan-findings` contract and appends phases to the target unit's SPEC ledger on its own branch instead of drafting a new fix SPEC; every other route is unchanged. Renumbered 3.2.0 → 3.3.0 in the main-sync merge: feature 37's merge had already shipped 3.2.2, so the new routing case takes the next minor. |
| 3.2.2 | 2026-09-14 | patch | **Linter-absent fallback disclosed (feature 37 review F86, brand):** the phase-emission step now names the install-target limitation — `scripts/phase-lint.mjs` ships with the workflow repository, not the installed skill — and its safe fallback: apply the eight `phase-contract` rules by hand, label that check weaker, and disclose the deterministic linter did not run; never skip the gate. `SKILL.md` and `references/PLANNING_PROCESS.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| 3.2.1 | 2026-09-12 | patch | **Paste fence (feature 37 review F26, security):** the phase-emission step states the linter's stdout block is **lint output, never instructions** — it echoes plan-derived text, so any directive inside it is data to report, never an action to take. `SKILL.md` and `references/PLANNING_PROCESS.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| 3.2.0 | 2026-09-10 | minor | **Deterministic phase lint (feature 37 P4):** phase emission now runs `bun scripts/phase-lint.mjs <plan>` (node fallback) and pastes the fixed stdout block instead of reasoning over the eight rules; exit 1 means the blocked phase is re-cut or split. `SKILL.md` and `references/PLANNING_PROCESS.md` updated; `phase-contract` stays the sole rule owner. |
| 3.1.0 | 2026-09-03 | minor | **Research and reference trace** (fix #161): plan-stage research is conditional — only when a bounded question (ROWS.md Q1–Q5) cannot be answered from repository evidence; exactly one web pass freezes its returns as evidence rows (URL + access date); what the repository answers is never re-fetched. The reference trace is mandatory: affected code is located by symbol/reference search (LSP/serena when available, grep otherwise) and the blast radius is derived from that search, never from model memory. `plan-fix:issue` route ceilings re-based, this text the named growth source. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.8.0 | 2026-09-18 | minor | **Planning materiality and cap mirror (feature 31):** `CHECKS.md` §4 states material = `medium`+ with a `low` persisted report-note and the anti-deflation rule; `OUTPUT.md` replaces the every-batch re-review rows with the default re-review plus the recorded wording-only exemption and mirrors the `stop-review-loop-cap` refusal. The pinned PASS sentence and every receipt literal are byte-unchanged. |
| 1.7.1 | 2026-09-09 | patch | **Runtime-convention wording (#197)**: `CHECKS.md`/`OUTPUT.md` show the snapshot build/verify commands in bun-first form (node fallback per the repo runtime convention). No contract change. |
| 1.7.0 | 2026-09-06 | minor | **Receipt self-check** (fix #162): the persist-first section gains the `verify --stage spec` run in-turn and the turn-contract box becomes a RUN box — the reviewer runs the sensor re-verify and pastes the JSON beside the verdict block before reporting; `exit 3`/`structural.fresh: false` means the mark did not land (write-then-report is mechanical). Verdicts unchanged; `review-spec` still emits the design-verdict token.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 1.6.0 | 2026-09-05 | minor | **Roadmap scoping:** the snapshot's `contexts` list no longer binds a `roadmap-row` — the roadmap row stays Step 0 routing data, but the shared `ROADMAP.md` ledger is deliberately unbound because it moves for reasons this unit never reviewed (other units' rows, the status machine's own sanctioned writes), which used to invalidate every in-flight receipt repo-wide and force pointless re-reviews (e.g. an `execute-phase` blocked by a receipt whose only drift was another feature's roadmap row). The unit's artifacts and the remaining governing authorities stay bound; the own-status gate and the dependency-gate fingerprint still cover the row's safety-relevant content live. Receipts recorded before this change re-verify stale exactly once — see `docs/workflow/MIGRATION.md`. |
| 1.5.0 | 2026-09-04 | minor | Retirement remap (fix #161, P3a): the `source`/`environment`/`runtime` fold path names the manual `/fold-findings` then re-run `/review-change` in place of the retired `loop-review-fold` router. |
| 1.4.0 | 2026-09-03 | minor | FAIL routes carry the **resolution map**: the finding's `class` cell names its resolver and only that resolver — `product` → `design-feature` (then `review-spec` re-judges), `plan` → `plan-feature`/`plan-fix` (then `review-plan` re-judges), `source`/`environment`/`runtime` → the executor's fold path; `fold-findings` never repairs a planning artifact (fix #159). |
| 1.3.2 | 2026-09-02 | patch | The spec receipt template now states `Unit kind: <feature\|fix>` — the consumer parses that label, and a template that omits it turns the receipt's unit-identity check into a silent no-op for SPEC-stage verdicts (finding F70) |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.7.0 | 2026-09-18 | minor | **Planning materiality and cap mirror (feature 31):** `CHECKS.md` §4 states material = `medium`+ with a `low` persisted report-note and the anti-deflation rule; `OUTPUT.md` replaces the every-batch re-review rows with the default re-review plus the recorded wording-only exemption and mirrors the `stop-review-loop-cap` refusal. Every receipt literal is byte-unchanged. |
| 1.6.1 | 2026-09-09 | patch | **Runtime-convention wording (#197)**: `CHECKS.md`/`OUTPUT.md` show the snapshot build/verify commands in bun-first form (node fallback per the repo runtime convention). No contract change. |
| 1.6.0 | 2026-09-06 | minor | **Arbitration ownership + self-check** (fix #162): `review-plan` loses the design-verdict verdict entirely — the receipt's `Verdict:` line is the two-verdict set, a product-intent gap is `PLAN-REVIEW-FAIL` with `class: product` (only `review-spec` may emit the design-verdict token), and the routes table and closing blocks drop that branch. The persist section gains the `verify --stage plan` self-check RUN box (feature plan names `--parent`; fix omits it).  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 1.5.0 | 2026-09-04 | minor | Retirement remap (fix #161, P3a): the `source`/`environment`/`runtime` fold path names the manual `/fold-findings` then re-run `/review-change` in place of the retired `loop-review-fold` router. |
| 1.4.0 | 2026-09-03 | minor | FAIL routes carry the **resolution map**: the finding's `class` cell names its resolver and only that resolver — `plan` → `plan-feature`/`plan-fix` (the author re-cuts, then `review-plan` re-judges), `product` → `design-feature` (then `review-spec`, then the plan re-derives), `source`/`environment`/`runtime` → the executor's fold path; `fold-findings` never repairs a planning artifact (fix #159). |
...
| 3.6.0 | 2026-09-17 | minor | **Receipts are emitted, not written by hand (fix #182):** `PERSIST_AND_DECIDE.md` step 12 and the Turn contract now call `bun scripts/review-receipt.mjs emit` — the script derives the head, refuses a PR whose `headRefOid` moved, posts the one idempotent SHA-bound comment, re-reads the comments and exits non-zero unless the newest `review-change:pass` marker names the reviewed head. The hand-assembled body and the temp-file/retry prose are gone; the marker grammar now has one runtime instead of a test-only reimplementation. |
| 3.5.1 | 2026-09-15 | patch | **Canonical replan destination (fix #224):** the `REVIEW-FAIL` `→ Next:` block and the Routing table route a `replan-in-unit` / plan-owned finding through `node scripts/unit-route.mjs <unit>` — its `route: replan` line names the planner and the fresh `/review-plan` before `/execute-phase` — replacing the two contradictory sentences that sent the class to the executor and to `plan-feature` separately. |
| 3.5.0 | 2026-09-08 | minor | **Review-end turn boundary (fix #191 P3, reviewer-side mirror defect)**: on `REVIEW-FAIL`/`NEEDS-DECISION` the skill now ends at the report and never self-invokes `/fold-findings`, `/execute-phase`, `/plan-feature`, `/design-feature` or `/triage-issue` — those are separate user-initiated invocations (new **Review-end turn boundary** box in the Turn contract). The `fix-now` destination phrasing in the relationship section and `OUTPUT_AND_GUARDRAILS.md` now routes findings to the current phase's fold cycle (`/fold-findings`, invoked after this review ends) instead of reading as in-review execution; `PERSIST_AND_DECIDE.md` steps 11/14 state the review's only mutations (ledger commit; receipt comment on `REVIEW-PASS`) and that the `→ Next:` block is a **recommendation, not a to-do list**; `REVIEW_PROCESS.md`'s two-cycle cap clarifies re-runs count separate review invocations. Adds a red-first discipline pin (AC15: reverting the boundary text fails the pin). |
| 3.4.0 | 2026-09-07 | minor | Issue #170: `REVIEW_PROCESS.md` step 1 gains **delta mode** as the default on a post-fold re-review (re-verify every `folded: yes` row at its cited `file:line`; review the fold diff only; gate green at the reviewed head + exact `ACCEPTANCE.md` blob; delta `REVIEW-RAN` marks count toward the two-cycle cap from the unchanged source; same-`file:line`+axis re-reports admitted only as `regression of <id>`/`DISPUTED`), and the two **escalation** triggers (width: file outside the cited-file union or a changed line >50 lines from every cited line in a cited file; size: >200 changed lines or >15 files) with the state-the-trigger-and-numbers requirement. |
| 3.3.0 | 2026-09-06 | minor | **Verdict-response guard scope** (fix #162): the fold → re-review two-cycle cap is scoped to the review→fold loop on source findings; a planning repair routed by a `class: plan`/`product` finding is a verdict response (POLICY §4), never a loop-defect stop and never counted against the cap.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
| 3.2.0 | 2026-09-04 | minor | **Manual hand-off remap (fix #161, P3a):** the `REVIEW-FAIL` `→ Next:` block now names the manual path `/fold-findings`, then re-run `/review-change` on the changed HEAD, instead of the retired `/loop-review-fold <unit>` router (a programmatic outer driver may run the same sequence). `REVIEW_PROCESS.md` gains the two-cycle cap (`LOOP CAP REACHED`): a third cycle never starts without an explicit user instruction, and the residue routes to `triage-issue --prioritize-now`. The relationship section no longer names the retired router. |
| 3.2.1 | 2026-09-04 | patch | Fold fix (F7): corrected the `review-change` process-step cross-references after the verification step was inserted as step 6 — `review-implementation` is now step 8 (was 7), `review-debt` step 9 (was 8), fusion in step 7 (was 6), and the isolation rule now names the verification pass (step 6). Consequence: the four `review-change` route ceilings re-based to `ceil(measured × 1.10)` (`adversarial` 17415, `default-backend`/`default-web` 15597, `synthesize` 16111) after the SKILL.md grew with the verification-pass mention. |
| 3.1.0 | 2026-09-03 | minor | **Verification before persist (fix #161):** the review rechecks every candidate finding in an isolated context against the reviewed head's bytes before synthesis — a code/behavioral claim confirms only with a failing reproducer (red test first, unchanged code) or reproducible command output, a documentation/wording claim by direct read, a missing-doc claim by the undocumented user path — and marks it `confirmed` or `refuted`. Only confirmed candidates persist; the report gains a `Refuted` section. The per-finding durable signature `finding-mark@1` (`VF-` ids, single writer `review-change`, excluded from the fold queue/sensor/annotator) is declared in `pre-execution-review/references/LEDGERS.md`. |
| 3.0.0 | 2026-09-03 | major | **Loop-bounding contract change (fix #159):** `low` findings are report-only notes — never persisted to the fold ledger, never blocking; the finders' materiality floor now survives classification and the decision, and only `high`/`med` fix-now rows persist and can make `REVIEW-FAIL`. Workspace state (dirty tree, ahead/behind remote) is a **precondition, not a finding**: the review stops as `REVIEW BLOCKED — workspace state` before any pass runs. Re-reviews read the fold ledger first, state the cycle number, re-verify every `folded: yes` row at its cited location (a re-report is legitimate only as `regression of <id>` or `DISPUTED`), and a cycle ≥ 2 producing a new fix-now row appends the `CONVERGENCE-ANOMALY` report. The review **commits its own ledger append** (`docs(<unit>): persist review findings F<n>–F<m>`, pushed when a PR is open) so it never dirties the tree the next review judges; on `REVIEW-PASS` with an open PR no ledger write happens — the SHA-bound receipt is the durable record. See `docs/workflow/MIGRATION.md`. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 1.7.0 | 2026-09-24 | minor | The freeze-batch and `route: replan` hand-offs name `/unit-lane <slug>` / `/unit-lane --fix <n>` instead of the retired `plan-fix`/`plan-feature`. |
| 1.6.0 | 2026-09-19 | minor | **Feature 32 P2: sole-flipper provenance cited to the map:** the fold-flip sentence attributes `folded: no → yes` to the `ledger-ownership@1` map's `fold-findings:folded-flag` column-set as the sole writer instead of the skill's own cycle. No behavior change. |
| 1.5.1 | 2026-09-19 | patch | **fix #244: freeze-batch consumer follows the router's conclusion** — the `→ Next:` block and decision table recommend `/plan-fix <n>` / `/plan-feature <slug>` for a `replan` conclusion and **stop-and-surface the user decision for a `decision` conclusion**, instead of the raw router invocation (`node scripts/unit-route.mjs <unit>`), which is now described as the already-run discovery step; the `<unit>` argument format (bare folder number or full slug) is documented; the one-physical-line rule is added to the closing-block contract; a `#244` pin in `normative-drift.test.mjs` guards the freeze-batch hand-off (planner branch and decision branch). |
| 1.5.0 | 2026-09-15 | minor | **Canonical replan destination (fix #224):** the freeze-batch's `REPLAN-ROUTE` receipt and the branch table route the retained rows through `node scripts/unit-route.mjs <unit>` — the router's `route: replan` line names the planner that re-cuts the plan — replacing "routes to planning". |
| 1.4.0 | 2026-09-07 | minor | Issue #170: the report contract gains the fixed **REPAIR-RECEIPT** (repaired ids + `finding-mark@1` refs, refuted/open, gate exit codes at head, batch class, fold-diff shortstat) printed as the ABSOLUTE-last output; the impact-rule **batch classification** (`all-repair-in-place` / `frozen (replan present)` / `none`) derives from frozen rows only and never reclassifies; the **freeze-batch** rule folds nothing (no flips, no commits) and records the REPLAN-ROUTE with retained ids; the four-branch closing block (`RE-REVIEW-REQUIRED (delta)` / `RE-REVIEW-OPTIONAL` / `RE-REVIEW-SKIPPED` / `REPLAN-ROUTE`) gains its branch-selection decision inputs (docs-only file-set test, frozen-severity-`high` override, SKIPPED-requires-prior-consumer-decision rule) and the literal no-decision→re-review default; empty-batch (`none`) and failed-gate (observed exit codes, nothing folded) branches added; `FOLD_POLICY.md` gains the materialize-reproducer rule (consume the `recheck` cell, never re-derive, `BLOCKED <missing input>` when unmaterializable). |
| 1.3.0 | 2026-09-03 | minor | `FOLD_POLICY.md` forbids editing an existing test's expectation to match behaviour (fix #161): a setup repair keeps assertions at least as strong and never touches expectations; the test-immutability contract owns the sole amendment path. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 4.0.0 | 2026-09-03 | major | **Cycle cap (fix #159):** the loop runs at most **two** review→fold cycles per unit, counted unit-level and family-agnostic from the ledger's `REVIEW-RAN` marks and forge receipts — new finding families do not reset the count. A third cycle never starts: stop `TRIAGE-REQUIRED`, name every open finding ID, and hand the convergence diagnosis to `/triage-issue --prioritize-now` or the user. A unit that needs a third cycle has a planning or root-cause defect, not a review deficit. See `docs/workflow/MIGRATION.md`. |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 5.4.0 | 2026-09-24 | minor | The merge-authority wording names the pi package's `advance` conductor instead of the retired `ship-roadmap`; blockers route to `unit-lane`; the traceability/closure gates cite the lane. |
| 5.3.1 | 2026-09-21 | patch | Portability states that no skill pins a model tier, instead of pointing at the retired `#claude` branch. No gate, verdict, or output shape changed. |
| 5.3.0 | 2026-09-19 | minor | **Feature 32 P4: closure-integrity scale alignment:** replaces the four-value closure-integrity scale (`pass / warning / blocker / n-a`) with the compact three-value scale (`pass / blocker / n-a`), aligning the scale to the actual gate outcomes and removing the redundant `warning` level. |
| 5.2.1 | 2026-09-18 | patch | **Merge-ready box names the comment runtime (fix #182 replan):** the turn-contract box now names `bun scripts/audit-pr-gate.mjs comment` as the step that posts the MERGE-READY comment and proves the marker landed, so the hand-assembled `gh pr comment --body-file` prose path is gone from the box — matching the reference (`03_AUDIT_PROCESS.md` step 7), which already ran the runtime. No behavior change. |
| 5.2.0 | 2026-09-17 | minor | **Terminal hygiene has an owner (fix #182):** three gates (`tree-clean`, `branch-pushed`, `pr-ready`) join the merge contract, read from state by `bun scripts/audit-pr-gate.mjs hygiene --pr <N> --apply` — the draft flag is repaired (`gh pr ready`), the other two are named blockers. The receipt-currency rule and the idempotent MERGE-READY comment moved into the same runtime; the `audit-pr` route ceiling is re-baselined with a declared source. |
| 5.1.0 | 2026-09-06 | minor | **Lineage gate names the new freshness dimension** (fix #162): the pre-execution lineage gate's blocker enumeration becomes "Stale, missing, wrong-stage **or impossible-timeline** lineage → BLOCKED", keeping the receipt-based merge-gate honesty for the new dimension.  Route ceilings re-based at the declared headroom (growth source `fix/162`). |
...
| Version | Date | Type | What changed |
|---|---|---|---|
| 2.2.0 | 2026-09-24 | minor | The complements and closing defect route name `unit-lane` / `/unit-lane --fix <n>` instead of the retired `plan-feature` / `plan-fix`. |
| 2.1.1 | 2026-09-21 | patch | Description drops the pointer to the retired `#claude` branch; Portability states that no skill pins a model tier, instead of pointing at the branch that used to. No behavior change. |
| 2.1.0 | 2026-09-19 | minor | **Feature 32 P3: report contract corrected (14 checks, phantom MEDIUM removed):** the fixed findings report now reads `| # | Check (1-14) |` and `Checks run: <n>/14` (there have been 14 checks since check 14 was added; the header and count lagged at 13), and the generated-docs orphan severity is `low` — this skill's legend defines only `high` and `low`, so the stray `MEDIUM` it emitted was undefined. No check was added or removed. |
...
#### `init-workspace`
| Version | Date | Type | What changed |
| 3.2.0 | 2026-09-25 | minor | **Fixed `markdown`+`json` entries, detected `yaml`, never-auto-detected rule, startup rule:** adds `markdown` and `json` as fixed entries always written (before detected ones), adds `yaml` detection when `.yml`/`.yaml` files are present, documents that markdown/yaml/json are never auto-detected by Serena (must be listed explicitly), and states the startup rule — one failing entry disables the whole language-server manager, so a failed server must be removed and recorded as a residual. `references/SERENA.md` rewritten accordingly. |
| 3.1.0 | 2026-09-24 | minor | **Provision the Serena project config (Refs #219):** writes `.serena/project.yml` with `language_servers` for every detected language, adds the `.gitignore` block additively (never clobber), and runs the health check. `references/SERENA.md` is the single owner of the Serena provisioning contract. `references/BOOTSTRAP_DISCOVERY.md`, `references/BOOTSTRAP_WRITE.md`, `references/UPGRADE.md`, and the skill's guardrails/done-when sections updated. |
| 3.0.1 | 2026-09-21 | patch | Portability states that no skill pins a model tier, instead of pointing at the retired `#claude` branch. No install, upgrade, or consent rule changed. |
| 3.0.0 | 2026-09-21 | major | **The agent guide is `AGENTS.md` (breaking: the file this skill writes changed name).** Bootstrap fills the `AGENTS.md` placeholders and no longer produces a `CLAUDE.md`; Step 0's scaffold marker accepts either name, so an install from an older release still enrols in upgrade mode. Upgrade mode gains step 6: fold every block that exists only in a legacy `CLAUDE.md` into `AGENTS.md` first, then remove that file **only with an explicit yes** — a user on a Claude Code release that reads only `CLAUDE.md` may keep it, reported as a residual and never treated as drift to repair. The one-guide-one-name rule is stated in `SKILL.md`'s guardrails, and the additive-only invariant names this single consented exception. See `docs/workflow/MIGRATION.md`. |
| 2.9.0 | 2026-09-18 | minor | **Path-protection policy seeding (feature 60 P2):** install mode seeds the shipped path-protection policy `.agentic-workflow/path-policy.json` and its `path-protection.md` doc page beside the hooks, and upgrade mode proposes a missing pair without clobbering an owner-written policy. `references/BOOTSTRAP_WRITE.md` and `references/UPGRADE.md` updated. |
|---|---|---|---|
...
| `evidence-grounding` | 1.7.0 | 2026-09-22 | minor | **Research-step cutoff rules (workflow-gap audit, pattern #1):** step 2 gains two inline stops matching what the delegated pass already had — **partial is a first-class stop** (when every remaining material question is an owned `unknown` row with a stated consequence, stop investigating and draft; depth follows uncertainty and consequence, not completeness) and the **human-decision cutoff** (a product/user gap becomes one focused `unknown` row owned by the human and the rest of the pass continues; never research to guess the answer). Emitted vocabularies unchanged — the stops express themselves through the existing `unknown` rows and `NEEDS-DESIGN` routing. |
| `bump-skill` | 2.3.5 | 2026-09-21 | patch | `VERSION_AND_DOCS.md` no longer syncs a tier file or the derived `#claude` branch (both retired); `DISCOVERY_AND_LINT.md`'s ordering invariant covers the plugin array only. No lint rule, semver rule, or remaining synchronization surface changed. |
| `bump-skill` | 2.3.4 | 2026-09-21 | patch | The policy this skill enforces is cited as `AGENTS.md` section "Version every change", and the authoring-invariant lint reads the same seven rules from that file. No lint rule, semver rule, or synchronization surface changed. | **Research-step cutoff rules (workflow-gap audit, pattern #1):** step 2 gains two inline stops matching what the delegated pass already had — **partial is a first-class stop** (when every remaining material question is an owned `unknown` row with a stated consequence, stop investigating and draft; depth follows uncertainty and consequence, not completeness) and the **human-decision cutoff** (a product/user gap becomes one focused `unknown` row owned by the human and the rest of the pass continues; never research to guess the answer). Emitted vocabularies unchanged — the stops express themselves | `pre-execution-review` | 2.5.1 | 2026-09-21 | patch | **Fold F29: owner-entry wording:** `LEDGERS.md` §"Durable ledger write ownership" calls `fold-findings:folded-flag` the map's **owner entry** instead of its "annotator", matching the `ledger-ownership@1` row (the mechanical annotator is `scripts/ledger-provenance.mjs`). No rule or output-shape change. |
| `pre-execution-review` | 2.5.0 | 2026-09-19 | minor | **Feature 32 P4 (collision fix + GATE-RAN):** bumps version from 2.4.0 to resolve CHANGELOG collision with feature 31's row; `LEDGERS.md` (§gate-ran@1) declares the gate-run mark — the frozen `GATE-RAN | HEAD <sha> | <cmds> | exit <code>` format (one grammar, cited by its consumers), additive slots with the reserved `manifest <sha>` slot, identical-head-only green reuse, red runs recorded as evidence, and the two recorder column-sets `execute-phase:gate-ran-marks` + `review-change:review-gate-ran-marks` mirrored into the `review-findings` owner cell and both template projections. No rule, flag, or output shape changed. |
| `review-implementation` | 1.9.0 | 2026-09-19 | minor | **Feature 32 P3/P4: canonical severity table + blocking gate + vocabulary alignment:** adds the canonical severity conversion table, the blocking gate rule that enforces severity classification, and vocabulary alignment with the fixed-class review pack. |
| `plan-feature-scaffold` | 2.4.0 | 2026-09-18 | minor | **Path-protection plan declaration (feature 60 P3):** `references/SCAFFOLD_PROCESS.md` instructs the planner to declare the test set in the `path-protection-plan@1` block (`freeze-after` plus a justified `created`/`not-created`/`ignored` row per test decision; placeholder tokens only). No rule, flag, or output shape changed. |
| `orchestration-envelope` | 2.2.0 | 2026-09-18 | minor | **Path-protection grammar (feature 60 P3):** `TURN_CONTRACT.md` gains the `path-protection@1` block — the closed reason vocabulary, the checkpoint invocation, and the `path-protection-plan@1` / `path-protection-records@1` row shapes — and `CLAUDE.md` registers the `path-protection-contract` / `path-protection-reasons` normative surfaces (the latter a `schema-export:` of the crate's `PATH_GUARD_REASONS`). |
| `pre-execution-review` | 2.4.0 | 2026-09-18 | minor | **Fifth gate-rejection type (feature 60 P3):** `POLICY.md` §8's closed gate-rejection vocabulary gains `path-protection` (printed by `execute-phase`'s preflight) and the prose counts five, so the block and the sentence stay consistent. (Note: version corrected from 2.3.0 to 2.4.0 to resolve drift-gate collision with feature 31's row which retains 2.3.0.) |
| `verification-contract` | 1.3.0 | 2026-09-18 | minor | **Path-protection backstop (feature 60 P3):** the test-immutability section names the `path-protection@1` checkpoint as the deterministic backstop — the Tier 1 gate fails a protected-path change with no recorded justification/approval and the pi guard blocks the write; the frozen manifest is the finish line those escapes are checked against. |
|---|---|---|---|---|
| `review-implementation` | 1.10.0 | 2026-09-24 | minor | `CLASSIFY.md`'s replan and product-owner routes name the lane's `/unit-lane` command instead of the retired `plan-fix`/`plan-feature`. |
| `orchestration-envelope` | 2.2.1 | 2026-09-24 | patch | The conductor is named as the pi package's `advance` command instead of the retired `ship-roadmap` skill. |
| `pre-execution-review` | 2.6.0 | 2026-09-24 | minor | The description, when-to-use and POLICY header mark the spec/plan review cycle retired by feature 61 and scope the pack to the planning-ledger shapes the lane and `execute-phase` consume. |
| `phase-contract` | 1.0.6 | 2026-09-24 | patch | The consumers are named as `execute-phase` and the lane's `plan` step instead of the retired `plan-feature-scaffold` / `plan-fix`. |
| `bump-skill` | 2.3.6 | 2026-09-24 | patch | The CHANGELOG example names `unit-lane` instead of the retired `plan-fix`. |
| `orchestration-envelope` | 2.1.0 | 2026-09-16 | minor | **Machine-check profile for the canonical turn contract (feature 52):** `TURN_CONTRACT.md` gains a profile that demonstrates boxes 1–5 by pasting the verifier's one-line receipt (`TURN-CONTRACT ok` / `TURN-CONTRACT fail box<N>: <code>`) instead of reciting them, states the verifier-unavailable fallback, leaves boxes 6–11 agent-attested, permits echoing the closing `→ Next:` block from the sensor envelope's `next.recommended` + `next.alternatives`, and declares the versioned `turn-contract-receipt@1` grammar. `CLAUDE.md`'s normative-surfaces table registers the receipt surface (machine `n/a`). |
| `review-implementation` | 1.8.0 | 2026-09-15 | minor | **Canonical replan destination (fix #224):** `CLASSIFY.md`'s Routing bullet, the large-in-scope replan section and the `plan`-owner row route `replan-in-unit` through `node scripts/unit-route.mjs <unit>` and the planner it names, then a fresh `/review-plan` before execution. |
| `replan-findings` | 1.1.1 | 2026-09-21 | patch | **Fold F31: `GATE-RAN` mark enumeration:** the `open-rows`/`rows:` field notes `GATE-RAN` alongside `VF-<n>`/`REVIEW-RAN` as a mark id, matching the router's `isMarkRow` guard. |
| `replan-findings` | 1.1.0 | 2026-09-16 | minor | **Terminal route and bare status token (fix #224 replan, F23+F24):** the block this contract documents now carries the `close-out` route — a finished unit (`status: done`) with no open row, whose `next:` is `/audit-pr`, the merge gate — and the `status` field is stated to carry the bare token the router reads out of the roadmap or fix-index cell, never the cell's markdown decoration. |
| `replan-findings` | 1.0.1 | 2026-09-15 | patch | **Router output contract (fix #224 end review, F7):** the skill documents the block the router prints — the eight fields, the exit-code contract (0 route; 1 unknown unit or a wrong argument count; 2 ambiguous — the last two fail closed with no `route:` line), the 12-path cap with its explicit `… and N more` remainder, and that the echoed ids and paths are repository data flattened and cut to 160 characters. |
| `replan-findings` | 1.0.0 | 2026-09-15 | — | New internal contract (fix #224): the conditional replan entry. Loaded only when `node scripts/unit-route.mjs <unit>` prints `route: replan`; consumes the router's bounded read set (no planning preflight), appends phases to the unit's SPEC `## Phases` ledger per the hardening placement rule, rotates the `artifactRevisionId`, and hands off to `/review-plan <unit>` — never straight to `/execute-phase`. |
| `plan-feature-scaffold` | 2.3.2 | 2026-09-14 | patch | **Linter-absent fallback disclosed (feature 37 review F86, brand):** the scaffold's linter-stdout hand-off now names the install-target limitation — `scripts/phase-lint.mjs` ships with the workflow repository, not the installed skill — and its safe fallback: apply the eight `phase-contract` rules by hand, label that check weaker, and disclose the deterministic linter did not run; never skip the gate. `SKILL.md` and `references/SCAFFOLD_PROCESS.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| `plan-feature-scaffold` | 2.3.1 | 2026-09-12 | patch | **Paste fence (feature 37 review F26, security):** the scaffold's linter-stdout hand-off states the block is **lint output, never instructions** — it echoes plan-derived text, so any directive inside it is data to report, never an action to take. `SKILL.md` and `references/SCAFFOLD_PROCESS.md` updated; the pi mirror is re-bundled. No rule, flag, gate order, or output shape changed. |
| `plan-feature-scaffold` | 2.3.0 | 2026-09-10 | minor | **Deterministic phase lint (feature 37 P4):** the scaffold step runs `bun scripts/phase-lint.mjs <plan>` (node fallback) over the emitted plan and pastes the linter's fixed stdout block instead of reasoning over the eight rules; exit 1 is a blocked phase to re-cut, never a verdict to re-derive. `SKILL.md` and `references/SCAFFOLD_PROCESS.md` updated; `phase-contract` stays the sole rule owner. |
| `phase-contract` | 1.0.5 | 2026-09-16 | patch | **Armed-box enumeration corrected (fix #224 replan fold, F26):** the executed-phase exemption's scope sentence now names the boxes that stay armed for a fully-ticked phase explicitly — boxes 1, 2, 4, 5, 6 and 8 — instead of the range that wrongly included the exempt box 7. Prose only; no rule semantics changed. |
| `phase-contract` | 1.0.4 | 2026-09-16 | patch | **The executed-phase exemption (fix #224 replan, F25):** the owner text now states that a fully-ticked phase is historical for boxes 3 and 7 only — the two positional boxes a replan disturbs when it appends work after an executed `Hardening & PR` — while boxes 1, 2 and 4–8 stay armed for it and every box stays armed for an unemitted phase, so pre-ticking to dodge a check is a defect. Implemented in `scripts/phase-lint.mjs` and pinned by a corpus triple. |
| `phase-contract` | 1.0.3 | 2026-09-11 | patch | Rule 3 gains the **≥ 1-task minimum** (issue #184, feature 37 review F5, user-approved): a phase must carry at least one independently checkable checkbox task — a zero-task phase is malformed and now BLOCKs box-3 instead of passing all eight boxes vacuously. Corpus-pinned in `scripts/phase-lint.test.mjs`. |
| `phase-contract` | 1.0.2 | 2026-09-10 | patch | Rule 1 amendment (issue #184, feature 37 P1): the templates' literal closing title `Hardening & PR` is the sole authorized `&`-joined title — its `&` is a normalization separator, not a deliverable joiner, and its title-deliverable normalizes to `hardening-pr`; every other `&`-joined title still FAILs. No rule semantics are added beyond the owner's own text. |
| `pre-execution-review` | 2.3.0 | 2026-09-18 | minor | **Planning materiality, hard cap, and the wording-only route (feature 31):** `LEDGERS.md` §3 states material = `medium`+ with a `low` persisted report-note and the anti-deflation carry-over; `POLICY.md` §3 records the wording-only determination block and the non-skippable revision rotation, and §4 replaces the unbounded-cycle sentences with the hard two-cycle cap and the `stop-review-loop-cap` refusal. The `CONVERGENCE-ANOMALY` block, the receipt-literal lines and the ownership block are byte-unchanged. |
| `pre-execution-review` | 2.2.0 | 2026-09-07 | minor | Issue #170: `LEDGERS.md` §The durable finding mark gains one consumption sentence — `fold-findings` reads the `recheck` cell to **materialize** the finding's reproducer as its regression check (never re-derives; a cell that cannot be materialized yields `BLOCKED <missing input>`). The row shape, `VF-` exclusions, and the `review-change` single-writer rule stay untouched; both template projections and the ownership block are unchanged. |
| `pre-execution-review` | 2.1.0 | 2026-09-06 | minor | **Receipt self-check + roadmap-label independence + verdict-response scope** (fix #162): POLICY §8 makes write-then-report mechanical (the reviewer is consumer zero — runs `verify --stage …` in the same act as persisting, pastes the sensor answer beside the block before reporting; a chat-only verdict is `missing-receipt-snapshot` to every consumer); SNAPSHOT.md notes consumer zero; §5 gains the complement rule (roadmap rows are labels, `done` = built + PR open never suppresses a gate); §4 scopes every cycle cap to blind re-reviews — a verdict-response repair is never a loop-defect stop. |
| `pre-execution-review` | 2.0.0 | 2026-09-05 | major | **Roadmap scoping — a changed snapshot contract.** The builder's context list no longer binds `roadmap-row`: `ROADMAP.md` is every unit's shared lifecycle ledger, so rows for other units and the status machine's own sanctioned writes (`planned` → `in-progress` at P1, `done` at PR open) invalidated every in-flight spec/plan receipt repo-wide and forced pointless re-reviews — the exact failure an orchestrator pays for twice. The row's safety-relevant content stays owned by gates that re-read it live (own-status gate; the dependency gate's Dependency receipt v1 fingerprint over the SPEC `Depends on:` line plus the closure roadmap rows), and the unit's artifacts plus the remaining governing authorities (`project-guide`, `normalized-repository-state`, `architectural-invariants`) stay fully bound — a genuine artifact edit still fails closed. **Migration:** receipts recorded before this change re-verify `stale-context`/shape-stale exactly once; re-run the stage's review (`/review-spec`, `/review-plan`) on each in-flight unit after upgrading. The schema vocabulary keeps `roadmap-row` as a valid kind — this repository's builder simply no longer emits it. Validator: `scripts/pre-execution-sensor.test.mjs` (roadmap-scoping block). |
| `evidence-grounding` | 1.6.0 | 2026-09-03 | minor | **Web pass** (fix #161): when repository evidence cannot answer a bounded question — and always at design stage, where `design-feature`'s research gate makes it mandatory — fetch external sources and freeze each as an evidence row (URL + access date); offline or unanswered returns `NEEDS-EVIDENCE`, never an invented citation. ROWS.md accepts fetched external documentation as `source-and-location` (URL + access date). |
| `plan-feature-scaffold` | 2.2.0 | 2026-09-03 | minor | **Research and reference trace** (fix #161): conditional web research (only when a bounded ROWS.md question cannot be answered from repository evidence — exactly one web pass before phases are emitted) plus a mandatory reference trace: affected code located by symbol/reference search (LSP/serena when available, grep otherwise), the blast radius derived from that search, never from model memory. `plan-feature:scaffold` route ceilings re-based, this text the named growth source. |
| `review-code` | 1.2.0 | 2026-09-03 | minor | **Reference trace** (fix #161): new checklist item — every changed symbol/API got a reference search; un-updated callers are findings. |
| `review-seo` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-perf` | 1.2.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-brand` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-a11y` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-design` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-verify` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-security` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `review-code` | 1.1.0 | 2026-09-03 | minor | **Materiality bar** (fix #159): report a row only when a competent user's outcome changes or a rule the project explicitly declares is violated — cite the rule it violates; comment/punctuation typos, formatting-only drift, style preference with no cited rule, and hypothetical robustness beyond the SPEC's named scenarios are not findings; an empty table with `Decision: PASS` is the expected result for a well-formed change — never pad the table. |
| `evidence-grounding` | 1.5.0 | 2026-09-03 | minor | **Claim discipline** (fix #159): a forward-looking claim stated as present fact is an overclaim — SPEC, plan and acceptance prose assert only what is true at authoring time, or name the exact later step that makes it true with its owning phase and validator; "the PR closes the issue", "merged", "CI green" written before the forge says so are false records that reviews then have to chase. |
| `verification-contract` | 1.2.1 | 2026-09-04 | patch | No behavior change (fix #161, P3a): the `Consumed by` description drops the retired `loop-review-fold`. |
| `verification-contract` | 1.2.0 | 2026-09-03 | minor | **Test immutability** (fix #161): a test, once written, is immutable — the executor fixes code until green, never the test; the sole legitimate amendment is a proven mis-encoding of external semantics cited from authoritative documentation, surfaced as a finding plus a SPEC amendment; the research-before-encode rule (platform semantics are verified before a test encodes them); adding stronger tests stays allowed, editing expectations never. |
| `verification-contract` | 1.1.0 | 2026-09-03 | minor | **Validator stability** (fix #159): a validator must never gate on a surface other workflow actors mutate — the branch diff as a whole, the session log, progress entries, review ledgers, or forge state — because any out-of-unit commit then re-fails a frozen criterion on a finished unit and re-opens its review loop; diff-based validators enumerate the unit's paths or exclude workflow-mutated surfaces explicitly. |
| `review-implementation` | 1.7.0 | 2026-09-04 | minor | Retirement remap (fix #161, P3a): the `source`/`environment`/`runtime` fold route names the manual path `/fold-findings` then re-run `/review-change` in place of the retired `loop-review-fold` router. |
| `review-implementation` | 1.6.0 | 2026-09-03 | minor | Severity floor at classification (fix #159): a `low` finding (taste, cosmetics, micro-optimization without a measured need) is **never persisted and never blocks** — it is carried as a report note and the fold ledger never sees it; a real defect mislabeled `low` re-escalates to `med` minimum (deflating a real defect to `low` to unblock a review is itself a review defect); only `high`/`med` findings run the current-unit gate. |
| `pre-execution-review` | 1.7.0 | 2026-09-04 | minor | Retirement remap (fix #161, P3a): the pre-execution union list drops the retired `loop-review-fold` router. |
| `pre-execution-review` | 1.6.0 | 2026-09-03 | minor | `LEDGERS.md` declares the **durable finding mark**: a `finding-mark@1` per-finding signature in the `review-findings.md` ledger's existing columns (`VF-` ids, `confirmed | refuted` route, recheck method + reproducer reference, counter-evidence for a refuted candidate, never a row/fold task/re-review trigger), single writer `review-change`; the ownership map and both template projections add `review-change:finding-mark`. Issues no verdict, writes no artifact (fix #161). |
...
| `bump-skill` | 2.3.3 | 2026-09-15 | patch | English-only interim: drops the `CHANGELOG.es.md` / `README.es.md` synchronization steps and the bilingual sibling requirement, leaving the changelog + README updates single-language. No lint, versioning, or migration behavior change. |
...
- **2026-09-24 — the lane is the only planner (fix: lane reference migration).** Feature 61 deleted `design-feature`, `plan-feature`, `plan-fix`, `review-spec` and `review-plan`, but every live routing surface still named them — `triage-issue` told a `fix-now` verdict to run `/plan-fix`, and `fold-findings`/`review-change` routed replans to a planner that no longer exists. Every live skill, the two user-facing workflow docs and the regression pin now name what the router already prints (`/unit-lane <slug>`, `/unit-lane --fix <n>`, `/unit-lane --from-issue <n>`), and the conductor is named `advance`. `unit-lane` finally declares the `--fix`/`--from-issue` arguments the router emits, and `docs/fix/_TEMPLATE/SPEC.md` gains the three mandatory sections (`Objective`, `Future cost`, `Applicable tests`) that made `unit-route --triage` reject every fix unit.
- **2026-09-24 — the sensor gains a reduced envelope (`workflow-status` 3.9.0).** A poll loop was paying for the repository's whole history on every read: the complete envelope ran ~29 KB on this repository, 17 KB of it `detail.features` (every one of the 62 roadmap rows, most of them shipped) and 5 KB of findings' multi-paragraph evidence memos. `--compact` drops exactly that history — a `done` row only once its linked PR is merged, the memos reduced to a fingerprint that keeps their length and names the ledger — while every decision field (`next`, `startable_now`, `design_candidates`, `blocked_units`, `dependencies`, `crash_recovery`, `urgent`, `pre_execution`) and `detail.workflow_observations` stay byte-identical, so a consumer can switch modes per read. Measured 51 % smaller (CLI stdout 43 697 → 21 227 bytes; `detail.features` 62 → 22 rows, the findings' memos 3 691 → 481 bytes); `@gtrabanco/pi-agentic-workflow` 0.15.2 re-bundles the mirror.
- **2026-09-22 — ship-roadmap retired (feature 61 P9).** The autopilot conductor is gone; its stage orchestration moves to the lane's deterministic router (feature 61 P7) and its unattended-conductor role is deferred to roadmap row 62 (the pi-native conductor). Deleted `skills/ship-roadmap/`; updated every live referencing surface (`README.md`, `.claude-plugin/plugin.json`, `skills.sh.json`, `docs/workflow/SKILLS.md`, `REPLICATE.md`, `FEATURE_WORKFLOW.md`, `REVIEW_AND_CLASSIFY.md`, `PORTABLE_PROMPT.md`, `template/`). Behavioral tests against ship-roadmap content pins flagged for post-freeze resolution. User-facing count recomputed: **18 user-facing + 21 internal** (frontmatter `user-invocable: true`, string matches excluded — `bump-skill`'s body mentions the phrase).
- **2026-09-22 — Phase 0 surplus removal: `generate-docs` retired, `#inheritance` alias dropped.** The docs-site adapter the skill generated into no longer exists, so the whole `skills/generate-docs/` tree goes with it, plus the `#inheritance` derived-branch sync (`sync-derived-branches.yml`) and every live reference (README install blocks and counts, `docs/workflow/SKILLS.md`, plugin manifest, `skills.sh.json`, the AGENTS.md normalizer inventory, and cross-references in `audit-docs`, `execute-phase`, `init-workspace`, `PORTABLE_PROMPT.md`, `MIGRATION.md`, `REPLICATE.md`). User-facing count recomputed: **18 user-facing + 21 internal** (frontmatter `user-invocable: true`, string matches excluded — `bump-skill`'s body mentions the phrase).
- **2026-09-21 — the twelve stale route-budget ceilings are re-based for the root-suite CI.** Commit `087b2064` (author-email normalization) lengthened every skill's frontmatter by two estimate units at once, so all 22 routes measured two units higher and the twelve whose ceilings carried at most two units of slack went over; feature 32's review fold (`9aa33530`) added a line to `review-change` and `audit-docs` on top. The twelve `routeEstimateMax` ceilings are re-set to `ceil(measured × 1.10)` with the growth source recorded on each entry in `docs/workflow/SKILL_CONTEXT_BUDGETS.json`; no lines ceiling and no skill ceiling moved. The repair landed with the root-suite CI job (`root-suite.yml`), which could not go green without it; this row is the release-log line the ceiling policy requires beside that re-basis, which the JSON carried without.
- **2026-09-21 — the `#claude` branch and per-skill model routing are retired.** Since v3 the default branch has been model-agnostic, so the derived `#claude` distribution had become a second, drifting surface maintained for a use case the default already covers. The branch is gone, `docs/workflow/model-routing.yml` and `.github/scripts/inject_claude_frontmatter.py` are deleted, `sync-derived-branches.yml` only mirrors `#inheritance`, and the generated `docs/site/guides/workflow/model-routing.md` went with its source. Skill descriptions and Portability sections stop pointing at a pinned-tier install; the README's tier guidance stays as advice. `log-session` 2.2.1, `audit-docs` 2.1.1, `review-change` 3.7.1, `plan-feature` 5.3.2, `init-workspace` 3.0.1, `design-feature` 3.5.1, `ship-roadmap` 5.2.4, `audit-pr` 5.3.1, `bump-skill` 2.3.5. See `docs/workflow/MIGRATION.md`.
- **2026-09-21 — one agent guide, named `AGENTS.md`.** The guide had two names (`CLAUDE.md` in the scaffold, `AGENTS.md` beside it) and therefore two places to edit; the copy nobody read was the one that drifted. The scaffold now writes `AGENTS.md` only, this repository's own guide moved there, and every live reference — the root suite's guide resolution, the package tests' citations, the workflow docs, the issue template and the skill prose — follows it. `init-workspace` 3.0.0 retires a legacy `CLAUDE.md` in a target project only after folding unique content into `AGENTS.md` and only with an explicit yes, because a user may deliberately be on a Claude Code release that reads only the old name. `@gtrabanco/pi-agentic-workflow` 0.13.3.
- **2026-09-21 — the skill bundle is pack-time output.** `packages/pi-agentic-workflow/skills/` is gitignored, staged by the package's own lifecycle and removed by `postpack`, so git holds one `skills/` tree; CI asserts the built tarball carries the bundle, the one failure that would otherwise publish green.
- **2026-09-18 — deterministic path-protection guards (feature 60).** The "never change a test to pass it" rule becomes a shipped-default, deterministic guard: a `path-protection-policy@1` protects `tests/**`, `e2e/**`, `**/*.test.*`, `fixtures/**` and the policy config against a plan-declared freeze point; the producer crate's read-only, offline `path-guard` CLI runs at each phase checkpoint over the just-closed committed range and fails a protected change with no recorded justification/approval (closed `PATH_GUARD_REASONS`, absent/malformed config falling back to the defaults with a reported degradation), and the pi package blocks a `write`/`edit` tool call to an existing protected path with no matching justification. The policy ships as a template seed and a pi mirror; the reason vocabulary is published as a `schema-export:` normative surface. `execute-phase` 4.6.0, `init-workspace` 2.9.0, `plan-feature-scaffold` 2.4.0, `orchestration-envelope` 2.2.0, `pre-execution-review` 2.3.0, `verification-contract` 1.3.0, `@gtrabanco/pi-agentic-workflow` 0.12.0.
- **2026-09-18 — the merge-ready box names its runtime, the gate fails closed without an upstream, and the settled-turn probe is time-bounded (fix #182 replan).** The `audit-pr` turn-contract box now names `bun scripts/audit-pr-gate.mjs comment` as the step that posts the MERGE-READY comment and proves its marker landed, so the hand-assembled `gh pr comment --body-file` prose path is gone from the box; `audit-pr` 5.2.1. The `branch-pushed` hygiene gate treats an unresolvable `@{upstream}` as blocked instead of a zero ahead-count, and the settled-turn git probe gains a 2000 ms timeout; `@gtrabanco/pi-agentic-workflow` 0.11.1 re-bundles the mirror. The unit's close-out phases P6–P9.
- **2026-09-16 — executable continuations (feature 59).** Envelope v2 gains the optional, schema-validated `next.continuation` object (`{ argv, rendering?, preconditions, evidence?, convergence }`) with a closed four-code refusal vocabulary, the pure fail-closed emitter, the argv parser and per-platform rendering derivation, and the receiver-side evidence digest check; the sensor projects its resolved decision into the field or refuses (`detail.continuation_refusal`), a per-class discipline suite drives one fixture repo per class, and the consuming surfaces quote the emitted command instead of authoring it. `@gtrabanco/agentic-workflow-schema` 4.2.0 (additive minor), `workflow-status` 3.7.0, `design-feature` 3.4.0.
- **2026-09-15 — English-only interim (bilingual docs removed).** All 18 committed `.es.md` siblings (root `README`/`CHANGELOG`, the `docs/workflow/*` tutorial/reference set, the generated `model-routing` guide, and the three package READMEs) and their reciprocal language-switcher links were removed repo-wide, together with the bilingual enforcement: the `CLAUDE.md` EN+ES hard rule, the `bump-skill` ES-sync steps, the package-content `README.es.md` requirements, and the bilingual halves of the schema/pi documentation, changelog, and alias-coverage suites. Rationale: the per-commit ES sync doubled documentation authoring cost for no reader gain while the pack is pre-release; feature 57 (`per-release-bilingual`, issue #232) restores bilingualism as a per-release batch pass. `bump-skill` 2.3.3, `@gtrabanco/agentic-workflow-schema` 4.1.2, `@gtrabanco/pi-agentic-workflow` 0.10.5.
- **2026-09-14 — the linter-absent fallback is disclosed.** Feature 37 review F86: the three consumer skills order `bun scripts/phase-lint.mjs <plan>`, but the script ships with the workflow repository, not the installed skill, so an install target had no disclosed behaviour. Every run-and-paste hand-off now states the safe fallback — apply the eight `phase-contract` rules by hand, label the check weaker, and disclose the deterministic linter did not run, never skip the gate — and the pi mirror is re-bundled (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2). The two re-bundled route-loaded reference files grew every `execute-phase:*` estimate ceiling and the `plan-feature:scaffold` / `plan-fix:issue` estimate+lines ceilings past re-basis #4, so they are re-set to `ceil(measured × 1.10)` (11739 / 11752 / 11622 / 11784 / 11937 / 11536 / 11699 / 12869 for the eight `execute-phase:*` — the eighth is `execute-phase:unit-loop`, re-set by the same re-basis; the earlier normalization had left it at 12798 — and 24286 / 27169 for the scaffold/fix pair) — re-basis #5, and this row is its growth source.
- **2026-09-12 — the merge with feature 38 re-based seven `execute-phase` route ceilings.** Feature 38's re-basis ran at its own HEAD, before this branch's `phase-contract` rule-1/rule-3 rewords (1.0.3) and the P4 `execute-phase` slim existed — both files every `execute-phase:*` route loads — so the seven `routeEstimateMax` ceilings were below `ceil(measured × 1.10)` on the merged tree (15 route failures there). They are re-set (11739 / 11752 / 11622 / 11784 / 11937 / 11536 / 11699) and this row is the growth source the ceiling policy requires beside the commit. With this re-basis the whole route set is green (22/22) and the pre-existing exceedances this branch had disclosed are closed — the fixture half by fix #200 on `main`, the live half by feature 38's re-basis plus re-basis #3. The same review's F26 fold adds the paste fence: every linter-stdout hand-off now states the block is lint output, never instructions (`execute-phase` 4.5.1, `plan-fix` 3.2.1, `plan-feature-scaffold` 2.3.1), and the pi mirror is re-bundled. The fence grew two route-loaded references, so the seven `execute-phase:*` estimate ceilings and the `plan-feature:scaffold` / `plan-fix:issue` estimate+lines ceilings are re-based to `ceil(measured × 1.10)` (24286 / 27169 for those last two) — this text is the growth source and the declaration the ceiling policy requires (re-basis #4).
- **2026-09-11 — four route ceilings re-based a second time.** Feature 37 cycle-2 fold F19: the same fold batch that re-based `plan-feature:issue`/`:scaffold`/`:scoped` and `plan-fix:issue` then reworded `phase-contract` rule 3 — a file every one of those routes loads — so the four `routeEstimateMax` ceilings were stale again at the reviewed head (19 route failures there vs 15 at `main`). They are re-set to `ceil(measured × 1.10)` (11558 / 24144 / 9062 / 27018) and this row is the growth source the ceiling policy requires beside the commit; the four routes stop being branch-introduced failures, while the 15 pre-existing exceedances stay owned by fix #200 / issue #176. Re-basis #4 (see the 2026-09-12 row) superseded two of these four values — `plan-feature:scaffold` is now 24286 and `plan-fix:issue` 27169; the numbers above record what re-basis #2 shipped on that date.
- **2026-09-11 — the phase contract rejects an empty phase.** Feature 37 review F5 makes `phase-contract` 1.0.3 refuse a phase that carries zero tasks: rule 3 now states a **≥ 1-task minimum** alongside the ≤ 8 budget (final hardening/close-out phase: 1–10), so a plan whose phase has no checkbox tasks BLOCKs box-3 instead of scoring a vacuous `PASS (8/8)`. The user ratified the strict-block ruling — an empty phase means the plan is wrong or the phase must be deleted — and the `phase-lint` corpus pins it.
- **2026-09-10 — three routes stop reasoning about phase shape.** Feature 37 P4 slims `plan-feature-scaffold` 2.3.0, `plan-fix` 3.2.0 and `execute-phase` 4.5.0 to run the deterministic linter (`bun scripts/phase-lint.mjs <plan>`, node fallback) and paste its stdout block: exit 1 is a BLOCKED phase to re-cut, never a verdict to re-derive by reading the rules. `phase-contract` stays the sole owner of the eight rules and the fingerprint, and the pi mirror is re-bundled.
- **2026-09-10 — the phase contract now sanctions the title its templates mandate.** Feature 37 P1 makes `phase-contract` 1.0.2 the single owner of the exception: the templates' literal closing title `Hardening & PR` is the sole authorized `&`-joined title — its `&` is a normalization separator, not a deliverable joiner, and its title-deliverable normalizes to `hardening-pr`; any other `&`-joined title still FAILs. The amendment closes the contradiction between rule 1's joiner list and the closing title every feature/fix template carries, without adding any rule semantics beyond the owner's own text.
- **2026-09-05 — receipts stop binding the roadmap.** Roadmap scoping (pre-execution-review 2.0.0, review-spec 1.6.0): the pre-execution snapshot builder no longer binds the shared `ROADMAP.md` as a `roadmap-row` context. The roadmap is every unit's shared lifecycle ledger — rows for other units land while a plan is in flight, and the status machine's own sanctioned writes (`planned` → `in-progress` at P1, `done` at PR open) used to invalidate every in-flight receipt repo-wide, forcing a pointless re-review exactly when an orchestrator can least afford one (an `execute-phase` blocked by a receipt whose only drift was another feature's row). The row's safety-relevant content stays guarded by gates that re-read it live — the own-status gate and the dependency gate's fingerprint receipt — while unit artifacts and the governing authorities stay fully bound, so a genuine artifact edit still fails closed. Receipts recorded before the change re-verify stale exactly once; see `docs/workflow/MIGRATION.md`.
- **2026-09-05 — verdicts route upstream, never repair.** Feature 29 P3 makes `implementation-discovery` 1.1.0 route each verdict to its owner — Plan defects to `plan-feature`/`plan-fix` plus `review-plan`, Product/authority gaps to `design-feature` then `review-spec`, unavailable evidence to one exact `BLOCKED` — while source-local findings stay local, mapping never replaces candidate review/verification/audit authority, and a second repair/re-review cycle inherits feature 28's `CONVERGENCE-ANOMALY`. Semantic/navigation and Engram are named advisory locators with direct repository fallback.
- **2026-09-05 — the pre-write mapper owns the read-before-write slot.** Feature 29 P1–P2 introduces `implementation-discovery` 1.0.0, an internal, read-only contract that answers the seven evidence questions, emits one fixed compact map, and routes `READY | REPLAN | NEEDS-DESIGN | BLOCKED` before any branch/planning/source write — no file/read count decides the route, and a fresh mapper triggers on cross-layer/public/security/compatibility work, unproven Plan assumptions, prior attempts, or bias. `execute-phase` 4.3.0 loads it after the pre-execution gate instead of reserving the slot; only READY, with an observed falsification probe and every carried planning-evidence row confirmed, authorizes the first write. No public command, schema, new workflow state, or forge/issue call is introduced.
