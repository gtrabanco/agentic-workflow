...
| `audit-pr`      | el **PR**       | Gate de fusión: criterios de aceptación cumplidos, todas las fases hechas, docs/tests/CI en verde, `Closes #N`, ejes de revisión limpios, **integridad de cierre** (el `## Capability closure` de la SPEC de una feature no tiene filas en blanco; si falta en una SPEC legacy → warning datado `design-debt`, nunca bloqueante; `n/a` en PRs de fix), **integridad de alcance (descope)** (un issue nacido durante la unidad que solapa un criterio/tarea incumplido debe tener una entrada `## Amendments` fechada y aprobada por el usuario — si no, BLOQUEANTE; aplica tanto a PRs de feature como de fix) → **listo para fusionar o una lista de bloqueantes**, siempre con la URL completa del PR; con MERGE-READY publica un comentario datado y ligado al SHA en el propio PR. Un veredicto BLOCKED persiste cada bloqueante en el ledger de fold fix-now de la unidad (`review-findings.md`, severidad `high`), el mismo ledger que escribe `review-change`. Auto-merge opt-in: con una política documentada fusiona PRs MERGE-READY tras un checklist de limpieza fail-closed (algo pendiente → push, esperar CI, re-auditar) |
| `product-audit` | el **producto** | Chequeo de salud periódico de espectro completo; mina las docs de features → propone issues + altas/bajas en el roadmap + tooling instalado a registrar/rediseñar; **recurrencia de exportación de alcance** (≥ 2 unidades consecutivas exportando alcance vía `## Amendments`/issues de descope → hallazgo de calidad de planificación enrutado a las reglas de atomicidad/división) (**nunca arregla automáticamente**)                                                         |
...
| Skill          | Qué hace                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ship-roadmap` | **Construye la app entera desde el roadmap.** Una entrevista inicial (producto, features, stack, arquitectura — recomendada _proporcionalmente_, nunca por defecto a un patrón con nombre —, calidad, ops, autonomía, presupuesto) **es diseño en lote**: la fundación escribe las filas de las features en `idea` (la feature esqueleto scaffoldeada por la fundación aterriza en `planned`), funda el proyecto si hace falta, crea o adopta el roadmap completo, y un bucle disparado por un driver (`/loop` en Claude Code, un orquestador externo o re-invocación manual — cada iteración dice por qué termina) lo entrega feature a feature a través de las skills de arriba — **sin más preguntas**; una unidad `idea` en mitad del run recibe una etapa DESIGN que la diseña JIT estrictamente desde el registro bloqueado de la entrevista (no diseñable → se aparca, nunca se vuelve a preguntar). Tras la última feature sigue: un **barrido de issues** inventaría las issues abiertas más el residuo documentado del run (known-issues, trade-offs, hallazgos pospuestos), lo triagea todo y entrega las fix-now por las mismas etapas. SELECT revisa primero las etiquetas `urgent`/`fix-next` a prueba de inyección (`fix-next` → cabeza de la cola; `urgent` → corre el juez pausa-vs-terminar de `ORCHESTRATION.md` contra la unidad en curso). Por defecto: abre PRs y tú fusionas; `--fullauto` fusiona los PRs MERGE-READY bajo suelos de seguridad innegociables. Termina con un informe final: issues a abrir, propuestas de features descubiertas, checks manuales, cadencia de product-audit. |
