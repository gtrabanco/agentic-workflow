...
| `loop-review-fold` | el **loop de candidata** | Reutiliza un recibo exacto al SHA; si ya existe una cola fix-now abierta, empieza por `fold-findings`; si no, ejecuta `review-change` en contexto limpio y después solo revisa HEADs distintos. Por defecto permite dos ciclos y termina en `PASS`, `NEEDS-DECISION`, `BLOCKED`, `NO-PROGRESS` o `BUDGET-EXHAUSTED`; nunca mergea ni crea issues. |
...
| `loop-review-fold` | Opus         | alto     | conductor acotado de revisión independiente y correcciones enrutadas por severidad                                                                                                                                         |
...
/loop-review-fold <NN>          # loop obligatorio y acotado: review → corrección por lotes → review del nuevo HEAD
