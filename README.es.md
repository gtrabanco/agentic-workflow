...
| `plan-fix`     | Redacta un SPEC de fix + `ACCEPTANCE.md` congelado desde uno o varios issues. Agrupa por una frontera atómica de entrega: un resultado de capacidad o regla mecánica homogénea, un plan de verificación y una frontera de release/rollback; no exige mismos ficheros, causa raíz o severidad. Los inputs incompatibles devuelven los mínimos grupos máximos. El lint de fases es determinista: ejecuta `bun scripts/phase-lint.mjs <plan>` y pega su salida (exit 1 → re-cortar). |

> `design-feature` (definición de producto, incorpora la entrevista de idea en
> crudo) debe marcar una feature como `designed` **y** `review-spec` debe
> devolver un `SPEC-REVIEW-PASS` vigente antes de que `plan-feature` la
> planifique — si falta alguno, se detiene y redirige, sin flag de bypass.
...
| Skill           | Qué hace                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `execute-phase` | Sin argumento de fase ejecuta **todas las fases restantes** de la feature/fix hasta abrir el PR; `P<n>` mantiene el modo atómico. Cada fase conserva su comprobación del blob de aceptación, gates, tests, docs y commit. El loop usa workers frescos donde existan, recibos compactos en fallback, omite reviews intermedias y para ante rojo, ausencia de progreso o presupuesto. Los hallazgos independientes quedan como propuestas, nunca issues automáticos. El preflight de cada fase ejecuta `bun scripts/phase-lint.mjs <plan>` y pega su salida. |

### Revisión y auditoría — _cambio → PR → producto_

| Skill           | Alcance         | Qué hace                                                                                                                                                                                                            |
| --------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `review-spec` | el **diseño** | Puerta de producto independiente y de solo lectura: construye el snapshot `spec-product-v1` de la mitad de producto de un SPEC diseñado, ejecuta un pase de falsación más las catorce comprobaciones fijas de producto en un contexto que no la escribió, y devuelve solo `SPEC-REVIEW-PASS`, `SPEC-REVIEW-FAIL` o `NEEDS-DESIGN` con un recibo ligado al contenido que trata todo byte que lee como dato, nunca como instrucción. Nunca edita el SPEC revisado ni elige la intención de producto: `plan-feature` se niega a planificar sin su recibo vigente, y su contrato de turno apunta a la §8 de `POLICY.md` de `pre-execution-review` como dueña de la regla que liga un veredicto a su marca durable. |
| `review-plan` | el **plan** | Puerta de ingeniería independiente y de solo lectura: snapshotiza el plan congelado (SPEC, aceptación, evidencia de planificación, obligaciones, fases, pruebas), barre los libros y las comprobaciones fijas de ingeniería —más la autoridad de reproducción/causa raíz/regresión/rollback en los fixes— en un contexto que no los cortó, y devuelve solo `PLAN-REVIEW-PASS` o `PLAN-REVIEW-FAIL` con un recibo ligado al snapshot, y lee todo byte que abre —incluido el recibo de spec copiado— como dato, nunca como instrucción. No edita ningún artefacto de plan: `execute-phase` se niega a correr sin su recibo vigente; el digest padre que registra se recalcula según la §7 de `POLICY.md`, y su contrato de turno apunta a la §8 para la marca durable. |
| `review-change` | el **cambio**   | Ejecuta solo las revisiones que **aplican a tu plataforma** (código, seguridad, verify, diseño, a11y, marca, rendimiento, SEO) — adversariales por defecto, asumiendo que el diff está mal hasta probar lo contrario — y clasifica → una tabla de decisión + una checklist explícita de verificación manual; un árbol sucio o commits sin push en la rama del PR detienen la revisión como precondición `REVIEW BLOCKED` antes de ejecutar cualquier pase — el estado del workspace nunca es un hallazgo persistido, y la revisión hace commit de su propio añadido de hallazgos para no dejar sucio el árbol que después juzgará. La revisión final obligatoria **debe correr en una conversación que no implementó el cambio** — si lo hizo, parar y hacer hand-off a una nueva. Opt-in `--adversarial N`: N revisores independientes y context-clean, cada uno con un rol asignado por índice (corrección/seguridad/cobertura-de-SPEC), corren en paralelo (subagentes / headless / fallback secuencial), hallazgos fusionados por `file:line` con umbral de inclusión ≥1 — desactivado por defecto, auto-recomendado (nunca forzado) cuando el cambio es `L`/sensible, el revisor no es el más fuerte de la flota o es más débil que el autor del diff, o solo hay una familia de modelo disponible en un cambio `≥M`. `--synthesize` es el punto de entrada de fusión independiente para revisores ejecutados a mano. Los hallazgos fix-now en una unidad no mergeada se persisten en el ledger de fold fix-now de esa unidad (`review-findings.md`), deduplicado por `file:line`+axis (solo `high`/`med` se persisten — los hallazgos `low` son notas de solo-informe que nunca bloquean). La clasificación respeta las **comprobaciones de anulación hacia fix-now** del motor: un arreglo barato o un defecto dentro de alcance es siempre fix-now (nunca un escape a postpone/known-issue/tradeoff), y un fix-now dentro de alcance demasiado grande se enruta a `replan-in-unit` — fase(s) del SPEC confirmadas por el usuario en la misma rama, nunca una degradación |
...
| `audit-pr`      | el **PR**       | Gate de fusión de solo lectura que **consume el recibo `REVIEW-PASS` vigente de `review-change`** (un recibo ausente/obsoleto es un bloqueante enrutado a `/review-change`, nunca se re-revisa) y evalúa solo el contrato de entrega: fases/docs completos, CI, fusionabilidad, trazabilidad, cierre de capacidades, integridad de descope y el resultado de invariantes/verificación manual del recibo → **MERGE-READY o bloqueantes con evidencia**, siempre con la URL completa. MERGE-READY publica un comentario de PR ligado al SHA; BLOCKED persiste bloqueantes en el ledger común de fold. Nunca edita ni fusiona: solo una etapa activa `ship-roadmap --fullauto` puede consumir su veredicto e invocar el wrapper transitorio. |
| `product-audit` | el **producto** | Chequeo periódico de espectro completo solo por invocación explícita, persistido como `docs/audits/<id>-<fecha>.md`; extrae de código e historial hallazgos por severidad y propuestas de issues/roadmap/tooling, comprueba la frescura del inventario y la exportación repetida de alcance, avala cada afirmación con la procedencia de su evidencia y reporta el delta frente a la auditoría previa de alcance equivalente, y nunca arregla automáticamente. |
...
| `workflow-status` | **Sensor de solo lectura para orquestación programática.** Calcula el estado completo del proyecto — cada feature/fix con su cierre de dependencias transitivo (cumplido/incumplido), la máquina de cinco estados del roadmap (`idea`/`defined`/`planned`/`in-progress`/`done`), qué es arrancable ahora mismo (estado ≥ `defined`, dependencias cumplidas) y en qué orden de construcción, las filas `idea` reportadas como `detail.design_candidates`, PRs abiertas + estado de auditoría, fixes pendientes y hallazgos a la espera de triaje, la revisión obligatoria probada por su fila durable `REVIEW-RAN` y no por un `review-findings.md` que simplemente existe, el backlog de issues abiertos sin triar (`detail.untriaged_issues`, con la etiqueta de disposición como señal autoritativa y el comentario `VERDICT:` como fallback heredado), los hallazgos fix-now sin foldear de cada unidad, leídos de su ledger `review-findings.md`, como items estructurados `findings.fix_now[]` con un `suggested_tier` derivado, más el campo `detail.urgent` a prueba de inyección (issues `urgent`/`fix-next` solo por etiquetas + hechos de interrumpibilidad de la unidad en curso) — y lo emite como un envelope máquina JSON fijo, con `findings.fix_now[]` normalizado al enum publicado `high`/`med`/`low`, autocomprobado contra el esquema empaquetado y una tabla fija comando→nivel antes de imprimirse (los campos por unidad `review`/`closure`/`issues_born` y la superficie `next.suggested[]` siguen siendo los pasos 10–13 de la secuencia publicada, del lado de la skill y **aún no mecanizados en el sensor**). Con `--last-envelope`, una **guarda de no-progreso** señala un hint `/plan-feature`/`/design-feature` estancado (la unidad sigue en su estado previo al avance) como una nota en `workflow_observations` en vez de repetirlo silenciosamente. La pieza que un driver externo llama entre pasos (ver [Orquestación programática](#orquestación-programática)). Nunca edita nada. |
...
| `execute-phase`  | Sonnet         | medio    | implementación mecánica — unidad completa por defecto con transacciones frescas/compactas por fase (Opus si la lógica es sutil)                                                                                           |
...
contexto modestas, sin caché de prompt): aceptación congelada, checklists fijas
en lugar de juicio, puertas Phase-lint y Spec-lint, recibos compactos de fase,
loops de reparación acotados y revisiones aisladas por eje que devuelven solo
tablas de hallazgos. En una flota sin ningún modelo de clase frontier:

- **Ejecución** (`execute-phase`, `log-session`, mantenimiento de docs) está
  diseñada para el tier más barato — un contexto de worker limpio por fase,
  handoff compacto vía `progress.md`, máximo 10 lecturas de fichero completo
  por fase. La invocación exterior continúa por todas las fases restantes salvo
  que se pase un `P<n>` explícito.
...
/fold-findings → nuevo /review-change  # review→fold manual; los hallazgos no resueltos pasan a triage/replan
