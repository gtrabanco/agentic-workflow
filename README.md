...
| `loop-review-fold` | the **candidate loop** | Reuses a current exact-SHA receipt; when an open fix-now queue already exists, starts with `fold-findings`, otherwise runs context-clean `review-change`, then re-reviews only changed HEADs. Defaults to two correction cycles and stops `PASS`, `NEEDS-DECISION`, `BLOCKED`, `NO-PROGRESS`, or `BUDGET-EXHAUSTED`; never merges or creates issues. |
