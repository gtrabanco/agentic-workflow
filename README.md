...
| `product-audit` | the **product** | Periodic full-spectrum health check; mines feature docs → proposes issues + roadmap add/remove + installed-tooling to register/re-design; checks capability-inventory freshness (`docs/CAPABILITIES.md` ↔ code drift); **scope-export recurrence** (≥ 2 consecutive units exporting scope via `## Amendments`/descope issues → planning-quality finding routed to the atomicity/split rules) (**never auto-fixes**)                                                                          |
