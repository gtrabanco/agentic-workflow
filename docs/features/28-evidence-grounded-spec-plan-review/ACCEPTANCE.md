...
| AC10 | Schema package is released as additive minor `3.5.0`, both new schemas pack, the Pi package is released as minor `0.2.0` (its publish workflow fires on merge), every changed skill is bumped/changeloged, root/Pi skill bundle parity and Pi tests pass, intended skills are discoverable, context budgets pass, and EN/ES docs/migration are synchronized | schema version/pack checks; `cd packages/pi-agentic-workflow && npm run bundle:skills && npm test`; Pi `package.json` version newer than registry; `node scripts/check-skill-context.mjs`; `npx skills add . --list`; repository doc/version checks -> exit 0 |
