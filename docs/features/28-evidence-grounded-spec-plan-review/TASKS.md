...
- [x] Fold every fix-now row still open at this head with `fold-findings`, including the F32 owner verdict and F35. (*2026-09-02 fold: F38 fixed at the root — `SENSOR_CORE.md:85-91` now judges a mark by ancestry plus `git log <mark-sha>..HEAD -- <bound paths>` empty (the bound set cited from `SNAPSHOT.md`, `STAGE_ARTIFACTS` exported from `scripts/pre-execution-snapshot.mjs` so nothing restates it) — and its fixture now derives HEAD from a commit it makes, 7/7 where three real states are distinguished; F39 fixed at the owner of each stage's output contract (`Snapshot: refused` beside the builder's code, identical at both stages, `CHECKS.md` cites it), pinned by `pre-execution-quality`'s F39 case; F40/F41/F36 carry their `· fold <sha>` tokens from `scripts/ledger-provenance.mjs`, whose escaped-pipe blindness (a row containing `\|` silently vanished from the recount, `--check` and `--annotate` — found by this unit's own F38 row) is fixed with a test; F38/F39 sit `folded: yes` awaiting their token at the close-out sha. Also closed here per its own prescription: known-issue 17's `.engram/` blob.*)
...
- [x] Route `sha256HexSync` to `globalThis.process?.getBuiltinModule?.("crypto")` when that object exposes `createHash`, and to the existing pure-JS implementation when it does not. — `packages/agentic-workflow-schema/src/sha256.ts:136-151` is the lookup (guarded call, shape-checked, `null` when the host exposes nothing) and `:167-179` is the routed entry point, whose `catch` at `:173` keeps a binding that fails mid-flight on the JS core. The host shape is typed locally at `:110-118`, which is what keeps it compilable without a `node:` specifier or `@types/node` (measured: adding the specifier makes `tsc --noEmit` exit 2 on `error TS2591`).
- [x] Keep the public sync signature and the lowercase 64-hex result identical on both paths. — the signature is byte-identical to pre-P17 (`sha256HexSync(data: string): string`, `src/sha256.ts:167`), and one `toHex` helper (`:153-157`) now formats the native, JS and WebCrypto results, so a path cannot drift in shape. Evidence: `sha256HexSync answers from the host native SHA-256 and all three paths agree` (green, `test/pre-execution-canonical.test.mjs:350`) and `npm run probe:sha256-paths` -> exit 0, `identical: YES` on all three corpus shapes.
- [x] Add the three-path agreement case — native binding, pure JS, WebCrypto async — over the ASCII, multibyte, and oversized corpus to `test/pre-execution-canonical.test.mjs`. — `test/pre-execution-canonical.test.mjs:314` declares `SHA_PATH_CORPUS` (an ASCII selection, a multibyte selection carrying every UTF-8 width, and the real feature-28 SPEC projection as the oversized case) and `:350` is the case, which asserts native/pure-JS/WebCrypto/`node:crypto` all return the same 64-hex string and that the native binding was consulted per call. Written first, and red: `node --test test/pre-execution-canonical.test.mjs` on `git archive a42cf485` + this file only -> **24 tests, 23 pass, 1 fail, exit 1** on `the native path answered, and answered per call (got [])`; green in the package suite at 675/675, exit 0.
- [x] Correct the header comment in `src/sha256.ts:13` to name `test/pre-execution-canonical.test.mjs`, the file that actually pins the digests (finding F36). — `src/sha256.ts:13-17` names that file and no other; `grep -rn "pre-execution-sha256" src/` returns nothing outside `dist/` (a build artifact of the old header), and `ls test/*sha256*` still finds no such file, so the header now cites one of the three real pin sites (`test/pre-execution-canonical.test.mjs:51`, `:198`, `:295`) plus the agreement case this phase added.
- [x] Record the measured cost of the three rejected alternatives — static `node:crypto`, an `@noble/hashes` dependency, and vendoring its 1,419-line `sha256` closure — in `architecture-notes.md`. — `architecture-notes.md:70` §"Digest paths (AC21, F32, D36)" carries the numbers measured on this host: pure JS costs +457% / +774% / +742% against the routed native path at 6 KiB / 52 KiB / 4 MiB, `@noble/hashes@2.4.0` +52% / +140% / +108% and 691,646 unpacked bytes (167,626-byte tarball, 60 files), the vendored closure is 1,419 lines across `sha2.js` 446 + `_md.js` 209 + `_u64.js` 77 + `utils.js` 687, and the static specifier costs `tsc --noEmit` exit 2 plus 2,534,873 bytes / 89 `.d.ts` of `@types/node`. What was *not* measured is named there (browser bundling of noble, Deno).
- [x] Add the standing vendoring rule to `CLAUDE.md`: any code copied from a third party carries source URL, author, version, and license name in a header comment. — `CLAUDE.md:57-68`, §"Working rules": the four provenance fields are mandatory, a copy without them is called a provenance defect, and the rule names this unit's own arithmetic (the 1,419-line closure was weighed and refused) so the next reader sees vendoring as a weighed alternative rather than a default.
- [x] Rewrite the package README's zero-runtime-dependency line to state the contract precisely: no dependencies and no static builtins, with the host's native hash used opportunistically when exposed. — `README.md:11-16` states the claim as `package.json` declaring no `dependencies` and no module under `src/` carrying a `node:` specifier, and `README.md:410-421` says where the native hash fits, per call and never cached; `README.es.md:11-16` / `:423-434` carry the same text in Spanish in this same commit (the bilingual-pair rule, not deferred).
