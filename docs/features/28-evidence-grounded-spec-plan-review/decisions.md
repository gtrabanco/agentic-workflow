...
justification | scripts/pre-execution-attribution.test.mjs | P8b | 2026-09-22 | execute-phase | P8b migrate editedSpec() fixture from Goal override to Objective override: Goal is not a required section in UNIT_DOC_REQUIRED_SECTIONS, so the old override was a no-op on the Product projection (same digest); the edit exercises stale-artifact-content by changing an actual required section.
justification | scripts/pre-execution-sensor.test.mjs | P8b | 2026-09-22 | execute-phase | P8b replace specText() fixture with 13-section unit doc: the spec-product-v1 selector no longer accepts the retired two-half SPEC format.
justification | scripts/pre-execution-timeline.test.mjs | P8b | 2026-09-22 | execute-phase | P8b replace specText() fixture with 13-section unit doc: the spec-product-v1 selector no longer accepts the retired two-half SPEC format.
