...
| PE-017 | None of the six clauses is implemented in the skill tree on this branch, so adopting them is real work, not bookkeeping | repository skills | `grep -ril "drift gate\|write-then-report\|normalization before freeze\|clean-review sensing\|delegate-only"` over `skills/` → zero hits; the only matches are the prose of this unit's `progress.md` | O15–O20 | current at 2026-09-01 | verified | — |
| PE-018 | Roadmap row 29 diverged the same way (main carries the fuller writer-handoff clause), so the branch must re-base both rows before the conflict is row 28's status alone | git | `diff <(git show origin/main:docs/features/ROADMAP.md \| sed -n '39p') <(sed -n '39p' docs/features/ROADMAP.md)` → main adds the frozen-authority/map/confirmed-evidence-IDs parenthetical; row 30 `bilingual-sibling-drift-check` exists only on main | F22 | current at 2026-09-01 | verified | rows 29 and 30 re-based byte-identical by this amendment |
