...
| id | file:line | axis | severity | class | route | folded |
|---|---|---|---|---|---|---|
| F38 | skills/workflow-status/references/SENSOR_CORE.md:85-86 (`counts it only while the head sha it names is the unit's current head`) against skills/pre-execution-review/references/LEDGERS.md:167 (`REVIEW-RAN \| HEAD <40-hex sha>`) | architecture (proof unreachable by the flow it governs) | high | fix-now | fold: step 8 must judge currency by "no commit after the named sha touched a bound input" (`git log <mark>..HEAD -- <bound paths>` empty), which is what the digests already mean, and `scripts/workflow-status-pre-execution.test.mjs` must derive `headSha` from a commit it makes rather than injecting one — AC20's proof is otherwise unsatisfiable on a branch | no |
| F39 | skills/review-spec/references/CHECKS.md:49-50 ("a refused build … never a hand-computed substitute") and skills/review-plan/references/OUTPUT.md:16 (`Snapshot: <64-hex>`) | workflow (template demand beats prohibition) | high | fix-now | fold: name the contracted terminal form for a refused or unbuildable snapshot at both stages (what `Snapshot:` carries when there is no snapshot object) — a weak executor facing both texts obeyed the template and substituted the raw file SHA-256 on one stage and `null` on the other, both self-declared | no |
