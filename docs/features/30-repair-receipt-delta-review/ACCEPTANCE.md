...
| AC-03 | Branch contract pinned: the discipline suite asserts the three closing-branch outcomes (`RE-REVIEW-REQUIRED (delta)` / `RE-REVIEW-OPTIONAL` / `RE-REVIEW-SKIPPED`) and the literal default line ("no decision → re-review") in `skills/fold-findings/SKILL.md`'s closing-block spec | `node --test scripts/review-loop-discipline.test.mjs` -> exit 0 (branch pins) |
