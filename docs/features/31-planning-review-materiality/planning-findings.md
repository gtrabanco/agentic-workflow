...
Ledger flip (closed): `design-feature:product-class-resolutions` flipped both rows
to `status: resolved` / `resolving-artifact-revision: 31-spec-15` with resolution
evidence pointing at batch `31-spec-15` (`decisions.md` D-31-13) — the substance
repair `spec-review-31-15` already verified (14/14 PASS, zero new findings,
`material open: 0`). No reviewed artifact byte was modified by this fold.
