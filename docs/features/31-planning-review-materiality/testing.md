...
| Additive release (schema package) | the package gate passes, the bumped version's two pins move with it (`test/release-contract.test.mjs`, `test/verification-gates.test.mjs`), and no enum value or contract id is removed | `cd packages/agentic-workflow-schema && bun run gate:pre-execution` → exit 0 + the vocabulary-diff walk (AC9) |
| Published limits (schema package) | every `PRE_EXECUTION_LIMITS` key and value is documented in the package README's `### Published limits` block, including the new `reproducerChars 1024` | `cd packages/agentic-workflow-schema && bun run test` → exit 0 with the `pre-execution-docs` walk green (AC3, AC9) |
