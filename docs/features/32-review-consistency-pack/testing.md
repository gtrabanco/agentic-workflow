...
| Release bookkeeping (P4) | budgets re-based for every touched skill at the declared re-basis, drift gate green | `node scripts/check-skill-context.mjs` → exit 0; `node --test scripts/normative-drift.test.mjs` → exit 0 |
