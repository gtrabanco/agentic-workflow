...
criteria AC1–AC10. One stable ID per SPEC criterion; validators copied from the
criteria. Modifying this manifest during execution requires a user-approved
SPEC amendment.
...
| AC8 | The three consumer skills reference the script; `phase-contract` unchanged (sole rule owner); context budgets + CLI discovery pass; pi mirror re-bundled | `grep -n "phase-lint.mjs" skills/plan-feature-scaffold/SKILL.md skills/plan-fix/SKILL.md skills/execute-phase/` → matches in all three; `git diff --name-only main...HEAD -- skills/phase-contract` → empty; `bun scripts/check-skill-context.mjs` → exit 0; `npx skills add . --list` → exit 0; `npm run bundle:skills` re-run |
