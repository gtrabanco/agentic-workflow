...
- [ ] Implement the eight rule checks (`box-1`…`box-8`) with per-phase verdict lines in the fixed output block
...
- [ ] Write the corpus fixtures in `scripts/phase-lint.test.mjs` (valid, invalid, ambiguous-layer, no-phases, missing-plan) asserting verdicts + reason codes
...
- [ ] Keep `skills/phase-contract/SKILL.md` unchanged (sole rule owner)
