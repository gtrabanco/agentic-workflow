...
| 2026-09-12 | `gh pr edit <n> --body-file` fails on this repository's PRs (`GraphQL: Projects (classic) is being deprecated in favor of the new Projects experience`, exit 1) because PR #212 carries a classic project card, so the P6 close-out could not refresh the PR body through the forge path the skills document | `gh pr edit 212 --body-file /tmp/pr212-body.md` → `exit=1`; `gh api --method PATCH repos/gtrabanco/agentic-workflow/pulls/212 -F body=@/tmp/pr212-body.md` → exit 0, body updated (verified: `gh pr view 212 --json body` carries no literal backslash-backtick escape) | n/a — environment/tooling; no repository change identified | low | no | Proposal | independent tooling defect outside this unit's files; a same-turn workaround landed the mandated body refresh, and execution never runs `gh issue create` | revisit if `gh pr edit` keeps failing after GitHub's classic-projects sunset, or the first time a workflow step must set a PR body non-interactively | proposal |
