...
criteria A:1–A:23 + the read-verified criterion. One stable ID per SPEC
criterion (`A-01`…`A-23`, `A-RV`); validators copied from the criteria and made
...
| A-07 | Urgency labels read from the labels object only (bodies/comments never fetched or read); `title` is allowed (SENSOR_CORE step 3 emits `urgent.issues[].title` — output, not a scan) | `grep -nE '\b(body\|comment)' scripts/workflow-status.mjs` -> no match AND `grep -cE 'labels' scripts/workflow-status.mjs` -> ≥ 1 |
