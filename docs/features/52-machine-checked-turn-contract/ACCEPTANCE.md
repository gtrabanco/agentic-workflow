...
| AC12 | The shim needs only bash + git + gh — no node/bun/npm/npx token in the file | `grep -n -e node -e bun -e npm -e npx template/.agentic-workflow/hooks/turn-contract.sh` → no matches (same alternation as SPEC AC12, written as repeated `-e` patterns so the copied command actually works — an escaped-pipe form matches the literal string and can never fail) |
