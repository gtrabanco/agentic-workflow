...
| AC5 | The doc slim holds: the exemption pipeline `grep -rn "csv-export-command" docs/ \| grep -vE '^(docs/features/\|docs/workflow/GOLDEN_FIXTURE\.md:[0-9]+:[\|] 20[0-9]{2}-)'` returns no output — SPEC AC5's frozen pipeline in its markdown-table-safe form (the row's literal pipe written `[\|]`, the sole alternation written `\|`; both forms exempt the dated run-log rows and `docs/features/` and yield the identical line set on the tree); `docs/workflow/GOLDEN_FIXTURE.md` is ≤ 150 lines and points at `scripts/fixtures/golden-fixture/`; `grep -n "Tool-calling smoke test" docs/workflow/GOLDEN_FIXTURE.md` and `grep -n "Fixed pass criteria" docs/workflow/GOLDEN_FIXTURE.md` each return a match, and the retained pass criteria carry the "No invented steps" rule | command: the exemption pipeline printed in `## Commands` (byte-exact, unescaped) plus `wc -l docs/workflow/GOLDEN_FIXTURE.md` → ≤ 150; no live slug hit outside the grandfathered rows |
...
- `grep -rn "csv-export-command" docs/ | grep -vE '^(docs/features/|docs/workflow/GOLDEN_FIXTURE\.md:[0-9]+:[|] 20[0-9]{2}-)'` (AC5 — byte-exact; the table row above is the same command with markdown pipes escaped)
