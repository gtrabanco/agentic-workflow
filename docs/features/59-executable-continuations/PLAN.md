# PLAN — 59-executable-continuations

Five implementation phases (schema → sensor emission & discipline proof →
quote-surface adoption → batched interview → hardening close-out). The product
sketch's "P2 emission + quote surfaces" is cut into two phases by the
...
Add the optional schema-validated `next.continuation` object, the closed refusal
vocabulary, the pure emitter API, vectors, and the additive-minor release to the
schema package. The envelope stays valid without the field; every invalid shape
...
Phase-lint: PASS (8/8) · fingerprint `P1:config/infra:7:envelope-continuation-schema`

## P2 — Sensor continuation emission

Layer: config/infra

Project the sensor's already-resolved `next` command into an executable
continuation at the emission hook points, with fail-closed refusals and the
per-class discipline proof. The emitter consumes `resolveNext()`'s output — it
projects, never re-derives the decision (PE-004, PE-005).
...
State the quote rule at the five pinned consuming surfaces, add the one
FEATURE_WORKFLOW pointer, and register the two normative rows with their drift
declarations. Humans keep rendered prose; skills quote the emitted command.
...
Phase-lint: PASS (8/8) · fingerprint `P3:docs:7:quote-surface-adoption`

## P4 — Batched design interview

Layer: docs

Replace the one-question-per-turn interview rule with the bounded form protocol
in `design-feature`, update the golden-fixture expectations, and re-base the
touched skills' context budgets now that all skill text is final.
...
Phase-lint: PASS (8/8) · fingerprint `P4:docs:7:batched-design-interview`
...
Phase-lint: PASS (8/8) · fingerprint `P5:hardening:8:hardening-pr`
