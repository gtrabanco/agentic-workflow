...
Where a criterion names a shorthand invocation
(`node --test packages/agentic-workflow`), the validator freezes the same
assertion in the runnable bun/glob form (E-60-10): a directory positional is not
a Node ≥ 22 `--test` interface and exits with `MODULE_NOT_FOUND`.
...
|---|---|---|
| AC-01 | Policy matrix fixtures green over the phase × path-class matrix, including create/modify/delete/rename rows for the `tests/**`, `e2e/**`, `*.test.*`, `fixtures/**` families and the policy config itself | `bun test packages/agentic-workflow/test/` → exit 0 with the freeze × class × operation matrix cases green (Node 24 fallback: `node --test packages/agentic-workflow/test/*.test.mjs` → exit 0) |
...
| AC-05 | Escape hatch: a post-freeze protected change with a justification but no recorded owner approval fails the gate; adding the recorded approval makes the identical diff pass; no code path grants approval automatically | `bun test packages/agentic-workflow/test/` → exit 0 including the `approval-required` and no-auto-approval negative cases (Node 24 fallback: `node --test packages/agentic-workflow/test/*.test.mjs` → exit 0) |
| AC-06 | Creation ≠ modification: matrix fixtures prove pre-freeze test authoring (create) passes without a marker while modify/delete of already committed test files requires one | `bun test packages/agentic-workflow/test/` → exit 0 including the create-versus-modify cases (Node 24 fallback: `node --test packages/agentic-workflow/test/*.test.mjs` → exit 0) |
...
| AC-08 | Checkpoint wiring: the `execute-phase` preflight proves the Tier 1 gate runs at the phase fingerprint checkpoint and a gate fail blocks the phase | read-verified: the path-protection step text (base-ref record + `--base` committed-range invocation) quoted in the phase handoff; command-verified by AC-02's gate suite |
...
- `bun test packages/agentic-workflow/test/` (Node 24 fallback: `node --test packages/agentic-workflow/test/*.test.mjs`)
