...
|---|---|---|
| Policy model + evaluator (P1) | the `path-protection-policy@1` model parses, the shipped defaults resolve, `resolvePathPolicy` unions globs and takes the requirement maximum, and the two gap parsers fail closed | `node --test packages/agentic-workflow` → exit 0 |
| Tier 1 gate (P1) | the freeze × class × operation matrix (create / modify / delete / rename for the tests, e2e, test-file, fixtures, and policy-config classes), creation-versus-modification, justification / approval / unmatched-record / malformed-declaration, shipped-default fallback + degradation, and the CLI exit codes with the fixed block | `node --test packages/agentic-workflow scripts/path-protection.test.mjs` → exit 0 |
| Template ship (P2) | the template seed is byte-identical to the crate default; the doc page exists; the hooks README documents the guard; both `init-workspace` references seed it | the mirror diff is empty; `grep -c 'path-policy' skills/init-workspace/references/*.md` ≥ 2 |
...
| Qualification (P5) | the whole ladder green, the pi mirror byte-identical, the acceptance blob unchanged | `node --test scripts/*.test.mjs` + `node --test packages/agentic-workflow` + the pi suite → exit 0; parity green |
