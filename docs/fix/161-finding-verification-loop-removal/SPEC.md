...
| O26 | CLAUDE.md mirror rule | The Pi package mirror is re-bundled in the same PR as the last `skills/**` edit: `npm run bundle:skills` (in `packages/pi-agentic-workflow/`) runs in P3a task 7 — after every skill edit of P1/P2/P2b/P3a — so the mirror never keeps advertising the retired route (AC5 re-verifies parity) | P3a | 7 | execute-phase | `node --test packages/pi-agentic-workflow/test/skill-parity.test.mjs` → 0 failing | test output | planned |
