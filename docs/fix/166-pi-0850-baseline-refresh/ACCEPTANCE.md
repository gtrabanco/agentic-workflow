...
| AC6 | The fix-index row for #166 is closed with the PR link after the PR opens, in the index's backticked done-row convention. | ``grep -cE "\[#166\]\(https://github.com/gtrabanco/agentic-workflow/issues/166\) \| pi-0850-baseline-refresh \| \`?done\`? · \[#" docs/fix/README.md`` → 1 |
...
- The baseline note is never refreshed to a pi version the package suite does
  not pass or on which any manual smoke observation failed (SPEC obligation
  O12); a red suite or a failed smoke observation (`- SMOKE … outcome: fail`)
  on 0.85.x stops the unit before P3 and is triaged as its own finding.
