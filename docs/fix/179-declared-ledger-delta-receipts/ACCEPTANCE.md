...
| AC2 | Fail-closed preserved: a content hunk, an undeclared flip, an uncommitted flip, a moved bound context, a forged declaration row naming a commit that never touched the path, or any out-of-class delta still answers `stale-source-revision`/`stale-artifact-content`/`stale-context` and blocks; proven red-first at pre-fix bytes | same suite → fail 0 (fail-closed cases (b)–(e), (g)); red-first run at pre-fix bytes pasted in the unit's `progress.md` (read-verified) |
