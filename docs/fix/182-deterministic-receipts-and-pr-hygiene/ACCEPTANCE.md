...
| AC4 | Terminal hygiene is derived from state: a clean tree, a level branch and a non-draft PR pass; a dirty path is named in the blocker; an unpushed branch names its commit count; `--apply` runs only the PR-ready mechanical repair. | `node scripts/audit-pr-gate.mjs hygiene` → exit 0 on a clean tree; the named blockers otherwise |
