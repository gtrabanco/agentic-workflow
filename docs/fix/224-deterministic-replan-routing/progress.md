...
## Pre-execution review receipt v1 — plan
- Review: rp-224-20260915-008 · Snapshot: 5286729185a902f0315236766f8eca3781d786fdddfbc5961d228a8e9901de02 · Verdict: plan-review-fail
- Unit: fix-224 · Stage: plan · Unit kind: fix
- Parent SPEC snapshot: null · Parent Product receipt: none
- Parent note: fix unit — no Product half exists (D6); the contract forbids a parent on a fix plan snapshot (D30)
- Source revision: 60993f44dc751cd162e1bddbf088d5ba54c299c7 · Artifact revision: 60993f44dc751cd162e1bddbf088d5ba54c299c7
- Reviewer: review-plan (fresh pi session) · Session: pi-web review turn on `fix/224-deterministic-replan-routing` (replan re-review, VERDICT-RESPONSE) · Role: reviewer · Author: plan-fix (replan commit `60993f44`) + execute-phase (ticks)
- Author exclusion: not-enforceable · Context clean: true
- Model diversity: same-model · Policy: v1
- Context-clean note: this conversation wrote or replanned no part of the unit; the only writes are this receipt block, its self-check output, the five finding rows it emits and the evidence commit — no reviewed artifact was touched.
- Started/finished: 2026-09-15T22:44:00Z/2026-09-15T22:58:00Z · Findings: 5 (material open: 5)
- Ledgers read: planning-evidence 15 rows (PE-001…PE-015, embedded in the SPEC) · obligations 17 rows (OB-1…OB-17, embedded; verified-capable: 16 — OB-1…OB-16 validators ran and their evidence is recorded in this ledger, OB-17 is `planned`)
- Prior plan receipt (re-review only): rp-224-20260915-007 @ 3d5009783b84b41be4c4000dc3ae0f7538a500a30c07c01824033fb1a74734fa
- Portability note: the planner's hand-off named `fix-224-artrev-0008` = `60993f44`; the builder resolved `sourceRevision`/`artifactRevisionId` to `60993f44dc751cd162e1bddbf088d5ba54c299c7` (`contentRevision` over the bound paths), so the label and the bound bytes agree (POLICY §7: the claimed value recorded beside the recomputed one, never substituted). HEAD is `854ee075`, the `progress.md`-only receipt commit of the replan hand-off — not bound by the snapshot — and `git rev-parse origin/fix/224-deterministic-replan-routing` → the same pushed head. The schema package was present (`NODE_PATH=packages/agentic-workflow-schema/node_modules`); the authoritative validator executed with no refusal and no install.
...
- Snapshot build: `NODE_PATH=packages/agentic-workflow-schema/node_modules node scripts/pre-execution-snapshot.mjs build --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing` → digest `5286729185a902f0315236766f8eca3781d786fdddfbc5961d228a8e9901de02`, schema-validated by the recipe owner; `unitKind: fix`, no `--parent` passed, `parentSpecSnapshotDigest: null`. Artifacts: `acceptance` (7057 B, digest `84ad83a6d440668841baaae232c889264093202651cc779aabd61dcfc7148761`) + `spec` (67410 B, digest `905a05580fefee5750dc79d2fc4c63651d14bf61089147b787c91d6f6be893ad`); both planning ledgers are embedded in the SPEC and bound through the whole-file `spec` row, so their own rows are `absent` (the shape cycles 1–7 bound and the authoritative validator accepted). Contexts: `project-guide` (`CLAUDE.md`) + `normalized-repository-state` (`docs/workflow/REPOSITORY_STATE.md`) present and bound; `architectural-invariants` absent (`docs/architecture/` does not exist — honestly unbound).
- Acceptance manifest blob re-derived, never copied: `git hash-object docs/fix/224-deterministic-replan-routing/ACCEPTANCE.md` → `ed2a25a819ba02483116c857b2809414566cea44`, equal to the value recorded in the SPEC `## Status` (`SPEC.md:823`) ✓.
- Prior-receipt staleness re-derived: `verify --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing` → `current:false`, `digestMatches:false`, `reasonCode: stale-source-revision`, `detail: reviewed at 72f2f58d…, the bound bytes now sit at 60993f44…`, `changedPaths:[SPEC.md]`, exit 4 — the replan's own rotation, not a plan defect (the hand-off named it as stale by construction; not reported).
- Repeat sanctioned (POLICY §4): the snapshot changed (`3d500978…` → `5286729185…`) and the review carries a named falsifiable question (does the `F25` exemption really make the append-after shape lintable, and did the AC1 vocabulary land in both bound authorities); this is a repair ordered by a persisted verdict, whose input produces a new snapshot by design, so the anomaly is printed and routed below, never a stop.
- Phase-lint re-derived with the shipped linter: `node scripts/phase-lint.mjs docs/fix/224-deterministic-replan-routing/SPEC.md` → `verdict PASS`, `Phase-lint: PASS (8/8)` for all fourteen phases (P1–P7, P9–P12, P8, P13, P14), exit 0; every per-phase fingerprint matches the SPEC `### Phase-lint` block, including `P13:config/infra:4:archived-unit-state` and `P14:hardening:10:hardening-pr`; overall fingerprint `dd3e140687ae85fff9b44b46478a840ae354375ee94c7df9321b61f0573aa27b`, matching the recorded value.
- **F25 exemption mutant test** (the hand-off's load-bearing question, reproduced on a `/tmp` copy of the bound linter, reviewed tree untouched): with `scripts/phase-lint.mjs:636`'s exemption branch disabled, the same SPEC lints `P8 box-3: phase has 10 tasks (limit 8 for layer hardening)` + `P8 box-7: task 5 carries a manual/external gate outside the hardening phase` → `verdict BLOCKED: lint-blocked`, exit 1. So the exemption is load-bearing and does exactly what its owner text claims: it skips boxes at index 2 and 6 for a fully-ticked phase only, P13 (unticked) still passes 8/8 with every box armed, and the two skipped reasons are purely positional (`P8` was the final close-out when it ran; the replan moved the last position to `P14`). No live box-4…8 defect is hidden — `P8` passes boxes 1, 2, 4, 5, 6 and 8 with the exemption off. `skills/phase-contract/SKILL.md` (version 1.0.5, `:57`) states the same rule owner-side ("a fully-ticked phase is historical", "boxes 1, 2, 4, 5, 6 and 8 stay armed", "pre-ticking a phase to dodge a check is a defect"), and `scripts/phase-lint.test.mjs:345-383` carries the corpus quadruple (executed passes; unticked blocks box 7; pre-ticked + box-8 defect blocks; pre-ticked + box-4 defect blocks).
- Repository gate at the bound revision: `NODE_PATH=packages/agentic-workflow-schema/node_modules node --test scripts/*.test.mjs` → exit 0 · 423 pass / 0 fail.
- Pi package suite: `cd packages/pi-agentic-workflow && bun run test` → 214 pass / 0 fail (includes `skill-parity.test.mjs`).
- Router licence for this review, re-derived: `node scripts/unit-route.mjs 224-deterministic-replan-routing` → `status: done`, `open-rows: 1`, `route: replan`, `next: /plan-fix 224`, `rows: F32`, exit 0 — the row that ordered the replan, still the only open row.
- `F32` re-read as data: `review-findings.md:234` → `replan-in-unit (plan-owned): the archived half of F24 — give the closed route table a historical/merged state for a unit whose status source is gone (AC1 vocabulary + fixtures), via the plan owner`, `folded: no`. The five source-owned rows of the same cycle (`F29`, `F30`, `F31`, `F33`, `F34`) read `folded: yes`.
- New-phase evidence re-derived, never copied: `scripts/unit-route.mjs:37` `ROUTES` is still the six-token set (`replan`, `decision`, `fold`, `execute`, `close-out`, `plan-from-issue`) and `historical` appears nowhere in the router (`grep -n historical scripts/unit-route.mjs` → 0 hits), so `P13`'s and `OB-17`'s deliverable is genuinely unexecuted; `scripts/normative-drift.test.mjs:1093` still pins the six tokens, so `P13` task 3's seven-token extension is genuinely pending; `status === "absent"` is reachable for a known unit folder with no index/roadmap row (`scripts/unit-route.mjs:384`), so the archived state `P13` targets exists and currently falls through to `route: execute`.
- Census drift re-checked (PE-007): `grep -rn "replan-in-unit" skills/ docs/workflow/ --include="*.md"` → 44 hits / 22 files at the bound revision against PE-007's 43/21 at `48aac035`. The row pins its observation revision; re-reading the 8 files outside the 14 converged routing surfaces shows none sends a replan row to `execute-phase` or the fold (`WORKFLOW_INVARIANTS.md:81` routes *unresolved loop-cap* findings to `triage-issue --prioritize-now`, a different state; `execute-phase/references/OPPORTUNISTIC_FINDING.md:21`, `review-change/SKILL.md:157`, `review-change/references/REVIEW_PROCESS.md:152` and `docs/workflow/SKILLS.md:227` name no destination). Immaterial pointer drift, recorded, not carried as a material row.
...
- L2 pass — PE-001…PE-015 are all `current` + `proven`, every row pins the revision it was observed at, and the two load-bearing rows of this replan re-verify (`PE-011`: `scripts/phase-lint.mjs` present; `PE-013`: row 37 `done · [#212]`). No `unknown` row lacks an owner; no `drifted`/`stale` row survives; the PE-007 census drift above is recorded as a non-material observation, not a row.
- L3 pass — OB-1…OB-17 cover every normative behaviour the fix adds (route decision, conditional load, bounded read, canonical destination, machine signal, fail-closed exits, budget/discoverability, mirror parity, determinism/read-only, sanitizer, version signal, terminal route, bare status token, executed-phase exemption, contract documentation, archived-unit state) and every required failure state (S1–S11); 17 unique ids, none missing, none duplicated, none `deferred`.
- L4 pass-with-finding — every row names exactly one phase and one task, an `execute-phase` owner and a validator copied from `ACCEPTANCE.md`/the phase done-when: OB-17 → P13/2 with `node --test scripts/unit-route.test.mjs` → exit 0 (the archived-state pins), matching `ACCEPTANCE.md` AC1's validator. Its `status` is `planned`, which is the honest pre-execution state — but see RP-224-16: the ledger the replan appended has no phase left that can move it to `verified`, while `audit-pr`'s obligation-closure gate blocks any `planned` row.
- L5 finding (**RP-224-18**) — S1–S11 each map to a phase and a validator that can fail, but the replan's own new failure category — a known unit with no open row whose status source is gone, mis-routed to the executor — has no scenario row, although S9/S10/S11 were added for the equivalent F23/F24/F25 categories.
- L6 finding (**RP-224-20**) — RP-224-1…RP-224-15 read `resolved` except RP-224-6, whose row carries 11 cells against the 10-column ledger shape: its `status` cell is the reviewer's `verified: …` prose, its `resolution-evidence` cell is the bare word `resolved` and its `resolving-artifact-revision` cell is the resolution prose, so the row's status is outside the closed `open | resolved | dismissed` vocabulary. No `dismissed` row exists and no open material row is being carried into execution (`review-findings.md` open rows = F32 only, which this review routes).
- P1 pass (every affected surface is named with a `path:line` evidence row or a Scope/Impact entry; the invariant posture is `## Rules that must never be violated` — frozen classification, linter grammar, agnosticism, budget and mirror constraints) · P2 pass (the closure is empty and correctly so: unit 37 is merged and every phase's input exists on the branch; `P13` needs only `scripts/unit-route.mjs` and its two pins) · P3 pass (`next.suggested` already exists at `envelope.schema.json:169`; the amended route vocabulary is the router's own contract; `review-change`/`fold-findings` classification is untouched) · P4 pass (the sanitizer owns the echoed ids/paths via OB-10/AC12/S7; the archived-state read adds no injection surface; no secrets/auth/PII) · P5 pass (no migration; EN+ES pairs landed; the mirror re-bundled) · P6 pass (per-phase receipts, idempotent `P8`/`P14` re-entry, exit-0 validators, the receipt tasks ordered after every tick) · P7 pass (the revert path is executable — revert the single PR, now naming the shared-linter edit and the dead ends a revert restores; the archived-unit mis-route a revert would restore is not named in the list, immaterial and recorded here) · P8 pass (the router block, the sensor's `next.suggested`, the terminal route, the docs and the suites observe the shipped behaviour) · P9 **finding** (RP-224-16: the ledger's fresh final close-out `P14` does not close the obligation the replan itself appended, so the unit's terminal state cannot satisfy `audit-pr`'s obligation-closure gate) · P10 pass (every phase's done-when is a command with an expected outcome on the repo's real gates; red-first explicit for P13; no validator weakened — the F25 exemption is owner-side documented, corpus-pinned and mutant-verified above, and P13's drift-pin task adds a token rather than editing an assertion) · P11 **finding** (RP-224-18: the archived-state failure category has no scenario row) · P12 **finding** (RP-224-19: the frozen manifest's AC1 enumerates six route tokens while the SPEC mirror and the manifest's own freeze note enumerate seven) · F1 pass (the operator report and the cited gates reproduce the dead end; the router supplies the missing deterministic selection) · F2 pass (four causes at `path:line`; `F32`'s cause is the unhandled `status === "absent"` fall-through at `scripts/unit-route.mjs:384`, which `P13` edits) · F3 **finding** (RP-224-17: `## Impact`'s blast-radius statement denies the route change `P13` makes) · F4 pass (revert; this receipt states plainly that no Product review preceded it — no fake Product-half ceremony).

### CONVERGENCE-ANOMALY — fix-224 plan (replan re-review)

- Finding ids: repeated: none / new: RP-224-16, RP-224-17, RP-224-18, RP-224-19, RP-224-20
- Snapshots: 3d5009783b84b41be4c4000dc3ae0f7538a500a30c07c01824033fb1a74734fa → 5286729185a902f0315236766f8eca3781d786fdddfbc5961d228a8e9901de02 (artifactRevisionId 72f2f58d0ebf19189c77b2e3e8e8b4213e4b7548 → 60993f44dc751cd162e1bddbf088d5ba54c299c7, label `fix-224-artrev-0008`)
- Missed: the terminal obligation reconciliation the previous close-out carried (P12 task 1) — the fresh `P14` closes the appended phase's deliverable but not the appended obligation row; the `## Impact` blast-radius sentence "No existing route changes for a unit with no plan-routed row", which `P13` falsifies; a scenario row for the archived state; and the manifest half of the AC1 vocabulary amendment.
- Owning stage: plan
- Why the prior repair/replan failed: the artrev-0008 replan added the archived-state behaviour, its obligation and the vocabulary amendment, but wrote them as a router-only extension — it did not rotate the sections that describe the unit's whole terminal state (`## Impact`, the scenario matrix, `ACCEPTANCE.md` AC1's route list) nor schedule the obligation reconciliation the append shape's own fresh close-out needs.
- Route to owner: `plan-fix 224` — one repair batch over RP-224-16…RP-224-20, rotate the artifact revision, then `/review-plan fix-224` re-reviews the new snapshot.

### Verdict

```text
PLAN-REVIEW-FAIL — fix-224 BLOCKED
- Snapshot: 5286729185a902f0315236766f8eca3781d786fdddfbc5961d228a8e9901de02 · Artifact revision: 60993f44dc751cd162e1bddbf088d5ba54c299c7
- Failed checks: P9, P11, P12, F3, L5, L6
- Findings (unioned, one row each):
  | id | severity | class | check | claim | evidence | verification |
  |---|---|---|---|---|---|---|
  | RP-224-16 | medium | plan | P9 | `P13`'s obligation `OB-17` has no phase left that can move it from `planned` to `verified`: the replan appended `P13` and a fresh final `P14`, but `P14` carries only the canonical close-out chain (gate, pending-docs, fix-index/PR link, the three receipts) and `P13`'s four tasks never touch the ledger — while `P12`, whose task 1 owned the reconciliation, is already executed and historical, so `audit-pr`'s obligation-closure gate blocks the terminal head on a `planned` row | `SPEC.md:206` (OB-17 `status: planned`), `SPEC.md:757-769` (`P13` tasks), `SPEC.md:771-792` (`P14` tasks), `SPEC.md:723-732` (`P12` done-when "every `OB-` row … carries `verified` or `n/a`" and task 1 the reconciliation), `skills/audit-pr/references/02_CLOSURE_AND_SCOPE_GATES.md:101-105` ("Any `planned`, `in-progress`, blank, or `deferred` row is BLOCKED, naming the ids"); the same phase's own precedent: at `0e35b189` (after P1–P7 executed) OB-1…OB-11 all still read `planned` and were flipped only by `P12`'s explicit task at `da9d0df2` — the executor does not reconcile per phase, so the plan must schedule it | bytes re-read at the bound revision; `P14` is already at the final close-out box-3 cap (10 tasks), so the repair is a re-cut, not an append |
  | RP-224-17 | medium | plan | P12, F3 | `## Impact`'s blast-radius statement was not rotated by the replan and is now false: it reads "The router is additive and read-only; the envelope gains one optional array. No existing route changes for a unit with no plan-routed row", while `P13` changes exactly that route — a known unit with no open rows and a gone status source moves from `route: execute` to `route: historical` (OB-17, AC1, `P13` fixture pins), so the regression-scope disclosure denies the behaviour the replan adds | `SPEC.md:310-315` (the blast-radius sentence) vs `SPEC.md:757-769` (`P13`) and `SPEC.md:206` (OB-17); `scripts/unit-route.mjs:384` (`status` computed before the route decision) and `:403-409` (the `status === "done"` / fall-through `execute` branch) show the changed fall-through; `grep -n historical scripts/unit-route.mjs` → 0 hits at the bound revision | re-read at `60993f44`; the section was last rotated by the artrev-0004 replan ("from the replan" names P9–P12), which predates the archived state |
  | RP-224-18 | low | plan | P11, L5 | The replan added a new failure category — a known unit with no open row whose status source is gone, mis-routed to the executor — without a scenario row: the matrix still ends at S11, while the equivalent F23/F24/F25 categories each gained a row (S9/S10/S11) in the previous cycle, and `S4`'s "the unit exists with no open row" validator covers only the `planned`/`done` halves | `SPEC.md:248-266` (S1–S11, `S4`, no archived row), `SPEC.md:757-769` (`P13`), `SPEC.md:206` (`OB-17`), `SPEC.md:215` (AC1's `historical`); `RP-224-11`'s resolution ("The scenario matrix gained S9 …, S10 … and S11") fixes the same expectation for the previous cycle | `grep -c` over the matrix section → 11 rows; the archived state is a named state of the bound SPEC with no scenario pointer |
  | RP-224-19 | low | plan | P12 | The manifest half of the AC1 vocabulary amendment did not land: `ACCEPTANCE.md` AC1 still enumerates six route tokens (`replan`, `decision`, `fold`, `execute`, `close-out` and `plan-from-issue`) with `historical` appearing only in the trailing `where` clause, while the SPEC's `## Acceptance` mirror enumerates seven and the manifest's own freeze note claims "AC1 gains the seventh route token (`historical`, F32)" — a byte diff of the two bound rows is exactly the missing `` `historical`, `` list entry | `ACCEPTANCE.md:16-18` (freeze note) and `:23` (AC1, six-token list) vs `SPEC.md:215` (AC1, seven-token list); `diff` of the two AC1 rows → only `close-out` vs `` `close-out`, `` + `` `historical` ``; both rows carry the same validator (`node --test scripts/unit-route.test.mjs`) | re-derived at the bound revision; same genus as RP-224-12 (the mirror must rotate with the frozen manifest), narrower in effect because `historical` is named in both texts |
  | RP-224-20 | low | plan | L6 | `planning-findings.md`'s `RP-224-6` row violates the ledger's own shape: it carries 11 cells against the 10-column definition, so its `status` cell is the reviewer's `verified: P4/P6 contain no review-change bump task…` prose, its `resolution-evidence` cell is the bare word `resolved` and its `resolving-artifact-revision` cell is the resolution prose — the row's status is outside the closed `open \| resolved \| dismissed` vocabulary that the ledger contract and the plan-review L6 check require | `planning-findings.md:14` header (`finding-id \| … \| status \| resolution-evidence \| resolving-artifact-revision`), the `RP-224-6` row (11 cells; all 14 other rows 10), `skills/pre-execution-review/references/LEDGERS.md:97` (`status` is `open \| resolved \| dismissed`), `skills/review-plan/references/CHECKS.md` L6; `scripts/pre-execution-quality.test.mjs:562` treats a material row whose `status !== "resolved"` as `PLAN-REVIEW-FAIL` in its pure model | cell counts recomputed programmatically (RP-224-6 = 11, RP-224-1…5 and 7…15 = 10); pre-existing at `fix-224-artrev-0003`, untouched by this replan, and latent — no live consumer indexes the row today |
- Repair owner: `plan-fix 224` — one batch over this whole set
- Parent state: n/a — fix unit, no Product half exists (D6); no Product lineage to go stale
...
Command and its JSON answer, run in this same act as the append above: the
newest receipt is this one, `structural.fresh: true` and `digestMatches: true`;
`current: false` / `exit 4` is the sanctioned form for a persisted FAIL verdict
(POLICY §8: "`exit 4`/`current: false` means a verdict persisted but not a PASS —
the verdict itself is the emit result, route per verdict").
...
NODE_PATH=packages/agentic-workflow-schema/node_modules node scripts/pre-execution-snapshot.mjs verify --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing
{
  ...
  "observedDigest": "5286729185a902f0315236766f8eca3781d786fdddfbc5961d228a8e9901de02",
  "digestMatches": true,
  "verdictIsPass": false,
  "structural": {
    "fresh": true,
...
## Pre-execution review receipt v1 — plan
- Review: rp-224-20260915-010 · Snapshot: e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8 · Verdict: plan-review-pass
- Unit: fix-224 · Stage: plan · Unit kind: fix
- Parent SPEC snapshot: null · Parent Product receipt: none
- Parent note: fix unit — no Product half exists (D6); the contract forbids a parent on a fix plan snapshot (D30)
- Source revision: 19d8e579744cdce3ac8ef7e2ecc5a527bef99e41 · Artifact revision: 19d8e579744cdce3ac8ef7e2ecc5a527bef99e41
- Reviewer: review-plan (fresh pi session) · Session: terminal plan re-bind after P13/P14 executed on `fix/224-deterministic-replan-routing` · Role: reviewer · Author: plan-fix (replans/repairs) + execute-phase (phase P13/P14 ticks)
- Author exclusion: not-enforceable · Context clean: true
- Model diversity: same-model · Policy: v1
- Context-clean note: this conversation wrote or replanned no part of the unit; the only writes are this receipt block, its self-check output, the two immaterial finding rows it emits and the evidence commit — no reviewed artifact was touched.
- Started/finished: 2026-09-15T23:07:00Z/2026-09-15T23:22:00Z · Findings: 2 (material open: 0)
- Ledgers read: planning-evidence 15 rows (PE-001…PE-015, embedded in the SPEC) · obligations 17 rows (OB-1…OB-17, embedded; verified-capable: 17 — every row reads `verified`)
- Prior plan receipt (re-review only): rp-224-20260915-009 @ 6089a9b6e4ff82e94ecea08dfd4b2dee2a272bef105d852f1317d7d6b0bb6570
- Portability note: the hand-off named `fix-224-artrev-0010` = `19d8e579`; the builder resolved `sourceRevision`/`artifactRevisionId` to `19d8e579744cdce3ac8ef7e2ecc5a527bef99e41` (`contentRevision` over the bound paths), so the label and the bound bytes agree (POLICY §7: the claimed value recorded beside the recomputed one, never substituted). HEAD equals that commit — no unbound `progress.md`-only commit stands above it; `git status --porcelain` was empty before the review writes. The schema package was present (`NODE_PATH=packages/agentic-workflow-schema/node_modules`); the authoritative validator executed with no refusal and no install.

### Review-run evidence (commands + results)

- Branch verified first: `git branch --show-current` → `fix/224-deterministic-replan-routing`; `git status --porcelain` empty before the review writes; `git rev-parse HEAD` → `19d8e579744cdce3ac8ef7e2ecc5a527bef99e41`.
- Snapshot build: `NODE_PATH=packages/agentic-workflow-schema/node_modules node scripts/pre-execution-snapshot.mjs build --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing` → digest `e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8`, schema-validated by the recipe owner; `unitKind: fix`, no `--parent` passed, `parentSpecSnapshotDigest: null`. Artifacts: `acceptance` (7071 B, digest `2ab6e4d6c16193ba1c96e1512edb0170b14f49e1387633a6060f2f395ac69a31`) + `spec` (68966 B, digest `661eda7a22b601f6b4bb4d740db15b3763ae2c2a61e1536cab7d40bf17c814a4`); the fix unit keeps both frozen planning ledgers embedded in the SPEC and bound through the whole-file `spec` row, so their own rows are `absent` — never pointed at non-existent files. Contexts: `project-guide` (`CLAUDE.md`) + `normalized-repository-state` (`docs/workflow/REPOSITORY_STATE.md`) present and bound; `architectural-invariants` absent (`docs/architecture/` does not exist — honestly unbound).
- Acceptance manifest blob re-derived, never copied: `git hash-object docs/fix/224-deterministic-replan-routing/ACCEPTANCE.md` → `13580b459b1d93e8e80176ecf47311ca165b68fa`, equal to the value recorded in the SPEC `## Status` ✓ (the snapshot's own `acceptance` digest `2ab6e4d6…` is a different algorithm over the same bytes, the shape cycles 1–9 bound).
- Prior-receipt staleness re-derived, not assumed: `verify --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing` before this receipt → `current:false`, `digestMatches:false`, `reasonCode: stale-source-revision`, `changedPaths: ["docs/fix/224-deterministic-replan-routing/SPEC.md"]` — exactly the ledger write `P13`/`P14` made, so the previous PASS `rp-224-20260915-009` is stale by construction over a bound artifact. Per the hand-off this is the review's reason to exist; it is **not** reported as a finding.
- **P13 re-derived, not copied** (the archived-unit state): `node scripts/unit-route.mjs 224-deterministic-replan-routing` → `status: done`, `open-rows: 0`, `route: close-out`, `next: /audit-pr`, `rows: none`, exit 0 — the live router now answers the closed state, so `F32` is genuinely folded and no open row survives. `node scripts/unit-route.mjs 100-stale-fix-index-rows` → `status: absent`, `open-rows: 0`, `route: historical`, prose `next:`, exit 0 — a real merged fix unit whose index row is gone, no longer mis-routed to the executor. `scripts/unit-route.mjs:38` `ROUTES` carries the seventh token (`replan`, `decision`, `fold`, `execute`, `close-out`, `historical`, `plan-from-issue`), the header (`:16`) and the usage block (`:334`) document it, and the fall-through that answers it sits at `:415`; `scripts/normative-drift.test.mjs:1093` pins the same seven-token list. The two new fixture units exist (`scripts/fixtures/unit-route/docs/fix/23-merged-fix`, `scripts/fixtures/unit-route/docs/features/16-merged-unit`) and `scripts/unit-route.test.mjs:308` (`F32/OB-17`) asserts `historical` + `status: absent` for both.
- **P14 re-derived**: every `P14` box in the SPEC `### P14` body reads `[x]` in one ledger write, the fix-index row `docs/fix/README.md:30` already reads `done · [#225](https://github.com/gtrabanco/agentic-workflow/pull/225)` (no flip, no link commit, no empty commit), and the whole phase ledger — `P1`…`P13` plus the final `P14` — carries no unticked task.
- Obligation ledger swept row by row, not sampled: all seventeen `OB-` rows carry a non-blank `status`; `OB-1`…`OB-17` read `verified` (the `OB-17` flip was `P13` task 5 as the row's own required-evidence names), none is `planned`/`in-progress`/`deferred`/`n/a`-without-evidence. Spot-checked validators re-run: `grep -c "scripts/unit-route.mjs"` → 1 each for `skills/plan-feature/SKILL.md` and `skills/plan-fix/SKILL.md` (OB-2); the eight converged skill files each ≥1 `unit-route` hit (OB-4); the six tutorial files each ≥1 (AC7); `grep -q "close-out" skills/replan-findings/SKILL.md` ✓ (OB-15); `grep -q "fully-ticked phase is historical" skills/phase-contract/SKILL.md` ✓ (OB-16); `skills/review-change/SKILL.md:4` `version: 3.5.1` + `| 3.5.1 |` cells in both `CHANGELOG.md` and `CHANGELOG.es.md` (OB-11).
- Phase-lint re-derived with the shipped linter: `node scripts/phase-lint.mjs docs/fix/224-deterministic-replan-routing/SPEC.md` → `verdict PASS`, `Phase-lint: PASS (8/8)` for all fourteen phases, exit 0; every per-phase fingerprint in the SPEC `### Phase-lint` block is reproduced, including `P13:config/infra:5:archived-unit-state` and `P14:hardening:10:hardening-pr`; overall fingerprint `1268a833c5fde7351e3dfbdcd591cba7de3737dee21be99880d5679f73697191`, matching the recorded value.
- Repository gate at the bound revision: `node --test scripts/*.test.mjs` → exit 0 · **424 pass / 0 fail**. Pi package suite: `cd packages/pi-agentic-workflow && bun run test` → **214 pass / 0 fail** (includes `skill-parity.test.mjs`).
- Source-evidence re-derivation (P12): `scripts/workflow-status.mjs:1192` `next.suggested = nextSuggested;` (PE-005), `:742` `function readFixNow` and `:753` the projection push (PE-006); `docs/features/ROADMAP.md:47` row 37 = `done · [#212]` and `scripts/phase-lint.mjs` present (PE-011/PE-013); the three cited routing surfaces re-read (`skills/review-change/references/PERSIST_AND_DECIDE.md:134`, `skills/fold-findings/references/FOLD_PROCESS.md:46,:61`, `skills/triage-issue/references/REVIEW_FINDING_PROCESS.md:17-21`). One residual is recorded below as immaterial.
- Read-only proof: the review's only writes are this evidence block, the two new rows in `planning-findings.md` and the evidence commit; `git status --porcelain` before those writes listed no reviewed plan artifact.

### Ledger sweep L1–L6 + Engineering/fix checks

- L1 pass — the snapshot binds `parentSpecSnapshotDigest: null` and this receipt says so plainly; the fix unit claims no Product parent (D6/D30), so there is no lineage to forge and no `review-spec` upstream to route.
- L2 pass — PE-001…PE-015 are all `current` + `proven` (15× `| current | proven |`, no `drifted`/`stale` row, no owner-less `unknown`); the load-bearing rows re-verify (`PE-005` `:1192`, `PE-006` `:742,753`, `PE-011` linter present, `PE-013` row 37 `done · [#212]`). The residual line-window drift of the root-cause prose is recorded as immaterial, not a `drifted` row.
- L3 pass — OB-1…OB-17 cover every normative behaviour the fix adds and every required failure state (S1–S12); 17 unique ids, none missing, none duplicated, none `deferred`.
- L4 pass — every row names exactly one phase and one task, an `execute-phase` owner, a validator copied from `ACCEPTANCE.md`/the phase done-when, and `required-evidence`; no blank status. `OB-17` → `P13`/2 with its flip owned by `P13`/5, and the executed `P13` receipt records the flip.
- L5 pass — S1…S12 each map to a phase and a validator that can fail; S12 (the archived state) → `P13` → the two pins at `scripts/unit-route.test.mjs:308`; S8's absent-ledger case is exercised by the `14-empty-unit` fixture, not a no-op; S9/S10/S11 keep their `P9`/`P10` pins.
- L6 pass — `planning-findings.md` reads `resolved` for RP-224-1…RP-224-20, each with resolution evidence and a resolving artifact revision; no `dismissed` row exists; RP-224-21 is `open` at the only immaterial severity (`info`), and this review's two new rows are also `info`. No open material row is carried into execution. Both new rows are recorded below.
- P1 pass (every affected surface is named with a `path:line` evidence row or a Scope/Impact entry; the invariant posture is `## Rules that must never be violated` — frozen classification, linter grammar, agnosticism, budget and mirror constraints) · P2 pass (the closure is empty and correctly so: unit 37 is merged; `P13` needed only `scripts/unit-route.mjs` and its pins, both on the branch) · P3 pass (`next.suggested` already exists at `envelope.schema.json:169`; the amended route vocabulary is the router's own contract; `review-change`/`fold-findings` classification untouched) · P4 pass (the sanitizer owns the echoed ids/paths via OB-10/AC12/S7; the archived-state read adds no injection surface; no secrets/auth/PII) · P5 pass (no migration; EN+ES pairs landed; the mirror re-bundled and parity-gated) · P6 pass (per-phase receipts, idempotent `P8`/`P14` re-entry, exit-0 validators, the receipt tasks ordered after every tick) · P7 pass (the revert path is executable — revert the single PR; `## Impact` now names the `execute` → `historical` change the revert restores for the archived state) · P8 pass (the router block, the sensor's `next.suggested`, the terminal/close-out routes, the docs and the suites observe the shipped behaviour) · P9 pass (all fourteen phases lint `PASS (8/8)` with their recorded fingerprints; the ledger ends with the executed final `P14 Hardening & PR`; order follows the `Depends on` closure and the `F25`-exempted append shape) · P10 pass (every phase's done-when is a command with an expected outcome on the repo's real gates; red-first explicit for `P13`; no validator weakened — the drift-pin task adds tokens rather than editing an assertion) · P11 pass (`S12` closes the archived-state category; S1–S12 all map to a phase and a validator) · P12 pass (the plan's substantive file/symbol claims match the repository at `sourceRevision`, re-derived above; two inline line-windows drifted from their evidence rows and are recorded as immaterial) · F1 pass (the operator report and the cited gates reproduce the dead end) · F2 pass (four causes at `path:line`; `F32`'s cause was the unhandled `status === "absent"` fall-through, which `P13` edited at `scripts/unit-route.mjs:415`) · F3 pass (`### In scope`, `## Impact` and the scenario matrix name the router, the pinned files, the shared-linter edit and the callers) · F4 pass (revert the single PR; this receipt states plainly that no Product review preceded it — no fake Product-half ceremony).

### CONVERGENCE-ANOMALY — fix-224 plan (terminal re-bind after execution)

- Finding ids: repeated: none / new: RP-224-22, RP-224-23 (both `info`, immaterial)
- Snapshots: 6089a9b6e4ff82e94ecea08dfd4b2dee2a272bef105d852f1317d7d6b0bb6570 → e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8 (artifactRevisionId `32a6c212` → `19d8e579`, label `fix-224-artrev-0009` → `fix-224-artrev-0010`)
- Missed: nothing ordered — the prior PASS emitted no repair. The delta is the executed `P13`/`P14` ledger write, which ticks `SPEC.md` (a bound artifact) and therefore rotates the receipt by construction; two pre-existing annotation defects ride along (`RP-224-21` unchanged, plus the two new `info` rows).
- Owning stage: plan (annotation only)
- Why the prior review/repair "failed": it did not — the previous PASS is stale because a later sanctioned write moved a bound artifact, which is the contract working, not a defect.
- Route to owner: none on this PASS; the three `info` rows ride with the plan and only `plan-fix 224` may touch them if the artifact is edited again.

### Verdict

```text
PLAN-REVIEW-PASS — fix-224
- Snapshot: e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8 · Artifact revision: 19d8e579744cdce3ac8ef7e2ecc5a527bef99e41 (fix-224-artrev-0010) · Checks: L1–L6 + 16/16 Pn + 4/4 Fn
- Obligations: 17 rows, none blank/deferred/unvalidated (all `verified`) · Material findings open: 0
- Read-only: no plan artifact modified
- Authority: execution may bind this receipt for this exact snapshot
```
...
NODE_PATH=packages/agentic-workflow-schema/node_modules node scripts/pre-execution-snapshot.mjs verify --stage plan --unit fix-224 --dir docs/fix/224-deterministic-replan-routing
{
  ...
  "unit": "fix-224",
  "receipt": {
    "id": "rp-224-20260915-010",
    "verdict": "plan-review-pass",
    "snapshot": "e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8",
    "authorExclusion": "not-enforceable",
    "contextClean": "true",
    "policy": "v1"
  },
  "observedDigest": "e7309f4cc87489bd1de913be189e116e87915ff9aeec93c01aa7135c1c16fcc8",
  "digestMatches": true,
  "verdictIsPass": true,
  "structural": {
    "fresh": true,
