...
| F32 | scripts/unit-route.mjs:344,370 | workflow | med | fix-now | replan-in-unit (plan-owned): the archived half of F24 — give the closed route table a historical/merged state for a unit whose status source is gone (AC1 vocabulary + fixtures), via the plan owner | yes |
