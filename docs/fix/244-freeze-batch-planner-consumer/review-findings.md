...
| F9 | docs/fix/244-freeze-batch-planner-consumer/progress.md:47 | code | med | fix-now | fold into current unit — replace the dead PR reference `#246`/`pull/246` with the unit's real PR #247 (`docs/fix/README.md:18` already cites it); CLAUDE.md requires documented cross-references to resolve | yes |
...
| F10 | skills/fold-findings/references/FOLD_PROCESS.md:46 (+ skills/fold-findings/SKILL.md:121-126,192) | brand/docs | med | fix-now | fold into current unit — split the freeze-batch consumer by the router's emitted conclusion: `replan-in-unit` → `/plan-fix <n>` / `/plan-feature <slug>`, `decision-required` → stop and surface the decision to the user, never a planner; extend the `#244` pin to the decision branch | yes |
