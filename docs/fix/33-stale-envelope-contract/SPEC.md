...
- Found during execution by this SPEC's own acceptance grep (plan amended,
  ...
  `package.json` `description` and the `src/index.ts` doc comment —
  comment/metadata only, zero behavior change, covered by the existing tests.
- CHANGELOG.md + CHANGELOG.es.md rows for `orchestration-envelope` 1.1.1.

### Out of scope

- Any behavior/schema change (the JSON Schema, types, and validators are
...
- `grep -n "every user-facing skill prints\|Every \*\*user-invocable\*\* skill"`
  over `skills/` and `packages/` returns nothing.
- `orchestration-envelope`'s description and opening section state the
  ...
  dated, below.
- `version: 1.1.1` in the skill frontmatter; CHANGELOG EN/ES rows present.
- Gate green: `npx skills add . --list` exit 0; `npm test` in
  `packages/agentic-workflow-schema` passes (README-only touch, tests prove no
