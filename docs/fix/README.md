...
|---|---|---|---|
| [#221](https://github.com/gtrabanco/agentic-workflow/issues/221) | sensor-null-parent-receipt | `done · [#223](https://github.com/gtrabanco/agentic-workflow/pull/223)` | workflow-status falsely reports fix plan receipts as missing: `Parent SPEC snapshot: null` parses as the string "null" (`pre-execution-contract.mjs:132`), the sensor spawns `verify --parent null` (`workflow-status.mjs:598-599`), and the verifier refuses `invalid-value@/parentSpecSnapshotDigest` — every sensed fix unit with a plan receipt lands a false `plan-stage receipt is missing` gate blocker (fix-191 live on this tree at 170b25f6). Fix: normalize the receipt lineage line through the contract module's own `recordedValue` and never bind a parent for fix receipts in the sensor; pinned parser suite + sensor e2e fixture. Scripts-only; no schema/skill bytes. Independent; #205's verdict-landing tail routes on these rows. |
| [#214](https://github.com/gtrabanco/agentic-workflow/issues/214) | model-selection-over-24-options | `done · [#217](https://github.com/gtrabanco/agentic-workflow/pull/217)` | Settings-console dialogs exceed pi-web's 24-option select cap when the live model registry or command list is long: the adapter's non-TUI pick fallback and the console's select paths forwarded full lists (`extension/index.ts:60-65`, `console.ts:269/371/391`). Fix: shared bounded paged-select helper (21/page + pager entries, `Type another reference…` last), provider-first two-step model selection over the cap, bounded command pickers, adapter paging. Package-only; version bump 0.9.2 + CHANGELOG/README EN+ES pairs same PR. Independent; rich seam from merged #154/#203 untouched ≤ cap. |
| [#154](https://github.com/gtrabanco/agentic-workflow/issues/154) | settings-picker-search-bulk-chain | `done` · [#203](https://github.com/gtrabanco/agentic-workflow/pull/203) | Settings-console picker defects A–D (no filter/scroll/current value, one command per pass) + ordered model fallback chain (E): searchable windowed picker over `ctx.ui.custom` + pi-tui `SelectList`, current-value preselection, field independence, bulk apply/clear, `/aw-settings` alias, `model` chain schema (1–4 refs) probed by dispatch without touching session state; `setModel` ≤ 1 per routed turn. Package-only; README EN+ES + version bump same PR. Independent of #201/#198/#196/#174; branch cut from `main@f48dff00` to stay disjoint from fix #200. |
...
| [#224](https://github.com/gtrabanco/agentic-workflow/issues/224) | deterministic-replan-routing | `done · [#225](https://github.com/gtrabanco/agentic-workflow/pull/225)` | A finding whose frozen route is the plan owner has no route that reaches the planner: both planners' progressive-loading allowlists exclude `review-findings.md`, `plan-feature`'s redirect gate stops on an in-flight unit without reading the ledger, three prose surfaces name three different destinations, and `workflow-status`'s documented `next.suggested` signal is never emitted. Fix: one deterministic router script decides the unit's route from the ledger, prints a bounded read set, and gates the conditional load of one internal replan contract; every destination surface the census names (eight skill files, three tutorial pairs) converges on it. |
