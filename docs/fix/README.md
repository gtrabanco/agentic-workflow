...
|---|---|---|---|
| [#200](https://github.com/gtrabanco/agentic-workflow/issues/200) | over-budget-fixture-stale-guard | `done · [#202](https://github.com/gtrabanco/agentic-workflow/pull/202)` | The `check-skill-context` over-budget fixture's hard-coded 10 000-char body now fits `review-change`'s grown ceiling (2800), so the fail-closed guard is never exercised and the suite is red on `main`. Resize the fixture from the manifest at runtime (effective-ceiling estimate × 2); checker untouched. Suite also carries a pre-existing out-of-scope route-budget red (`test.mjs:112`, 15 exceedances — #176/D2 territory). Independent of #198/#196 — re-base if they land first. |
