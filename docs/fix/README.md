...
| [#244](https://github.com/gtrabanco/agentic-workflow/issues/244) | freeze-batch-next-consumer | `done` · [#247](https://github.com/gtrabanco/agentic-workflow/pull/247) | Fix the freeze-batch `→ Next:` consumer in `fold-findings` from the raw router invocation (`node scripts/unit-route.mjs <unit>`) to the router's conclusion — the planner command (`/plan-fix <n>` / `/plan-feature <slug>`) on `route: replan`, or a stop-and-surface user decision on `route: decision` — document the `<unit>` argument format (bare number or full slug), add the physical-line rule for sub-bullets, and add a pin/test to guard the freeze-batch hand-off. Depends on none. |
