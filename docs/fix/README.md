...
|---|---|---|---|
| [#181](https://github.com/gtrabanco/agentic-workflow/issues/181) | pi-server-devdep-obsolete | `pending` · [no PR yet] | Drop obsolete `@earendil-works/pi-server@0.85.0` devDependency (zero imports, shim only needed for pi 0.85.0's packaging defect fixed in 0.85.1 by #9132); pin peer to `>=0.85.1` to prevent stale installs; refresh README baseline note 0.85.0→0.85.1. |
