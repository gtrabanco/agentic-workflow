...
| `65-fold-findings-skill` | New strict skill `fold-findings`: takes a `review-change`/`audit-pr` findings ledger and truly fixes each fix-now finding (frozen classification, definition-of-fixed `✓`-list, forbidden `✗`-list closing every escape hatch, one-commit-per-finding, `FOLDED`/`DISPUTED→triage`/`BLOCKED` verdicts) + minimal hand-off wiring (`review-change` FAIL `→ Next`, `execute-phase` fold-cycle reference) + registration (model-routing, SKILLS EN/ES, README tables, CHANGELOG) | done | — | [#65](https://github.com/gtrabanco/agentic-workflow/issues/65) |
