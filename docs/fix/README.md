...
| `81-legacy-spec-phase-lint-carveout` | Restore fix #64's backward-compat promise: add a legacy-SPEC carve-out to `execute-phase`'s phase-lint pre-flight guard so a SPEC with no `## Phases` section skips the guard entirely (no lint, no STOP) and runs the legacy single-pass flow — mirroring the exemption already stated in the *Workflows* section; the guard's behavior for phased SPECs is unchanged | pending | — | [#81](https://github.com/gtrabanco/agentic-workflow/issues/81) |
