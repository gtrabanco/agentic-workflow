...
| [#214](https://github.com/gtrabanco/agentic-workflow/issues/214) | model-selection-over-24-options | `pending` | Settings console crashes with "A select dialog must not offer more than 24 options" when the model registry returns > 24 references. Fix: split model selection into two steps — first providers, then models for the chosen provider. Apply the same bounded-list logic to command selection too. |
