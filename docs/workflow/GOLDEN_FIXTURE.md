...
Coverage note (feature 28, P6, 2026-09-01): these two rows plus the 2026-08-31
row cover every executor-path skill this unit changed, at its current text —
review-spec 1.1.0 (2026-08-31 row), review-plan 1.1.0, pre-execution-review
1.2.0, plan-fix 3.0.1, execute-phase 4.0.2, workflow-status 3.0.3, audit-pr
5.0.2. Intermediate versions folded inside the unit's unreleased cycle
(workflow-status 3.0.0–3.0.2, execute-phase 4.0.1, audit-pr 5.0.0/5.0.1,
plan-fix 3.0.0) never shipped separately — the npm publish is blocked by the
unit's known-issue 12 — and their wording is command-pinned by the suites their
fold commits added (`scripts/pre-execution-quality.test.mjs`).
