...
| F011 | Human workflow documentation is English-only (interim, 2026-09-15): the bilingual `.es.md` siblings and their switcher links were removed repo-wide; feature 57 restores them per release. Skills, prompts, SPECs, plans, code, commits, and PRs are English-only. | `AGENTS.md`; `docs/features/ROADMAP.md` row 57 | frozen |

## Accepted decisions

| ID | Decision | Rationale | Evidence |
|---|---|---|---|
| AD-002 | Human docs are English-only until feature 57 restores per-release bilingual siblings; programming artifacts stay English-only. | Removes the per-commit translation duty that doubled documentation cost, without duplicating machine contracts. | `AGENTS.md`; `docs/features/ROADMAP.md` row 57 |
| AD-004 | One implementation PR per delivery unit against `main`; direct-main work is exceptional and requires explicit user authority. | Keeps implementation units independently reviewable. This snapshot records an explicitly authorized documentation-only exception. | `AGENTS.md`; 2026-08-30 user instruction |
