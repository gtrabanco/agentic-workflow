...
| `review-implementation` | Encontrar → clasificar → tabla de decisión en dos fases (fix-now / replan-in-unit / decision-required / proposal); solo hallazgos, sin refactorizar. `user-invocable: false` — el motor que compone `review-change` (y que reutilizan `audit-pr` / `product-audit`) |
