...
| `review-plan` | **Puerta de ingeniería independiente.** El mismo contrato sobre el plan congelado (snapshot `stage: plan`: SPEC, `ACCEPTANCE.md`, evidencia de planificación, obligaciones, fases, tests; el digest del snapshot de producto padre es obligatorio): barrido de libros L1–L6, comprobaciones P1–P12 (F1–F4 en unidades de fix), luego `PLAN-REVIEW-PASS` / `PLAN-REVIEW-FAIL` + recibo. | `execute-phase` con un PASS vigente; un vacío que requiere inventar intención de producto → `design-feature` |
...
| `evidence-grounding` | Contrato de evidencia del lado del autor + preverificación de preparación (`stage: spec` y `stage: plan`): fila afirmación→autoridad→artefacto, el vocabulario fijo de preparación y la rotación de revisión que permite a un revisor detectar su propia escritura, más el pase delegado de solo lectura cuya lectura extensa se conserva en un `delegated-evidence.md` versionado por unidad (advisory hasta que la habilidad autora comprueba sus citas; `partial` o `blocked` bloquea la preparación) — y rige a quien lee y no escribió el artefacto, cualquiera que haya preguntado.. `user-invocable: false` — compuesta por `design-feature`, `plan-feature-scaffold` y `plan-fix`; nunca puede emitir un PASS de revisión |
| `pre-execution-review` | Dueño único del ciclo de revisión común (independencia de contexto limpio, etiquetas veraces de exclusión de autor y diversidad, hallazgos unidos, rechazo solo con contraevidencia, contenido no confiable, clases de reparación, no-progreso, `CONVERGENCE-ANOMALY`, adopción heredada, write-then-report: marca durable de los veredictos terminales y de los rechazos tipados de puerta) y de las formas, hogares y escritores de las tablas de los libros de planificación, incluida la fila de la marca durable de revisión. `user-invocable: false` — compuesta por `review-spec`, `review-plan` y las skills de autoría; no emite veredicto |
