...
| `review-change` + `fold-findings` (camino manual) | Poseer el bucle manual review→fold: ejecutar `/fold-findings`, y luego un nuevo `/review-change` sobre un HEAD cambiado, acotado a dos ciclos (`LOOP CAP REACHED`), y llevar los hallazgos no resueltos a `triage-issue --prioritize-now` para replan o ejecución manual de fases. Nunca fusionar ni descartar hallazgos silenciosamente. |
