...
const dispatchCase = async (args) => {
  ...
  const outcome = await session.dispatch({ name: "unit-lane", skill: "unit-lane" }, args);
  assert.equal(outcome.status, "dispatched");
  return session.log.sendUserMessage;
};

test("AC4: /unit-lane 27-pi-agentic-workflow forwards the skill and its argument verbatim", async () => {
  const sent = await dispatchCase("27-pi-agentic-workflow");
  assert.equal(sent.length, 1);
  assert.equal(sent[0].content, "/skill:unit-lane 27-pi-agentic-workflow");
...
test("AC4: argument tokens keep their order and content", async () => {
  ...
  assert.equal(sent[0].content, "/skill:unit-lane --from-issue 146 --scaffold");
...
test("AC4: a quoted argument with spaces survives intact", async () => {
  ...
  assert.equal(sent[0].content, `/skill:unit-lane ${args}`);
});
...
test("AC4: internal spacing is the operator's, not ours to normalise", async () => {
  ...
  assert.equal(sent[0].content, "/skill:unit-lane 27-pi-agentic-workflow   --fix");
});

test("AC4: no arguments dispatches the bare skill command", async () => {
  const sent = await dispatchCase("");
  assert.equal(sent[0].content, "/skill:unit-lane");
});

test("AC4: the routed model is applied before the skill is dispatched", async () => {
  const session = createSession({
    config: configFor({ commands: { "unit-lane": { model: "openai/gpt-5.2", thinking: "high" } } }),
    models: { "openai/gpt-5.2": { auth: true } },
  });

  const outcome = await session.dispatch({ name: "unit-lane", skill: "unit-lane" }, "27-pi-agentic-workflow");
  assert.equal(outcome.status, "dispatched");
  assert.deepEqual(session.log.setModel, ["openai/gpt-5.2"]);
  assert.deepEqual(session.log.setThinkingLevel, ["high"]);
  assert.equal(session.log.sendUserMessage[0].content, "/skill:unit-lane 27-pi-agentic-workflow");
});
