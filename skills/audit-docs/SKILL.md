...
A read-first audit that answers "do the docs still match reality?" Produces a
...
Run these and collect findings (cite paths/lines/issue numbers for each):
...
3. **Fix index hygiene.** Every entry in the fix index maps to an **open** issue
   and an unmerged branch; flag entries whose issue is closed or whose PR merged
   (they should have been removed). Flag open fix branches missing from the index.
...
5. **Broken intra-doc links.** Relative links/anchors in docs point at real
   files/sections.
6. **Issue references.** Acceptance/known-issues lines referencing `#N` aren't
   pointing at long-closed issues without note.
7. **Language & naming conventions.** Artifacts are in the project's docs
   language (this repo: **English**); file/dir naming matches the project's
   conventions (e.g. kebab-case TS files, PascalCase components).
8. **Invariant tags.** If the project uses invariant/decision IDs (e.g. INV-/D-/
   KI-), spot-check that referenced IDs exist where claimed.

Adapt the list to what the project actually has; skip checks for absent
structures and say so.
...
3. **Fix only on request.** With an explicit `--fix` (or user go-ahead), apply
   the low-risk corrections (remove a merged fix-index row, fix a dead link,
   register a missing roadmap entry). Leave judgment calls to the user.
...
- Complements `plan-feature` (which *creates* the docs this skill audits) and
  `triage-issue` (which keeps the fix index honest).
- Run it standalone at any time; it has no required predecessor.
