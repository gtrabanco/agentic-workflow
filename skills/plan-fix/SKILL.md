...
- **Multiple numbers** → `plan-fix 71 72 73`. Planning-process step 5 ingests
  all issues, then a fixed
...
- **Invalid input** (a non-number token, or an issue number that doesn't
  ...
  partially (see planning-process step 5).
...
- `docs/fix/<primary-issue-number>-<topic>/SPEC.md` — filled from
  ...
  issues merge into one unit (planning-process step 5). When issues merge, the SPEC
...
1. Every invocation: read [planning process](references/PLANNING_PROCESS.md)
   and execute its validation and multi-issue gate. A refusal or invalid input
   stops there.
2. Before asking a material question or writing a SPEC: read [question and SPEC
   contract](references/SPEC_CONTRACT.md).

Resources are normative and one hop from this file. Missing required resource →
stop; never approximate the merge/refusal blocks, phase rules, or SPEC sections.
